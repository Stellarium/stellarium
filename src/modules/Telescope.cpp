...
void TelescopeTcp::performReading(void)
{
	...
	if (rc < 0)
	{
		if (ERRNO != EINTR && ERRNO != EAGAIN)
		{
			qDebug() << "TelescopeTcp(" << name << ")::performReading: " << "recv failed: " << STRERROR(ERRNO);
			hangup();
		}
	}
	else if (rc == 0)
...
void TelescopeTcp::prepareSelectFds(fd_set &read_fds,fd_set &write_fds, int &fd_max)
{
	...
	if (IS_INVALID_SOCKET(fd))
	{
		...
		if (IS_INVALID_SOCKET(fd))
		{
			qDebug() << "TelescopeTcp(" << name << ")::prepareSelectFds: "<< "socket() failed: " << STRERROR(ERRNO);
			return;
		}
		if (SET_NONBLOCKING_MODE(fd) != 0)
		{
			qDebug() << "TelescopeTcp(" << name << ")::prepareSelectFds: " << "could not set nonblocking mode: " << STRERROR(ERRNO);
			hangup();
			return;
		}
		if (connect(fd,(struct sockaddr*)(&address),sizeof(address)) != 0)
		{
			...
			{
				qDebug() << "TelescopeTcp(" << name << ")::prepareSelectFds: " << "connect() failed: " << STRERROR(ERRNO);
				hangup();
				return;
			}
...
//               << "waiting for connection establishment";
		...
		{
			wait_for_connection_establishment = false;
			qDebug() << "TelescopeTcp(" << name << ")::prepareSelectFds: " << "connection established";
	...
	else
	{
		...
		if (wait_for_connection_establishment)
		{
			...
			if (now > end_of_timeout)
			{
				...
				qDebug() << "TelescopeTcp(" << name << ")::prepareSelectFds: " << "connect timeout";
				hangup();
				return;
			}
			FD_SET(fd,&write_fds);
	...
	if (!IS_INVALID_SOCKET(fd))
	{
		if (wait_for_connection_establishment)
		{
			if (FD_ISSET(fd,const_cast<fd_set *>(&write_fds)))
			{
				...
				{
					qDebug() << "TelescopeTcp(" << name << ")::handleSelectFds: " << "getsockopt failed";
					hangup();
				}
				else
				{
					if (err != 0)
					{
						qDebug() << "TelescopeTcp(" << name << ")::handleSelectFds: " << "connect failed: " << STRERROR(err);
						hangup();
					}
					else
					{
						qDebug() << "TelescopeTcp(" << name << ")::handleSelectFds: " << "connection established";
					}
				}
			}
		}
