...
void Translator::reload()
{
	...
	{
		/***** MSVC Stellarium Build *****/
		//Original == snprintf(envstr, 25, "LANGUAGE=%s", Translator::systemLangName.c_str());
...
//		cout << "TEST=" << envstr << " " << Translator::systemLangName << endl;
	...
	{
/***** MSVC Stellarium Build *****/
//Original == snprintf(envstr, 25, "LANGUAGE=%s", langName.c_str());
#ifdef __MSVC__
...
/***** MSVC Stellarium Build *****/
	...
	_putenv(envstr);
	/***** MSVC Stellarium Build *****/
	//Original == setlocale(LC_MESSAGES, "");
...
std::wstring Translator::iso639_1LanguageCodeToNativeName(const string& languageCode)
{
	if (iso639codes.find(languageCode)!=iso639codes.end())
		...
		return StelUtility::stringToWstring(languageCode);
}
...
std::string Translator::nativeLanguageNameCodeToIso639_1(const wstring& languageName)
{
	...
	return StelUtility::wstringToString(languageName);
}
