...
\begin{description}
  ...
  \item[ShowMySky] This model is the more realistic visual model of
    the atmosphere colors (see section~\ref{sec:skylight:Bruneton}). It
    relies on a precomputed dataset that can be chosen in the user
    interface after the \program{ShowMySky} model is enabled.
    \footnote{Due to compatibility issues, this mode is currently not
      available on the Apple Mac platforms.}
\end{description}
...
Click the ``Export KML...'' button will create KML file of selected eclipse. KML is a file format used to display geographic data in Earth browser, such as Marble, Google Earth, Google Maps. The file can be opened in applications that support KML version 2.2. Description of lines for solar eclipse are shown below (Fig.~\ref{fig:gui:AstroCalc:Eclipses:EclipseMap}).

Different colors are used to draw path of central eclipse. Red = total eclipse, blue = annular eclipse, and purple = hybrid eclipse. Limits of penumbral or partial eclipse are green.
