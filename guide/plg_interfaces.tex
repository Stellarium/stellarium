...
\includegraphics[width=.75\textwidth]{PointerCoordinates-plugin.jpg}
...
flag\_show\_constellation &bool& Add the 3-letter IAU\index{IAU} abbreviation for the constellation the mouse pointer sits in \cite{1987PASP...99..695R}.\\\midrule
flag\_show\_crossed\_lines &bool& Show crossed line under mouse cursor.\\\bottomrule
\end{longtabu}
...
your way around the web interface. The remote control automatically uses the 
same language as set in the main program. Tabs at the top allow access to 
...
curl -G -d 'id=propId=-1&actionId=-2' http://localhost:8090/api/main/status | \
     python -m json.tool 
\end{commands} 
Depending on your operating system or command shell, you may have to use double quotes in the \texttt{-\/-data} argument to \command{curl}.
