...
\begin{enumerate}
  ...
  \item Generate levels 4--10 (Star2/Star3 records) from the Gaia
        \file{.bin} files with \program{gaia2cat}.
  \item Remove duplicated stars between adjacent levels with \program{dedupcat}.
...
\begin{description}
  ...
  \item[\program{--sp-table}] spectral type table. If given (and it exists),
        ...
        catalogues. If omitted, a fresh table is built in first-encounter
        order and written to \file{<out-dir>/stars\_hip\_sp\_0v0\_6.cat}.
...
levels 0--2, and writes \file{stars\_0\_0v0\_21.cat},
\file{stars\_1\_0v0\_16.cat}, \file{stars\_2\_0v0\_17.cat} and
\file{stars\_3\_0v0\_10.cat}.
...
gaia2cat --gaia-bin <dir> --out-dir <dir> [--work-dir <dir>]
             [--workers n] [--star3-from 8] [--levels 4-10] [--dry-run]
\end{commands}
...
parallel, assigns each star to its geodesic zone and magnitude level, and
writes per-level bucket files; pass 2 sorts each bucket by (zone, V) and
writes the \file{.cat} files with the 28-byte header described in
section~\ref{sec:Catalogues:stars:sections}.

\subsection{Deduplication Between Adjacent Levels}
\label{sec:StarCatalogues:Generating:dedup}

Because the magnitude ranges of neighbouring levels may share boundary
stars (level 4 intentionally overlaps level 3, and levels 7--10 partition a
continuous flux range), run \program{dedupcat} against each adjacent pair
after generation:
...
can be mixed freely; the output keeps the second catalogue's record format.
Apply it in the chain $3 \rightarrow 4$, $4 \rightarrow 5$, \dots,
$9 \rightarrow 10$, replacing each fainter catalogue with its deduplicated
version. Levels 0--3 must \emph{not} be deduplicated: level 2 deliberately
contains all HIP stars regardless of magnitude.
...
report draws its provenance information from the
\texttt{source\_id\_provenance} column written by
\program{parse\_hip\_catalog.py} and uses the two spectral- and
object-type tables produced alongside the catalogues to decode
