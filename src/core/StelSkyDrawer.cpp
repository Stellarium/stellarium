...
void StelSkyDrawer::init()
{
	...
	const char *psfGlowFsrc =
		...
		"void main(void)\n"
		"{\n"
		"    highp float px = length(vCorner) * outPsfRadius;\n"
	...
	auto createPsfProgram = [this, psfVsrc](const char* fsrc, const char* name, PsfStarShaderVars& vars)
	{
		...
		if (!v.log().isEmpty()) { qWarning() << "StelSkyDrawer::init(): Warnings while compiling" << name << "vshader:" << v.log(); }
		QOpenGLShader f(QOpenGLShader::Fragment);
		f.compileSourceCode(StelOpenGL::globalShaderPrefix(StelOpenGL::FRAGMENT_SHADER) + fsrc);
		if (!f.log().isEmpty()) { qWarning() << "StelSkyDrawer::init(): Warnings while compiling" << name << "fshader:" << f.log(); }
