...
static QString getHipsType(const HipsSurveyP hips)
{
	...
	if (!hips->isPlanetarySurvey())
		return "dss";
	if (properties["type"].toString() == "planet" ||
	    properties["type"].toString() == "planet-normal" ||
	    properties["type"].toString() == "planet-horizon")
		return "sol";
	return "other";
}
...
void ViewDialog::updateHips()
{
	...
	for (auto &hips: hipslist)
	{
		...
		if (selectedIsPlanetary)
		{
			...
			static const std::unordered_map<QString, QString> typeNames = {
...
void ViewDialog::hipsListItemChanged(QTreeWidgetItem* item)
{
	...
	case HipsItemType::Group:
	{
		...
		{
			colors->setProperty("visible", false);
		}
		break;
	}
