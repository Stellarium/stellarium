...
Details about vector algebra related to astrometric calculations can be found in \citep{2012asas.book.....V}. For catalog with Star Data Record Type 0, in 
practice to convert \texttt{x0}, \texttt{x1}, \texttt{x2} to right accension $\alpha$ and declination $\delta$, one can use the following formulas:
...
and to convert proper motion vector (\texttt{dx0}, \texttt{dx1}, \texttt{dx2}) to proper motion in right accension $\mu_{\alpha^*}$ and declination $\mu_{\delta}$, one can use the following formulas:
