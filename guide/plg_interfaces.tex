...
\begin{enumerate}
    ...
    mode by default). If you don’t have one, click the \button{Astrometry ApiKey} hyperlink to obtain it 
    after registering or logging in. Check \checkbox{Remember me} to store the key for future use.

    \item \textbf{Upload and Solve}: Click \button{Upload} to begin uploading and solving the image. Ensure system 
    ...
    \begin{itemize}
        ...
        \item Use \button{Try Rendering this Texture} to preview the texture. If it overlaps with default 
		textures, check \checkbox{Disable default textures} to avoid visual conflicts. However, remember 
		to click \button{Stop Rendering this Texture} afterward to restore the rendering of the default nebula textures.
    \end{itemize}
    ...
    \item \textbf{Restart Stellarium}: After restarting Stellarium, switch to the \button{Custom Textures} page of the 
