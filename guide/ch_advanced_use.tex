...
in the \emph{Tools} tab of the \emph{Configuration} dialog (\guibutton[0.35]{2}{btd_config} or
\key{F2}).

\subsection{More Dee-Sky Objects}
\label{sec:ExtraData:DSOs}

\noindent\newFeature{v0.16.0}Stellarium is packaged with over 83 thousand deep-sky objects in the normal
program download (the standard edition of Stellarium DSO catalog --- see section \ref{sec:dso:catalog}), but the larger catalog with over one million objects (the extended edition of Stellarium DSO catalog contains data up to $19.0^m$) may be downloaded
from the SourceForge website\footnote{\url{https://sourceforge.net/projects/stellarium/files/Extra-data-files/DSO/3.1/}}:

\noindent\begin{tabular}{lllr}
\toprule
\emph{Version}&\emph{Filename}& \emph{MD5 hash}& \emph{Size}\\\midrule
3.1; extended edition & \file{catalog.dat} &\texttt{6a2b0deb8deccb6af98f74d9d5cd7177}& 28.2\,MB\\\bottomrule
\end{tabular}
...
for any input date~\cite{1988A&A...202..309B}. However, its use is recommended only for the year
...
Likewise for the moon, Stellarium by default uses ELP~2000-82B~\cite{1982CeMec..26...63C, 1983A&A...124...50C, ELP2000-82B}.
...
from NASA's Jet Propulsion Laboratory (JPL)~\cite{DE43x}. The data files have to be
...
The integration of this feature is still somewhat experimental.  Please compare to  JPL
...
\noindent In the \emph{Location} panel (see section~\ref{sec:gui:location})
you can receive your location from a \indexterm{GPS} device.
\newFeature{0.16} The exact way to receive GPS location depends on
...
bluetooth device, etc., but uses a connection to the \program{gpsd}
...
connect to it regardless of configuration entries.  You cannot change
the USB plug after you first have received a location fix.  If you
...
If GPS lookup fails, see the logfile for diagnostic messages.
