...
In version 0.18.3 \newFeature{v0.18.3} we added export the data to XLSX files in additional to CSV files.
...
You may also export the list of positions into XLSX or CSV file.
...
You can export the calculated ephemeris into XLSX or CSV file. 
...
You can export the calculated phenomena into XLSX or CSV file.
...
In version 0.18.2 \newFeature{v0.18.2} we added two additional columns for this tool --- \emph{solar elongation} (we used term \emph{elongation} in version 0.18.2 for this column) and \emph{lunar elongation} (we used term \emph{angular distance} in version 0.18.2 for this column). The first of those columns show angular distance from the Sun, second column --- angular distance from the Moon. Both column may be helpful for planning observation of phenomena.

\subsection{The Graphs Tab}
\label{sec:gui:AstroCalc:Graphs}

In version 0.18.3 \newFeature{v0.18.3} we added the new graph, who helpful for montly planning the observations of deep-sky objects and merged all graphs in one tab with four subtabs.

\subsubsection{The ``Altitude vs.\ Time'' Subtab}
\label{sec:gui:AstroCalc:Graphs:AltVsTime}
  
On this subtab (the first subtab in Graphs tab and it opens by default) you can compute the geometrical altitude of the currently selected object 
and date and draw it as a graph (Fig.~\ref{fig:gui:AstroCalc:Graphs:AltVsTime}).
...
An optional graphs for the Sun (with lines for civil, nautical and astronomical twilight) and the Moon also available.
...
This subtab \newFeature{v0.18.0} can show ``Monthly Elevation'' graph for the current year at selected time. This tool was introduced for planning yearly observations (Fig.~\ref{fig:gui:AstroCalc:Graphs:ME}).
...
In version 0.18.3 \newFeature{v0.18.3} this tool has been refactored --- the tool for search items from list of \emph{Matching Objects} was removed, the filter for magnitudes was moved in right ``column'' and we added new one filter here by limit the range of acceptable angular sizes of matched objects. In additional to the names we added 5 new sortable columns --- magnitude, rising time, transit time, setting time and angular size of object.
...
In version 0.18.2 \newFeature{v0.18.2} we enhanced this tool via add new tab (\emph{Graphs}), where you may see the change in the linear and angular distances between selected celestial bodies in range 600 days (\emph{today} is in the center of graphs) as graphs (Fig.~\ref{fig:gui:AstroCalc:PC:Graphs}).
...
The About Tab (Fig.~\ref{fig:gui:help:about}) shows version and licensing information, and a list of people who helped to produce the program. Since version 0.18.3 \newFeature{0.18.3} this tab has tool for check updates of planetarium.
...
This tool available through the Help Tab of Help window (see section~\ref{sec:gui:help:help}) and the Tools Tab of Configuration window (see section~\ref{sec:gui:configuration:tools}).
