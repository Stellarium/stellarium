...
const\_boundary\_color                  & \ccbox{0.8,0.3,0.3} & constellation boundaries.\\
const\_hulls\_color                     & \ccbox{0.6,0.2,0.2} & constellation areas (convex hulls).\\
asterism\_lines\_color                  & \ccbox{0.4,0.4,0.8} & asterism lines. \\
asterism\_names\_color                  & \ccbox{0.4,0.4,0.8} & asterism names. \\
rayhelper\_lines\_color                 & \ccbox{1.0,1.0,0.0} & the ray helper lines. \\
skyculture\_zodiac\_color               & \ccbox{1.0,1.0,0.0} & Zodiac band.\\
skyculture\_lunarsystem\_color          & \ccbox{0.0,1.0,0.5} & Band/boundaries of Lunar stations\,/\,mansions.\\\midrule
%% DSO
...
% ephemeris markers and planet trails
ephemeris\_generic\_marker\_color       & \ccbox{1.0,1.0,0.0} & ephemeris marker and trail color.\\
ephemeris\_secondary\_marker\_color     & \ccbox{0.7,0.7,1.0} & ephemeris marker and trail color.\\
ephemeris\_selected\_marker\_color      & \ccbox{1.0,0.7,0.0} & ephemeris marker and trail color.\\
ephemeris\_mercury\_marker\_color       & \ccbox{1.0,1.0,0.0} & ephemeris marker and trail color.\\
ephemeris\_venus\_marker\_color         & \ccbox{1.0,1.0,1.0} & ephemeris marker and trail color.\\
ephemeris\_mars\_marker\_color          & \ccbox{1.0,0.0,0.0} & ephemeris marker and trail color.\\
ephemeris\_jupiter\_marker\_color       & \ccbox{0.3,1.0,1.0} & ephemeris marker and trail color.\\
ephemeris\_saturn\_marker\_color        & \ccbox{0.0,1.0,0.0} & ephemeris marker and trail color.\\
ephemeris\_uranus\_marker\_color        & \ccbox{0.2,0.5,0.3} & ephemeris marker and trail color.\\
ephemeris\_neptune\_marker\_color       & \ccbox{0.2,0.3,0.5} & ephemeris marker and trail color.\\
% other colors
object\_trails\_color                   & \ccbox{1.0,0.7,0.0} & generic minor planet/comet trails. \\\midrule
...
Similar flags for selecting narration bits are named \texttt{flag\_narration\_name} etc.
...
info\_shadow\_radius                   & int   & 6     & [0\ldots64] blur radius of this shadow\\
info\_shadow\_opacity                  & int   & 127   & [0\ldots255] opacity of this shadow\\
info\_panel\_opacity                   & int   & 0     & [-100\ldots100] text background darkness\\
...
toolbar\_corner                 & int    & 0     & Corner where both bars are docked: 0=bottom-left, 1=bottom-right, 2=top-left, 3=top-right.\\%\midrule
...
flag\_use\_azimuth\_from\_south & bool   & false & If \emph{true}, calculate azimuth from south towards west 
                                                   (as in some astronomical literature)\\
flag\_use\_negative\_hour\_angles & bool & false & If \emph{true}, use hour angles -12h\ldots12h\\ 
flag\_use\_polar\_distance      & bool   & false & Use distance from North Celestial Pole, not declination from equator \\\midrule
...
\subsection{\big[speech\big]}
\label{sec:config.ini:speech}

These are the settings for the Text-to-Speech engine. 
Further settings are in section \texttt{custom\_selected\_info} (\ref{sec:config.ini:custom_selected_info}).

\noindent
\begin{tabularx}{\textwidth}{l|l|X}\toprule
\emph{ID} & \emph{Type} &  \emph{Description}\\\midrule
engine    & string &  speech engine. OS dependent. \\
pitch     & float  &  [-1.0\ldots1.0] modify voice a bit \\
rate      & float  &  [-1.0\ldots1.0] modify speed \\
voice     & string &  Depends on engine and system language. \\
volume    & float  &  [0.0\ldots1.0]\\
\bottomrule
\end{tabularx}
...
init\_bortle\_scale      & int         & initial value of light pollution on the Bortle scale. Typical value: \emph{3}.\\%\midrule
flag\_compact\_star\_storage & bool    & When \emph{true}, star catalogues at level~9 and above use a compact on-disk layout that omits the per-zone count metadata. Only effective with memory-mapped catalog loading; reduces memory pressure at high levels. Default: \emph{false}.\\%\midrule
missing\_bv\_value & float & Default B-V colour index assigned to stars whose catalog entry has a missing or sentinel B-V value (e.g.\ 32.767 in Star2). Used in the star colour computation. Default: \emph{1.0}.\\\bottomrule
\end{tabularx}
...
flag\_dynamic\_moon\_scale    & bool  & If \emph{true}, Moon scale is computed dynamically from FOV rather than
                               using the fixed \texttt{moon\_scale} multiplier. Requires \texttt{flag\_moon\_scaled} to also be \emph{true}. Default: \emph{false}\\%\midrule
moon\_scale\_min\_fov         & float & FOV in degrees at which dynamic Moon scaling equals $1\times$
                               (natural size). Below this FOV the Moon is unscaled. Default: \emph{10.0}\\%\midrule
moon\_scale\_max\_fov         & float & FOV in degrees at which dynamic Moon scaling reaches the fixed
                               \texttt{moon\_scale} value. Above this FOV the scale is clamped. Default: \emph{90.0}\\
