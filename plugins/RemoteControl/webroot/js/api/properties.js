define(["jquery", "./remotecontrol", "./updatequeue"], function($, rc, UpdateQueue) {
    ...
    // ============================================================
    // FIX: Remove evt.preventDefault() to allow remotecontrol.js
    // to update lastPropId properly
    // ============================================================
    $(rc).on("stelPropertiesChanged", function(evt, changes) {
        // If properties are not initialized yet, just return without preventing the event
        if (!propsInitialized) {
            console.log("Properties not initialized yet, changes will be processed later");
            return;
        }

        // Process all changes, adding unknown properties to cache instead of blocking
        for (var name in changes) {
            if (name in propData) {
                ...
                // Unknown property - add to cache instead of error
                console.log("Unknown property " + name + " changed - adding to cache");
                propData[name] = {
                    value: changes[name],
                    typeEnum: typeof changes[name] === 'boolean' ? 1 : 6,
                    typeString: typeof changes[name] === 'boolean' ? 'bool' : 'double',
                    isWritable: true,
                    canNotify: true
                };
                firePropertyChanged(name);
        ...
        // Do NOT prevent default - allow remotecontrol.js to update lastPropId
    });
