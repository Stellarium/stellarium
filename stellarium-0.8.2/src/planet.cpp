...
LinearFader Planet::flagShow = true;
bool Planet::isBlack = false;
...
Vec3d Planet::get_earth_equ_pos(const Navigator * nav) const
{
  ...
  Vec3d result;
  result = nav->helio_to_earth_pos_equ(v);		// this is earth equatorial but centered;
  /*kornyakov: fixme check after? this is for flying
  result = nav->helio_to_local(v);
  result = nav->local_to_eye(result);
  result -= Vec3d(.0, .0, nav->current_translation);
  result = nav->eye_to_local(result);
  result = nav->local_to_earth_equ(result);*/
...
double Planet::getSiderealTime(double jd) const 
{
	...
	{	
		static bool rollTemp = false;
		static double dayTime = 0.0;
		static double restoreDayTimeCoef = 1.1;
		if (!stopDayMotion || rollTemp)
		{
			if (startDayMotionTime >= 0.0)
			{
				restoreDayTimeCoef = (SDL_GetTicks() - startDayMotionTime) / restoreDayTimeDuration;
			}
			if (restoreDayTimeCoef >= 1.0)
			{
				if (!stopDayMotion)
				{
					dayTime = jd - static_cast<int>(jd);
				}
				return get_apparent_sidereal_time(jd);
			}
			else
			{
				return get_apparent_sidereal_time(static_cast<int>(jd) + dayTime +
					(jd - static_cast<int>(jd) - dayTime) * restoreDayTimeCoef);
			}
		}
		else
		{
			double lowJD = static_cast<int>(jd) + dayTime;
			double highJD = lowJD + 1;
			rollTemp = true;
			double lowSiderealTime = getSiderealTime(lowJD);
			double highSiderealTime = getSiderealTime(highJD);
			if (lowSiderealTime > highSiderealTime)
			{
				highSiderealTime += 360.0;
			}
			double siderealTime = lowSiderealTime + 
				(highSiderealTime - lowSiderealTime) * (jd - lowJD);
			rollTemp = false;
			return siderealTime;
		}
	}
...
float Planet::get_on_screen_size(const Projector* prj, const Navigator * nav)
{
  ...
  float lenght;
  Vec3d dist_vec = get_heliocentric_ecliptic_pos();
  dist_vec = nav->get_helio_to_eye_mat() * dist_vec;
  lenght = dist_vec.length();

  // kornyakov: the following line is original method of distance calculation
  // lenght = get_earth_equ_pos(nav).length();
  return atanf(rad*sphere_scale*2.f/lenght) * 180./M_PI/prj->get_fov() * prj->getViewportHeight();
}
...
double Planet::draw(Projector* prj, const Navigator * nav, const ToneReproductor* eye, int flag_point, bool stencil)
{
  ...
  // kornyakov
  bool b1 = prj->project_custom(Vec3f(0,0,0), screenPos, mat);
  bool b2 = screenPos[1]>viewport_bottom - screen_sz;
  bool b3 = screenPos[1]<viewport_bottom + prj->getViewportHeight()+screen_sz;
  bool b4 = screenPos[0]>viewport_left - screen_sz;
  bool b5 = screenPos[0]<viewport_left + prj->getViewportWidth() + screen_sz;
	if ( b1 && b2 && b3 && b4 && b5 )
	{
      ...
      //kornyakov: we have to subtract if we flying
      if (nav->nameOfSelected == this->getEnglishName())
        dist -= nav->currentTranslation;
			double n, f;
...
void Planet::draw_halo(const Navigator* nav, const Projector* prj, const ToneReproductor* eye)
{
	...
	// And we compensate the difference of brightness with cmag
	if (rmag<1.2f)
	{
		if (rmag<0.3f) return;
...
void Ring::draw(const Projector* prj,const Mat4d& mat,double screen_sz)
{
  ...
  float intensity = prj->GetIntensity();
	glColor3f(intensity*1.0f, intensity*0.88f, intensity*0.82f); // For saturn only..
	glEnable(GL_TEXTURE_2D);
	glEnable(GL_CULL_FACE);
	glEnable(GL_BLEND);
