...
A few hints: 
On Ubuntu, USB hotplug devices are handled by the \program{udev} daemon which detects newly plugged-in 
devices and creates device files in the \file{/dev} directory. Unfortunately, most GPS devices use the Prolific 2330 
...
For Windows, there is a project\footnote{\url{https://code.google.com/archive/p/gpsd-4-win/}} which provides a \program{gpsd} 
...
WRONG: This gpsd-4-win is outdated. On Windows, we must parse NMEA. gpsd authors do not support Windows, period.
