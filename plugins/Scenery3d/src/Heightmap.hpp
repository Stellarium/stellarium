...
private:
        ...
        struct GridSpace {
                FaceVector faces;
                float getHeight(OBJ& obj, const float x, const float y) const;

                //static float face_height_at(const OBJ& obj, const OBJ::Face* face, float x, float y);
                static float face_height_at(OBJ& obj, unsigned int* pTriangle, const float x, const float y);
        };
        ...
        bool face_in_area(OBJ& obj, const unsigned int* pTriangle, const float xmin, const float ymin, const float xmax, const float ymax) const;
