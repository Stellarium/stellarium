...
\noindent Some hightlights of the stars field:
\begin{description}
...
\item[Search Options] group allows for changes in the search result behaviour.
        \begin{description}
        \item[Use autofill only from the beginning of words]- searches for object names
        that begins with the same letters as your input. Example provided in section~\ref{sec:gui:SearchWindow:Object}.
        \item[Lock position when coordinates are used] %% TODO: EXPLAIN?
        \item[Show FOV center marker when position is search]%% TODO: EXPLAIN HOW?
        \end{description}

        \begin{description}
        \item[Recent Searches]- allows modification to your recent search data\newFeature{v0.20.3}
        Changes will automatically update the search results displayed in the Object tab.
        Example provided in section~\ref{sec:gui:SearchWindow:Object}.
                \begin{description}
                \item[Max recent searches to display] amount of\newFeature{v0.20.3}
                \emph{recent searches} that can appear in the search result.
                \item[\key{Clear Search History}] button
                deletes your recent search history\newFeature{v0.20.3}. If there are no recent searches saved,
                the button will deactivate (greyed out).
                \end{description}
        \end{description}
\end{description}
