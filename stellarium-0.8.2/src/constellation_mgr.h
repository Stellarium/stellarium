...
public:
	...
	void setFlagStickyContellationSelection(bool b) 
	{
		stickyConstellationSelected = b;
		if (!b)	
		{
			vector < bool> ::iterator iterStickySelection;
		    for (iterStickySelection = stickySelection.begin(); 
				iterStickySelection != stickySelection.end(); 
				++iterStickySelection)
		    {
	...
	bool getFlagStickyContellationSelection(void) 
	{
		return stickyConstellationSelected;
	};   	
...
private:
    ...
    vector<bool> stickySelection;
    s_font *asterFont;
    HipStarMgr *hipStarMgr;
	Constellation* selected;
	bool isolateSelected;
	bool stickyConstellationSelected;
