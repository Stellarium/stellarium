...
struct Star2Brief {
	...
	int16_t vmag, bv;
	uint16_t plx, plx_err;

	static Star2Brief decode(const uint8_t buf[32]) {
		Star2Brief s;
		std::memcpy(&s.gaia_id, buf, 8);
		std::memcpy(&s.x0,  buf+8,  4);
		std::memcpy(&s.x1,  buf+12, 4);
		std::memcpy(&s.dx0, buf+16, 4);
		std::memcpy(&s.dx1, buf+20, 4);
		std::memcpy(&s.bv,  buf+24, 2);
		std::memcpy(&s.vmag,buf+26, 2);
		std::memcpy(&s.plx, buf+28, 2);
		std::memcpy(&s.plx_err, buf+30, 2);
		return s;
	}

	void print(const char* label, int zone) const {
		printf("  %s Gaia %lld  zone=%u  V=%.3f  B-V=%.3f  RA=%.6f  DEC=%.6f  pm=(%.2f,%.2f) mas/yr  plx=%.2f mas\n",
			label, (long long)gaia_id, zone,
			vmag/1000.0, bv/1000.0,
			x0/3600000.0, x1/3600000.0 - 90.0,
			dx0/1000.0, dx1/1000.0,
			plx/100.0);
	}
};

static void compare(const std::string& a_path, const std::string& b_path) {
	// Open both files, read headers
	auto open_cat = [](const std::string& path, FILE*& f, int& level, int& nz,
	                   std::vector<uint32_t>& counts, int64_t& star_base, uint64_t& total) -> bool {
	...
	int64_t base_a, base_b;
	uint64_t total_a, total_b;

	if (!open_cat(a_path, fa, la, nza, ca, base_a, total_a)) return;
	if (!open_cat(b_path, fb, lb, nzb, cb, base_b, total_b)) { std::fclose(fa); return; }

	std::cout << "A: level=" << la << " stars=" << total_a << "\n";
	std::cout << "B: level=" << lb << " stars=" << total_b << "\n";

	int nz = std::max(nza, nzb);
	int64_t off_a = base_a, off_b = base_b;
	uint64_t only_a = 0, only_b = 0;
	int printed = 0;
	auto print_once = [&]() -> bool { return ++printed <= 200; };

	uint8_t buf[32];
	std::unordered_map<uint64_t, Star2Brief> map_a;

	for (int z = 0; z < nz; ++z) {
		uint32_t cnt_a = (z < nza) ? ca[z] : 0;
		uint32_t cnt_b = (z < nzb) ? cb[z] : 0;
		if (cnt_a == 0 && cnt_b == 0) continue;

		// Load zone A into map
		map_a.clear();
		if (cnt_a > 0) {
			fseeko(fa, off_a, SEEK_SET);
			for (uint32_t i = 0; i < cnt_a; ++i) {
				std::fread(buf, 32, 1, fa);
				auto s = Star2Brief::decode(buf);
				map_a[s.gaia_id] = s;
			}
		}

		// Read zone B and compare
		if (cnt_b > 0) {
			fseeko(fb, off_b, SEEK_SET);
			for (uint32_t i = 0; i < cnt_b; ++i) {
				std::fread(buf, 32, 1, fb);
				auto s = Star2Brief::decode(buf);
			auto it = map_a.find(s.gaia_id);
			if (it == map_a.end()) {
				only_b++;
				if (print_once()) s.print("only in B:", z);
			} else {
				map_a.erase(it);
			}
			}
		...
		// Remaining in map_a are "only in A"
		for (auto& [gid, s] : map_a) {
			only_a++;
			if (print_once()) s.print("only in A:", z);
		}
...
int main(int argc, char** argv) {
	...
	if (a.empty() || b.empty()) {
		std::cerr << "Usage: cmpcat <fileA.cat> <fileB.cat>\n";
		return 1;
	}
	compare(a, b);
	return 0;
