...
\begin{enumerate}
  ...
  \item Generate levels 4--10 (Star2/Star3 records) from the Gaia
        ...
        so that the already-catalogued HIP stars are not duplicated in
        levels~4--6 (levels~7--10 keep them, matching the official
        catalogues).
  \item Remove the level~3/4 boundary overlap: level 4 intentionally
        starts 0.25 mag brighter than level 3, and
        \program{gaia2cat}'s \program{--exclude-lower} drops every star
        that the level~0--3 catalogues already contain, so no star is
        lost and none is duplicated. No separate deduplication step is
        needed in the standard pipeline.
...
\begin{commands}
extract_columns.exe <csv_dir> <bin_dir>
        [workers]
\end{commands}
...
object-type table \file{otype.dat}; a copy of
\linebreak
\file{stars/hip\_gaia3/object\_types\_v0\_1.cat} works as-is.
\linebreak
The spectral type table \file{stars\_hip\_sp\_0v0\_6.cat} is optional input
...
\begin{description}
  ...
  \item[\program{--stars-config}] path to the Stellarium
        ...
        \texttt{\%APPDATA\%\textbackslash Stellarium\textbackslash stars\textbackslash hip\_gaia3\textbackslash starsConfig.json}
        on Windows and \texttt{\$HOME/.stellarium/...} elsewhere.
\end{description}
...
\begin{description}
  ...
  \item[\program{--exclude-lower}] one or more level~0--3 \file{.cat} files
        ...
        no separate deduplication step is required. Levels~7--10 keep HIP
        stars, exactly like the official level~7+ catalogues (whose ADQL
        queries do not apply this filter). The level~0--3 files are parsed
        on the fly, so no intermediate id list is needed.
