...
bool StelCore::putSelectedNearHorizon(const bool setting, const double offsetMin, const bool onLandscape, double &jde, const bool setThisTime)
{
	StelObjectMgr objMgr=GETSTELMODULE(StelObjectMgr);
	Q_ASSERT(objMgr);
	StelObjectP obj=objMgr.getSelectedObject().at(0);
	Q_ASSERT(obj);

	qdebug() << "We have an object:" << obj->getEnglishName();

	// Meeus 1998, p101 rise-transit-set
	Vec3d objPos=obj->getEquinoxEquatorialPos(this);
	// objPos.normalize(); // not needed if we ask via latitude()
	StelLocation* location=getCurrentLocation();
	double cosHo=(-std::tan(location->latitude * M_PI/180.0) * std::tan(objPos.latitude()));
	if ( abs(cosHo) > 1.0 )
		return false;

	// currently this mixes JD(UT) and JDE(ET)!
	double jd0=std::floor(jde); jd0+= ( (jde-jd0<0.5) ? 1.5 : 0.5); // JD of 0hUT.
	double Ho=std::acos(cosHo) * 180.0/M_PI; // hour angle of setting object, degrees.

	SolarSystem* ssystem=GETSTELMODULE(SolarSystem);
	Q_ASSERT(ssystem);
	PlanetP planet=ssystem->searchByEnglishName(location->planetName);
	Q_ASSERT(planet);
	double Theta0= planet->getSiderealTime(jd0); // sid. time at Greenwich, degrees, 0hUT
	double m0= ( objPos.longitude()*180.0/M_PI + location->longitude - Theta0 ) / 360.0;
	double m = (setting ? m0 + Ho/360.0 : m0 - Ho/360.0);

	double theta0=Theta0 + 360.985647*m; // local sidereal time of event.
	double H=theta0 - location->longitude - objPos.longitude()*180.0/M_PI;
	//This is now an approximate hour angle, should be close to what we need.

	double curTheta=planet->getSiderealTime(JDay); // current sid. time.
	double thetaDiff=theta0-curTheta; // degrees
	JDay += thetaDiff/360.0;
	// Now JDay is approximately correct so that the object should be close to the horizon.
	// We now fine-tune, and also get details from Landscape.
	LandscapeMgr* lmgr=GETSTELMODULE(LandscapeMgr);
	Q_ASSERT(lmgr);

	objPos=obj->getEquinoxEquatorialPos(this); // update position.
	Vec3d azaltPos=equinoxEquToAltAz(objPos, RefractionAuto);

	qDebug() << "Object at az" << azaltPos.longitude() << "alt" << azaltPos.latitude();
	float opacity=lmgr->getLandscapeOpacity(azaltPos);
	int direction=(opacity > 0.05 ? -1 : 1); //positive: progressing in time.
	if (!setting) direction*=-1;
	qDebug() << "Landscape opacity " << opacity << ", direction " << direction << "for " << (setting? "setting" : "rising") << "event";
	float oldOpacity=opacity;

	while ( fabs(opacity-oldOpacity) < 0.5f ) // has not changed significantly?
	{
		oldOpacity=opacity;

		JDay += direction*JD_MINUTE;
		objPos=obj->getEquinoxEquatorialPos(this); // update position.
		Vec3d azaltPos=equinoxEquToAltAz(objPos, RefractionAuto);

		qDebug() << "Object at az" << azaltPos.longitude() << "alt" << azaltPos.latitude();

		opacity=lmgr->getLandscapeOpacity(azaltPos);
	}
	// now we have JDay at the minute when opacity has changed dramatically, usually the horizon line.
	JDay +=offsetMin*JD_MINUTE;
