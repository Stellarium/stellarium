...
This name is used in the selection list in the skyculture GUI (see \ref{sec:gui:view:starlore}). 
...
The skyculture description should include Level 2 sections \texttt{Authors} and \texttt{Acknowledgements} where applicable. 
...
\item[\jtag{fallback\_to\_international\_names}] TBD
\item[\jtag{thumbnail}] TBD
\item[\jtag{thumbnail\_bscale}] TBD
\item[\jtag{highlight}] TBD
\item[\jtag{common\_names}] includes star names, see \ref{SC:common_names}
...
The lines in the array ``edges'' were taken from the given reference. The first and second component are ignored. 
...
\item[\jtag{id}] A unique figure identifier, consisting of \texttt{CON} (for constellations), skyculture id, and a short label still unique within the skyculture.
				 ...
				 the short label is displayed when setting Constellation name style to ``short'' (s.~\ref{sec:gui:view:starlore}). If you want to prevent
...
\item[\jtag{lines}] an array of arrays, each describing a polygon with star numbers. The star numbers are usually the Hipparcos (HIP) numbers. 
					...
					\emph{Single-star constellations} or particular highlights can be described with just one entry in a segment,\newFeature{25.1} 
					the respective star will be encircled with a radius of \jtag{single\_star\_radius}.
					
					To describe \indexterm{dark constellations} formed from dust clouds in the Milky Way or other non-stellar items, 
...
\item[\jtag{single\_star\_radius}] (float, default 1.0/TBD) radius of a circle (degrees) marking single-star segments in \jtag{lines}.		\newFeature{25.1} 
...
\item[\jtag{common\_name}] Dictionary of names. Required entries: \jtag{english}. Optional: \jtag{native}
	...
	These names are used in the Constellation name style settings (s.~\ref{sec:gui:view:starlore}): In \menu{Native} mode, the native element is shown. 
...
\item[\jtag{lines}] an array of arrays, each describing a polygon with integer star numbers. The star numbers are usually the Hipparcos (HIP) numbers. 
					...
					Another variant can use coordinate lists and is technically an array of arrays of 2-part arrays of float numbers, 
					each pair being right ascension $\alpha$ (decimal hours) and declination $\delta$ (decimal degrees) in equinox J2000.0. 
					%Single-star asterisms can be described with just one line vertex (star or coordinate pair)\newFeature{25.2} 
					%the respective star/coordinate will be encircled with a radius of \jtag{single\_star\_radius}. 
%\item[\jtag{single\_star\_radius}] (float, default 1.0/TBD) radius of a circle (degrees) marking single-vertex segments in \jtag{lines}.		\newFeature{25.2} 			
\end{description}
