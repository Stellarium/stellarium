...
This table \newFeature{v0.21.2}provides comparison data between JPL
...
coordinates, including corrections for nutation, light time,
...
computed by Stellarium from DE440/DE441 files, with corrections for
...
specialized to the DE ephemeris is used\footnote{For experts: The value
  ...
  is replaced by $\dot{n}=-25.8$.}  Among the causes for differences we
...
%\item Stellarium provides nutation only for 500 years around J2000.0. It is undocumented for how long the underlying model is valid.
\item JPL Horizons uses different models than Stellarium for precession and nutation\footnote{\url{https://ssd.jpl.nasa.gov/?horizons_doc#specific_quantities}}
\end{itemize}
