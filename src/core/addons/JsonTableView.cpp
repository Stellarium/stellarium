...
void JsonTableView::slotDataUpdated(const QString& category) {
...
void JsonTableView::scrollValueChanged(int)
{
...
void JsonTableView::setModel(QAbstractItemModel* model)
{
...
void JsonTableView::mouseDoubleClickEvent(QMouseEvent *e)
{
...
void JsonTableView::mousePressEvent(QMouseEvent *e)
{
	QModelIndex index = indexAt(e->pos());
...
void JsonTableView::clearSelection()
{
...
void JsonTableView::selectionChanged(const QItemSelection& selected, const QItemSelection& deselected)
{
...
void JsonTableView::setAllChecked(bool checked)
{
	for (int row=0; row < model()->rowCount(); row=row+2)
	{
