...
Q_EXPORT_PLUGIN2(FOV, FOVStelPluginInterface)
...
void FOV::init()
{
	...
	for (int i=0; i<10; i++)
	{
		...
		QString text = QString("Set FOV to %1%2").arg(getQuickFOV(i)).arg(QChar(0x00B0));
		QAction* action = shMgr->addGuiAction(name, true, text, shortcut, "", section, false, false, true);
		connect(action, SIGNAL(triggered()), this, SLOT(setFOV()));
	}
}
