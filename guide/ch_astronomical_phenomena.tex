...
right only a line-art version (or \emph{stick figure}) is shown. 
...
Gru & Grus                 & Gruis                 & UMa & Ursa Maior           & Ursae Maioris         \\
...
\begin{table}[p]
  ...
  \begin{tabular}{>{$}c<{$}l>{$}c<{$}l>{$}c<{$}l>{$}c<{$}l>{$}c<{$}l>{$}c<{$}l}
    \toprule
    \alpha    & alpha   & \epsilon  & epsilon & \iota     & iota    & \nu       & nu       & \rho     & rho       & \varphi  & phi    \\ 
    \beta     & beta    & \zeta     & zeta    & \kappa    & kappa   & \xi       & xi       & \sigma   & sigma     & \chi     & chi    \\ 
    \gamma    & gamma   & \eta      & eta     & \lambda   & lambda  & \omicron  & omicron  & \tau     & tau       & \psi     & psi    \\ 
    \delta    & delta   & \theta    & theta   & \mu       & mu      & \pi       & pi       & \upsilon & upsilon   & \omega   & omega  \\ 
     \bottomrule
 \end{tabular}
...
supported: Stellarium's default 'Modern' skyculture is an amalgamate of European scientific traditions 
that are rooted in Ptolemy's Almagest, with constellation figures mostly from ancient Greek mythology. 
In the upcoming era of Islam, the Almagest was translated to Arab, including
star names that often just described the location of a star within the constellation. 
Around the year 1000 the Almagest was re-introduced to Western Europe, but some star names were misspelled 
and transferred without re-tranalation, introducing a large number of star names of Arab origin now in use for many centuries.

Aside from historical and mythological value still valued by storytellers around the campfires, 
to the modern astronomer
...
an amateur observer is \emph{astrognosis} (learning the constellations) --- the
...
As scientific astronomy evolved in the Renaissance and later, star atlases were produced, 
each introducing more constellations to fill gaps left between the classic 48 Ptolemaic constellations, 
with artistic figures often overwhelming the actual star map.
The telescope age and needs to assign each fixed celestial object coordinates and a 'host constellation' 
finally had to bring an end to the uncontrolled growth of officially recognized constellations. 
In 1930, the International Astronomical Union \index{IAU} has adopted 88 
constellations as a common system for segmenting the sky (Table~\ref{tab:IAUConstellations}). 
Some formalisation has been adopted, each constellation having a \emph{proper name} in Latin, 
and a three letter abbreviation of that name.  
For example, Ursa Major has the abbreviation UMa. Also, the IAU
constellations have clearly defined \emph{boundaries}\citep{Delporte:1930}, which you can draw in
Stellarium when you press the \key{B} key\footnote{These boundaries or
  borders have been defined using coordinates from 1875. Due to the effect
  of \emph{precession}, these borders are no longer parallel to
  today's coordinate grid.}. The IAU constellation a selected object is placed in 
is also available in the object information data \citep{1987PASP...99..695R}, 
regardless of the currently active sky culture.  

On the other hand, the shapes of mythological figures, and also stick figures, have not been canonized, so 
you will likely find deviations between Stellarium's default skyculture and your favourite printed atlas. 
In this sense, do also not assume Stellarium's default stick figures to be in any way authoritative. 
As any other, they are an artistic interpretation that should help memorizing and discerning the constellations.

\subsubsection{Asterisms}
\label{sec:Phenomena:Asterisms}

As nonscientific observing of the sky became a pastime for non-scientists (amateur astronomers) in the late 19th century, 
some easy to recognize star groups were described and labeled in sky observing guides. 
These are internationally not standardized and may show even regionally differing names for (almost) the same figure. 
For example, the seven bright stars of Ursa Major are widely recognised, known variously as ``the plough'', the
``pan-handle'', ``greater wagon'' or the ``big dipper''. 

After the canonisation of the IAU constellations, these figures became commonly known as \emph{asterisms}%
\footnote{Note that in other scientific contexts, `asterism' may also be the general designation of a star figure (often of other cultures). 
However, in Stellarium an `asterism' can be understood as `informal second-class figure' that may be used for culture-dependent purposes.}. 
In Stellarium, you can display them as stick figures with name labels in colors different from the ``main'' constellations. 

\subsubsection{Non-European Constellations}
\label{sec:Phenomena:OtherSkycultures}

Many other sky cultures are available in Stellarium which show constellations and other elements of ethnoastronomy 
of other traditions, contributed by users and scientists from all over the world, and showing the diversity 
and ubiquity of human interest in the sky all around the globe.
 
We welcome well-made contributions, but as time evolves these skyculture contributions may be further extended by the original authors, 
other contributors (if allowed by respective licenses), or removed if found problematic. See chapter~\ref{ch:SkyCultures} for full details.
...
stars, and a `Working Group on Star Names' (WGSN) has been established. 
Apart from canonizing one spelling for the well-known stars and avoiding using the same name for multiple stars, 
recently WGSN has started to assign star names from non-European cultures to hitherto unnamed 
(usually dimmer, but still naked-eye) stars in order to diversify star names. 
(Constellation names and IAU borders are not affected.) 
Stellarium closely follows this development in the default `Modern' and `IAU' Skycultures.

Some companies offer a paid name service for commemoration or
...
The Moon %is a large reflective body which 
reflects not only sunlight
...
\indexterm{ashen glow}, is brightest when the Moon is most strongly
illuminated by the Earth, i.e., when the Moon as seen from Earth shows
only a thin crescent. In these days around the New Moon, the
...
\begin{figure}[tb]
  \centering
  \includegraphics[width=0.9\linewidth]{the_planets.jpg}
  \caption{The Planets}
  \label{fig:planets}
\end{figure}
...
Appendix~\ref{sec:ssystem.ini:minor} for more details. For long-term valid orbital data,
see Appendix~\ref{sec:ssystem.ini:MultiEpoch}.
...
conditions as Solar eclipses but appear to occur more often because the
Earth's shadow is so much larger than the Moon's, and a Lunar eclipse is
observable from more than half of the Earth's surface, while a Solar eclipse
...
First  Penumbral & P1 - Begin of (penumbral) eclipse. Unobservable. \\
                 & Moon first touches Earth's geometrical shadow cone from the outside. \\  
Second Penumbral & P2 - Begin of total immersion in penumbra\\
                 & Moon shows a notable darkening \\  
First  Umbral    & U1 - Begin of partial eclipse\\
                 & Moon first touches the central dark shadow \\  
Second Umbral    & U2 - Begin of totality\\
                 & Moon completely immersed in Earth's umbra (dark shadow) \\  
Maximum          & G - Mid-eclipse, central time\\
Third  Umbral    & U3 - End of totality\\
                 & A bright Lunar edge re-appears from complete occultation \\  
Fourth Umbral    & U4 - End of partial eclipse\\
                 & Moon leaves umbra completely \\  
Third  Penumbral & P3 - End of total penumbral phase\\
                 & Lunar edge leaves penumbra. The opposite side is still darkened\\  
Fourth Penumbral & P4 - End of (penumbral) eclipse. Unobservable. \\
