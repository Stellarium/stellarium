...
public:
  ...
  void setFlagBlackPlanets(bool b) {Planet::isBlack = b;}
  bool getFlagBlackPlanets(void) const {return Planet::isBlack;}
	...
	void setFlagStopDayMotion(bool b) {earth->setFlagStopDayMotion(b);};
	bool getFlagStopDayMotion(void) {return earth->getFlagStopDayMotion();};
  ...
  //kornyakov: for flying
  double getSelectedRadius(void) 
  {
    vector<Planet*>::iterator iter;
    iter = system_planets.begin();
    while ((*iter)->getEnglishName() != selected.getEnglishName()) iter++;
    return (*iter)->getRadius();
  }
