...
bottom or by pressing \key{\ctrl+W}. A right-click on that button \newFeature{0.21.2} (or \key{\ctrl+\shift+W})
opens the settings dialog. Once loaded and
displaying, you can walk around pressing \key{\ctrl} on PC or \key{Option} on Mac plus cursor keys. Change eye
height with \key{\ctrl+Page\,\arrowkeyup\,}/\key{\ctrl + Page\,\arrowkeydown\,} keys on PC or \key{Option+Page\,\arrowkeyup\,}/\key{Option + Page\,\arrowkeydown\,} keys on Mac. Adding \key{\shift} key increases speed by 10,
adding \key{\Alt} on PC or \key{Command} on Mac multiplies by 5 (pressing both keys multiplies by 50!). If you release \key{\ctrl}/\key{Option} before
the cursor key, animation will continue. (Press \key{\ctrl}/\key{Option}+any cursor key to stop
moving.)
...
\file{TX\_<Temple>\_ground}. You can delete the folder \file{TX\_<Temple>\_ground}, 
...
and altitude. The section is required if \verb:coord/convergence_angle={from_grid|from_utm}:, else
location is inherited from landscape.

\noindent%
\begin{tabularx}{\textwidth}{lX} 
\multicolumn{2}{l}{\texttt{planet = Earth}}\\
\texttt{latitude = +48d31'30.4"}      & Required if \texttt{coord/convergence\_angle=from\_\{grid|utm\}}\\
...
and, on a spherical globe, amounts to:
\begin{equation}
...
\begin{configfile}
convergence_angle=from_grid|from_utm|<double> 
grid_meridian=<double>|+<int>d<int>'<float>"      
\end{configfile}

\noindent \texttt{grid\_meridian} is the central meridian $\lambda_0$ of the grid
zone, e.g.\ for Gauss-Kr\"uger, and is only required to compute
convergence angle if \texttt{convergence\_angle=from\_grid}. 
If your model coordinates is based on UTM, you can use 
\texttt{coord/convergence\_angle=from\_utm} for a more accurate computation.
...
and some carefully selected \texttt{start\_az\_alt\_fov} in case you want to highlight 
a certain view corridor (temple axis, \ldots).

In some setups like digital full-dome planetaria \newFeature{23.2}
it is advisable to disable the use of the \texttt{start\_az\_alt\_fov}. 
You can do that in the Scenery3D configuration dialog or by the entry \texttt{ignore\_start\_az\_alt\_fov} in \file{config.ini}.
...
Stellarium \newFeature{0.16.1} working as ``time machine'' can display the sky for any calendar date in its range of valid dates. 
