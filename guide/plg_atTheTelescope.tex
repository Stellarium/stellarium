...
(section~\ref{sec:plugins:StellariumScope}), \program{RTS2} (section~\ref{sec:plugins:TelescopeControl:RTS2}) 
or \program{INDI} (section~\ref{sec:plugins:TelescopeControl:INDI}). This can also help DIY hardware 
...
\item[Sensor view] is group of options allows to change behaviour of the plugin for photographic observations mode.
 \begin{description}
   ...
   \item[Show sensor crop overlay] option\newFeature{v0.19.0} toggle drawing the crop diagram within frame of CCD.
 \end{description}
\item[Telrad view] is group\newFeature{v0.19.0} of options allows to change behaviour of the plugin for Telrad mode.
 \begin{description}
    \item[Enable scaling FOV for Telrad] option allows you to define whether or not to scale the Telrad circles to half of apparent FOV. When activated, the Telrad circles will fill your screen. 
 \end{description}
\end{description}
