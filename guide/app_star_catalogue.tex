...
\begin{enumerate}
  ...
  \item Generate levels 4--10 (Star2/Star3 records) from the Gaia
        ...
        catalogues).
  \item Remove the level~3/4 boundary overlap with \program{dedupcat}
        (level 4 intentionally starts 0.25 mag brighter than level 3 so
        that no star is lost at the boundary).
...
\begin{commands}
extract_columns.exe <csv_dir> <bin_dir> [workers]
\end{commands}
...
companion fork into \file{venv\textbackslash Lib\textbackslash site-packages\textbackslash}
of the Python environment after creating the virtual environment.

\paragraph{Spectral and object type tables.} \program{gaiahip2cat} needs the
object-type table \file{otype.dat}; a copy of
\file{stars/hip\_gaia3/object\_types\_v0\_1.cat} works as-is. The spectral
type table \file{stars\_hip\_sp\_0v0\_6.cat} is optional input (see
\program{--sp-table} below).
...
gaia2cat --gaia-bin <dir> --out-dir <dir> [--work-dir <dir>]
             ...
             [--exclude-lower <stars_0.cat> <stars_1.cat> <stars_2.cat> <stars_3.cat>]
...
\begin{description}
  ...
  \item[\program{--levels}] level range (default \texttt{4-10}).
        ...
        overlap with level 3, so that deduplication against level 3 (below)
        can remove the boundary duplicates without losing any star.
  \item[\program{--exclude-lower}] one or more level~0--3 \file{.cat} files
        ...
        all HIP stars already live in the level~0--3 catalogues. Levels~7--10
        keep HIP stars, exactly like the official level~7+ catalogues
        (whose ADQL queries do not apply this filter). The level~0--3 files
        are parsed on the fly, so no intermediate id list is needed.
...
\subsection{Deduplication Between Levels 3 and 4}
...
level 3. Run \program{dedupcat} once on that pair after generation:
...
the second catalogue's record format. Replace \file{stars\_4\_...\_dedup.cat}
with the original \file{stars\_4} file afterwards. Levels 0--3 must
\emph{not} be deduplicated: level 2 deliberately contains all HIP stars
...
\begin{commands}
cmpcat <ref.cat> <new.cat> [output.txt] [--compare-bv] [--bv-tol mag]
python analyze_only.py output.txt <mag_lo> <mag_hi> [margin]
\end{commands}
