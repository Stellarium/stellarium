...
bool Nebula::flagShow = true;
float Nebula::fadeCoefficient = 1.1;
...
void Nebula::draw_tex(const Projector* prj, const Navigator* nav, ToneReproductor* eye)
{
  ...
  if (flagShow)
  {
    intensity *= fadeCoefficient;
    if (intensity > 0.999) intensity = 0.99;
  }
  else 
  {
    intensity /= fadeCoefficient;
	...
	else
	{
		...
		glColor4f(cmag*intensity,cmag*intensity,cmag*intensity,intensity);
