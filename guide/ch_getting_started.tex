...
\begin{itemize}
\item \file{stellarium-\StelVersion-qt6-win64.exe} for 64-bit Windows 10 and later.
\item \file{stellarium-\StelVersion-qt6-arm64.exe} for Windows 10 and later on ARM64 device.
\item \file{stellarium-\StelVersion-qt5-win64.exe} for 64-bit Windows 7 and later.
\end{itemize}
...
\label{sec:GettingStarted:Installation:UNIX}

Check if your operating system has a package or port for Stellarium already --- 
if so you're probably best off using it. If not, you can download and build
the source.

\subsection{Haiku}
\label{sec:GettingStarted:Installation:Haiku}

Please use \textbf{HaikuDepot} or open \textbf{Terminal} and type:
\begin{commands}
pkgman install stellarium
\end{commands}
to install the Stellarium.
...
item in the \textbf{GNOME} or \textbf{KDE} application menus. If not, just open a
terminal and type \texttt{stellarium}.

\subsection{*BSD/Solaris/OpenIndiana}
\label{sec:GettingStarted:Running:UNIX}

If your operating system had a package or port you'll probably 
already have an item in the \textbf{GNOME} or \textbf{KDE} application menus. 
If not, just open a terminal and type \texttt{stellarium}.

\subsection{Haiku}
\label{sec:GettingStarted:Running:Haiku}

If you had installed a package you'll already have an
item in the \textbf{Applications} menus.

\section{Script Launch}
\label{sec:GettingStarted:Scripting}

Stellarium can be automated in the sense that a sequence of commands can be executed written in a variant of JavaScript. (See chapter~\ref{ch:scripting}.)

Stellarium scripting files use the filename extension \file{.ssc} and are registered during installation so that usually 
a double click in your file explorer should start Stellarium and run this script. 
If you want to run a script whenever you start Stellarium, add your commands into a script \file{startup.ssc}. 
To run an arbitrary script from the command line, see section~\ref{sec:CommandLineOptions}. 
