...
Note that a point may lie in zero, one or more than one constellation area when it overlaps two or even more adjacent figures. 

This functionality is an advanced topic mostly used by sky culture researchers who are also not afraid of calling scripting functions (see ch.~\ref{ch:scripting}), 
...
Please note that there is a little glitch when drawing the polygon. At some coordinates (0/6h/12h/18h in $\alpha$, and the celestial equator cause these problems) the lines are interrupted. 
...
\begin{script}
ConstellationMgr.outputHullAreas(<filename\_start>);
\end{script}
The argument \texttt{<filename\_start>} is optional and defaults to \texttt{"hullAreas"}. 
The created CVS file \file{<filename\_start>\_skycultureName.csv} in your Stellarium user data directory lists hull areas for all constellations of the current skyculture. 
