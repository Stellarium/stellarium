...
\newFeature{0.16.0}Some Observers like a simulated ocular view with very small, or more, stars, quite different from what Stellarium usually shows. 
These star scales can be set as usual in the view settings window (see section \ref{sec:gui:view:sky}). The Oculars plugin keeps separate sets of values 
for ocular views and CCD views, which are even stored permanently in the plugin's \file{ocular.ini} file.  
...
\label{sec:sec:plugins:TelescopeControl:Limitations}
...
very suitable for lunar or planetary observations. Many mounts can be set 
to a 'lunar' tracking speed, you may prefer this. 
...
Setting up RTS2 for Stellarium to be able to connect and command the telescope
is beyond scope of this handbook, but see section~\ref{sec:plugins:TelescopeControl:RTS2} for a few notes.
Please refer to the RTS2 Web pages\footnote{\url{http://rts2.org/}} for details.

Please be aware that some username might have only read-only access. If this is
case for used username, the telescope marker will be painted red (or
color\_telescope\_readonly entered in Stellarium settings).
...
\indexterm{RTS2}, the Remote Telescope System 2, is a complete robotic observatory control system by Petr Kub\'{a}nek who kindly provided the plugin code to make RTS2 interoperate with Stellarium.
We cannot give a full manual or any further support of RTS2 here, please refer to its website\footnote{\url{http://rts2.org}} for complete instructions. A few notes may be useful for beginners, though. 
...
Telescope \texttt{T0} is a dummy telescope which you can select and operate with commands like \texttt{move 120 35} (this moves to RA=120°=8h, DEC=35°).

Stellarium can communicate with RTS2 using its web interface. You must run \program{rts2-httpd} for this, 
which may require the right permissions to write the lock files in \file{/var/run}. This program requires 
that a database has been created as described in \file{RUN} and on the RTS2 website\footnote{\url{http://rts2.org/faq.html}}.   
Also, you must create a user for RTS2. This is not a regular Linux account, but an entry in the database. 
Create a user with \program{rts2-user -a <user>}. You should use a password different from your regular password, 
