...
// In this order: bottom, front, left, back, right, top
void LandscapeOldStyle::createFromCubemap(GLuint textures[]) {
    nbSideTexs = 4;
    sideTexs = new StelTextureSP[nbSideTexs];
    for (int i=0; i<nbSideTexs; ++i) {
        sideTexs[i] = StelApp::getInstance().getTextureManager().getTexture(textures[i+1]);
    }
    nbSide = 4;
    sides = new landscapeTexCoord[nbSide];
    for (int i=0; i<nbSide; ++i) {
        sides[i].tex = sideTexs[i];
        sides[i].texCoords[0] = 0;
        sides[i].texCoords[1] = 0;
        sides[i].texCoords[2] = 1;
        sides[i].texCoords[3] = 1;
    }
    nbDecorRepeat = 1;
    groundTex = StelApp::getInstance().getTextureManager().getTexture(textures[0]);
    groundTexCoord.tex = groundTex;
    groundTexCoord.texCoords[0] = 0;
    groundTexCoord.texCoords[1] = 0;
    groundTexCoord.texCoords[2] = 1;
    groundTexCoord.texCoords[3] = 1;

    fogTex = StelApp::getInstance().getTextureManager().createTexture(StelFileMgr::findFile("textures/fog.png"), StelTexture::StelTextureParams(true, GL_LINEAR, GL_REPEAT));
    fogTexCoord.tex = fogTex;
    fogTexCoord.texCoords[0] = 0;
    fogTexCoord.texCoords[1] = 0;
    fogTexCoord.texCoords[2] = 1;
    fogTexCoord.texCoords[3] = 1;
