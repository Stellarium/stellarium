...
disable specific settings from being applied on this client. Also, you can
specify which StelProperties should be excluded from synchronization, if you
know their ID, by entering them in the text box (one per line). Each client can
have different settings. This could allow installations with several screens
...
The names of all available StelProperties to be excluded from synchronisation can currently 
only be found with a little scripting. Open the script console \key{F12} and enter the following call:
\begin{commands}
