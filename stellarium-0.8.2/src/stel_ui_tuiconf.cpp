...
void StelUI::init_tui(void)
{
	...
	tui_menu_create_avi = new s_tui::MenuBranch(wstring(L"9. ") );
	...
	tui_root->addComponent(tui_menu_create_avi);
	...
	tui_general_sky_locale->addItemList(Translator::getAvailableLanguagesNamesNative(core->getLocaleDir()));
  ...
  tui_star_limiting_mag = new s_tui::Decimal_item(1,30, 1, wstring(L"4.2 ") );
	tui_star_limiting_mag->set_OnChangeCallback(callback<void>(this, &StelUI::tui_cb_stars));
	tui_star_maxradius = new s_tui::Decimal_item(1,30, 1, wstring(L"4.2 ") );
	tui_star_maxradius->set_OnChangeCallback(callback<void>(this, &StelUI::tui_cb_stars));
  ...
  tui_menu_stars->addComponent(tui_star_limiting_mag);
	tui_menu_stars->addComponent(tui_star_maxradius);
	...
	tui_scripts_local_edit = new s_tui::MultiSet_item<wstring>(wstring(L"6.3 "));
	tui_scripts_local_edit->set_OnChangeCallback(callback<void>(this, &StelUI::tui_cb_scripts_local_edit));
	tui_menu_scripts->addComponent(tui_scripts_local_edit);
	...
	tui_admin_setlocale->addItemList(Translator::getAvailableLanguagesNamesNative(core->getLocaleDir()));
	...
	//8. Create avi
	tui_scripts_create_avi = new s_tui::MultiSet_item<wstring>(wstring(L"9.1 ") );
	//	tui_scripts_local->addItemList(wstring(TUI_SCRIPT_MSG) + wstring(L"\n") 
	//			       + StelUtility::stringToWstring(app->scripts->get_script_list(core->getDataDir() + "scripts/"))); 
	tui_scripts_create_avi->set_OnChangeCallback(callback<void>(this, &StelUI::tui_cb_scripts_create_avi));
	tui_menu_create_avi->addComponent(tui_scripts_create_avi);
...
void StelUI::localizeTui(void)
{
	...
	tui_menu_create_avi->setLabel(wstring(L"9. ") + _("Create avi "));
  ...
  tui_star_limiting_mag->setLabel(wstring(L"4.3 ") + _("Limiting Magnitude: "));
	tui_star_maxradius->setLabel(wstring(L"4.4 ") + _("Maximum Star Radius: "));
	tui_star_labelmaxmag->setLabel(wstring(L"4.5 ") + _("Maximum Magnitude to Label: "));
	tui_stars_twinkle->setLabel(wstring(L"4.6 ") + _("Twinkling: "));
	...
	tui_scripts_local_edit->setLabel(wstring(L"7.3 ") + _("Edit Local Script: "));
	tui_scripts_local_edit->replaceItemList(_(TUI_SCRIPT_MSG) + wstring(L"\n") 
									   + StelUtility::stringToWstring(app->scripts->get_script_list(core->getDataDir() + "scripts/")), 0); 
	...
	//9. Create avi
	tui_scripts_create_avi->setLabel(wstring(L"9.1 ") +_("Create avi from script: "));
	tui_scripts_create_avi->replaceItemList(_(TUI_SCRIPT_MSG) + wstring(L"\n") 
									   + StelUtility::stringToWstring(app->scripts->get_script_list(core->getDataDir() + "scripts/")), 0);
	//tui_scripts_create_avi->
}
...
void StelUI::tui_update_widgets(void)
{
  ...
  tui_star_limiting_mag->setValue(core->getStarLimitingMag());
	tui_star_maxradius->setValue(core->getStarMaxRadius());
	...
	if (app->SelectedScriptForEditing=="")
	{
		tui_scripts_local_edit->setCurrent(_(TUI_SCRIPT_MSG));
	}
	...
	// 9. Create video
	if (app->SelectedScriptForAvi=="")
	{
		tui_scripts_create_avi->setCurrent(_(TUI_SCRIPT_MSG));
	}
}
...
void StelUI::tui_cb_scripts_create_avi() {
  ...
  if(tui_scripts_create_avi->getCurrent()!=_(TUI_SCRIPT_MSG)){
    app->SelectedScriptForAvi = StelUtility::wstringToString(tui_scripts_create_avi->getCurrent());
    app->SelectedScriptForAviDirectory = core->getDataDir() + "scripts/";
  } else {
    app->SelectedScriptForAvi = "";
	app->SelectedScriptForAviDirectory = "";
  }
}

// callback for changing script for editing in the notepad
void StelUI::tui_cb_scripts_local_edit() 
{
  if(tui_scripts_local_edit->getCurrent()!=_(TUI_SCRIPT_MSG)){
    app->SelectedScriptForEditing = StelUtility::wstringToString(tui_scripts_local_edit->getCurrent());
    app->SelectedScriptForEditingDirectory = core->getDataDir() + "scripts/";
  } else {
    app->SelectedScriptForEditing = "";
	app->SelectedScriptForEditingDirectory = "";
  }
}
...
void StelUI::tui_cb_admin_set_locale() {
	...
	app->setAppLanguage(Translator::nativeLanguageNameCodeToIso639_1(tui_admin_setlocale->getCurrent()));
}
...
void StelUI::tui_cb_stars()
{
  ...
  oss.str("");
	oss << "set star_limiting_mag " << tui_star_limiting_mag->getValue();
	app->commander->execute_command(oss.str());

	oss.str("");
	oss << "set star_max_radius " << tui_star_maxradius->getValue();
	app->commander->execute_command(oss.str());
}
