...
const double Observability_Old::Rad2Deg = M_180_PI;         // Convert degrees into radians
const double Observability_Old::Rad2Hr = 12./M_PI;           // Convert hours into radians
const double Observability_Old::UA =  AU;                    // 1.4958e+8;         // Astronomical Unit in Km. ==> HAS BEEN DEFINED IN StelUtils.hpp!
const double Observability_Old::TFrac = 0.9972677595628414;  // Convert sidereal time into Solar time
const double Observability_Old::halfpi = M_PI_2;         //  1.57079632675; // pi/2
const double Observability_Old::MoonT = 29.530588;           // Moon synodic period (used as first estimate of Full Moon). ==> FIND MORE DEC. PLACES!
const double Observability_Old::RefFullMoon = 2451564.696; // Reference Julian date of a Full Moon.
const double Observability_Old::MoonPerilune = 0.0024236308; // Smallest Earth-Moon distance (in AU).


Observability_Old::Observability_Old()
...
void Observability_Old::updateMessageText()
{
...
double Observability_Old::getCallOrder(StelModuleActionName actionName) const
{
...
/////////////////////////////////////////////
// MAIN CODE:
void Observability_Old::draw(StelCore* core)
{
	if (!flagShowReport)
...
double Observability_Old::calculateHourAngle(double latitude,
                                         double elevation,
                                         double declination)
{
...
double Observability_Old::Lambda(double RA1, double Dec1, double RA2, double Dec2)
{
...
double Observability_Old::HourAngle2(double RA, double ST)
{
...
void Observability_Old::double2hms(double hfloat, int &h1, int &h2, int &h3)
{
...
double Observability_Old::toUnsignedRA(double RA)
{
...
QString Observability_Old::formatAsDate(int dayNumber)
{
	int day, month, year;
...
QString Observability_Old::formatAsDateRange(int startDay, int endDay)
{
...
void Observability_Old::updatePlanetData(StelCore *core)
{
	double tempH;
	for (int i=0; i<nDays; i++)
	{
...
void Observability_Old::updateSunData(StelCore* core) 
{
...
void Observability_Old::updateSunH()
{
...
bool Observability_Old::CheckRise(int day)
{
...
int Observability_Old::calculateHeli(int imethod, int &heliRise, int &heliSet)
{
	Q_UNUSED(imethod)
...
///////////////////////////////////////////
// Finds the dates of Acronichal (Rise, Set) and Cosmical (Rise2, Set2) dates.
int Observability_Old::calculateAcroCos(int &acroRise, int &acroSet,
...
void Observability_Old::toRADec(Vec3d vec3d, double& ra, double &dec)
{
	vec3d.normalize();
...
double Observability_Old::sign(double d)
{
...
void Observability_Old::getSunMoonCoords(StelCore *core, QPair<double, double> JD,
...
// getBack controls whether Earth and Moon must be returned to their original positions after computation.
void Observability_Old::getMoonDistance(StelCore *core, QPair<double, double> JD, double &distance, bool getBack)
{
	if (getBack) // Return the Moon and Earth to their current position:
	{
		myEarth->computePosition(JD.second);
...
void Observability_Old::getPlanetCoords(StelCore *core, QPair<double, double> JD, double &RA, double &Dec, bool getBack)
{
	if (getBack)
	{
	// Return the planet to its current time:
...
bool Observability_Old::calculateSolarSystemEvents(StelCore* core, int bodyType)
{
	const int NUM_ITER = 100;
	int i;
	double hHoriz, ra, dec, raSun = 0.0, decSun = 0.0, tempH, /* tempJd, */
	tempEphH, curSidT, eclLon;
...
bool Observability_Old::configureGui(bool show)
{
	if (show)
...
void Observability_Old::resetConfiguration()
{
...
void Observability_Old::loadConfiguration()
{
	QSettings* conf = StelApp::getInstance().getSettings();
...
void Observability_Old::saveConfiguration()
{
	QSettings* conf = StelApp::getInstance().getSettings();
...
void Observability_Old::enableTodayField(bool enabled)
{
	show_Today = enabled;
	configChanged = true;
}

void Observability_Old::enableAcroCosField(bool enabled)
{
	show_AcroCos = enabled;
	configChanged = true;
}

void Observability_Old::enableGoodNightsField(bool enabled)
{
	show_Good_Nights = enabled;
	configChanged = true;
}

void Observability_Old::enableOppositionField(bool enabled)
{
	show_Best_Night = enabled;
	configChanged = true;
}

void Observability_Old::enableFullMoonField(bool enabled)
{
	show_FullMoon = enabled;
	configChanged = true;
}

bool Observability_Old::getShowFlags(int iFlag)
{
	switch (iFlag)
	{
...
Vec3f Observability_Old::getFontColor(void)
{
	return fontColor;
}

int Observability_Old::getFontSize(void)
{
	return fontSize;
}

int Observability_Old::getTwilightAltitude()
{
	return twilightAltDeg;
}

int Observability_Old::getHorizonAltitude()
{
	return horizonAltDeg;
}


void Observability_Old::setFontColor(const Vec3f& color)
{
	fontColor = color; // Vector3::operator =() is overloaded. :)
}

void Observability_Old::setFontSize(int size)
{
	fontSize = size;
}

void Observability_Old::setTwilightAltitude(int altitude)
{
...
void Observability_Old::setHorizonAltitude(int altitude)
{
...
void Observability_Old::showReport(bool b)
{
	if (b!=flagShowReport)
	{
		flagShowReport = b;
