 ...
 * - Dynamic action loading from Stellarium's native action system
 ...
 * - Full button remapping with dynamic action categories from Stellarium
 ...
 * - Dynamic action loading from Stellarium's native action system (v2.7.0)
 * 
 * @module gpcontroller
 * @requires jquery
 * @requires settings
...
define(["jquery", "settings", "api/remotecontrol", "api/viewcontrol", "api/actions", "api/time", "api/properties", "api/search", "jquery-ui", "ui/stellarium-utils"],
    function($, settings, rc, viewcontrol, actions, timeApi, propApi, searchApi, jqui, stelUtils) {
        ...
        // =====================================================================
        // SECTION 2: EDUCATIONAL CUSTOM ACTIONS (Not part of Stellarium's native action system)
        // =====================================================================
        ...
        var nativeActionCategories = {}; // Categories for native StelActions
        var actionsInitialized = false;   // Flag indicating if actions have been loaded
        var actionLoadCallbacks = [];      // Callbacks to execute when actions are loaded
        ...
        /**
         * Loads all StelActions from Stellarium's native action system.
         * This function replicates the behavior from ui/actions.js to dynamically
         * load the complete action list from the server.
         * 
         * @param {function} callback - Optional callback to execute when loading is complete
         */
        function loadNativeActions(callback) {
            console.log("[Gamepad] Loading native StelActions from Stellarium...");
            
            $.ajax({
                url: "/api/stelaction/list",
                success: function(data) {
                    nativeActionData = {};
                    nativeActionCategories = {};
                    
                    $.each(data, function(categoryName, actionsList) {
                        // Create category in nativeActionCategories if it doesn't exist
                        if (!nativeActionCategories[categoryName]) {
                            nativeActionCategories[categoryName] = [];
                        }
                        
                        $.each(actionsList, function(idx, action) {
                            // Store action data
                            nativeActionData[action.id] = {
                                id: action.id,
                                text: action.text,
                            ...
                            // Add action ID to category
                            nativeActionCategories[categoryName].push(action.id);
                        });
                    });
                    
                    actionsInitialized = true;
                    console.log("[Gamepad] Loaded " + Object.keys(nativeActionData).length + " native StelActions from Stellarium");
                    
                    // Execute any pending callbacks
                    for (var i = 0; i < actionLoadCallbacks.length; i++) {
                        actionLoadCallbacks[i]();
                    }
                    actionLoadCallbacks = [];
                ...
                error: function(xhr, status, errorThrown) {
                    console.error("[Gamepad] Error loading StelActions:", errorThrown);
        ...
        /**
         * Waits for native actions to be loaded before executing a callback.
         * @param {function} callback - The callback to execute when actions are loaded
         */
        function waitForActions(callback) {
            if (actionsInitialized) {
                callback();
            } else {
                actionLoadCallbacks.push(callback);
            }
        }

        /**
         * Gets the display name for an action (either native StelAction or educational custom action).
         * @param {string} actionId - The action identifier
         * @returns {string} The display name for the action
         */
        function getActionDisplayName(actionId) {
            // Check if it's an educational custom action
            if (EDUCATIONAL_CUSTOM_ACTIONS[actionId]) {
                return tr(EDUCATIONAL_CUSTOM_ACTIONS[actionId]);
            }
            
            // Check if it's a native StelAction
            if (nativeActionData[actionId]) {
                return tr(nativeActionData[actionId].text);
            }
            
            // Fallback for unknown actions
            if (actionId === "none") return tr("No action");
            return actionId;
        }

        /**
         * Checks if an action is checkable (toggle action).
         * @param {string} actionId - The action identifier
         * @returns {boolean} True if the action is checkable
         */
        function isActionCheckable(actionId) {
            return nativeActionData[actionId] ? nativeActionData[actionId].isCheckable : false;
        }

        /**
         * Gets the current checked state of a checkable action.
         * @param {string} actionId - The action identifier
         * @returns {boolean} Current checked state, or false if not checkable
         */
        function getActionCheckedState(actionId) {
            return nativeActionData[actionId] ? nativeActionData[actionId].isChecked : false;
        }

        /**
         * Builds the complete action structure for the dropdown menu.
         * Combines native StelActions from Stellarium with educational custom actions.
         * @returns {Object} Object with category names as keys and arrays of action IDs as values
         */
        function buildActionStructure() {
            var result = {};
            
            // Add native StelAction categories (copy from nativeActionCategories)
            for (var category in nativeActionCategories) {
                if (nativeActionCategories.hasOwnProperty(category)) {
                    // Filter out any actions that might have display issues
                    var filteredActions = [];
                    for (var i = 0; i < nativeActionCategories[category].length; i++) {
                        var actionId = nativeActionCategories[category][i];
                        // Include all native actions that have valid display text
                        if (nativeActionData[actionId] && nativeActionData[actionId].text) {
                            filteredActions.push(actionId);
                        }
                    }
                    if (filteredActions.length > 0) {
                        result[category] = filteredActions;
                    }
                }
            }
            
            // Add educational custom action categories
            for (var eduCategory in EDUCATIONAL_CATEGORIES) {
                if (EDUCATIONAL_CATEGORIES.hasOwnProperty(eduCategory)) {
                    result[eduCategory] = EDUCATIONAL_CATEGORIES[eduCategory];
                }
            }
        ...
        function testVibrationManual() {
            var device = gamepadManager ? gamepadManager.getActiveDevice() : null;
            if (!device) {
                showNotification(tr("No controller connected"));
                return;
            }
            ...
            if (success) {
                showNotification(tr("Vibration test sent to ") + device.getDisplayInfo().name + 
                        " (Intensity: " + Math.round(controllerSettings.vibrationIntensity * 100) + "%)");
            } else {
                showNotification(tr("Vibration not supported on this device/browser"));
            }
        }

        // =====================================================================
        // SECTION 7: EDUCATIONAL ACTION HANDLERS
        // =====================================================================

        /**
         * Handles educational custom actions (view directions, zoom, search, etc.).
         * These actions are not part of Stellarium's native StelAction system.
        ...
        GamepadDevice.prototype.importProfile = function(profile) {
            try {
                if (profile.deviceInfo && profile.deviceInfo.uniqueId !== this.uniqueId) {
                    ...
                    showNotification(tr("Warning: This profile was created for a different controller type."));
                }
        ...
        /**
         * Handles button actions using Stellarium APIs and educational custom actions.
         * @param {string} action - The action identifier
         */
        GamepadDevice.prototype.handleButtonAction = function(action) {
            ...
            // Tab navigation (custom action not in native StelActions)
            if (action === "tab_previous") { navigateTab(-1); return; }
            if (action === "tab_next") { navigateTab(1); return; }
            
            // View controls (custom actions)
        ...
        GamepadDevice.prototype.processInput = function() {
            ...
            if (this.index === activeDeviceIndex && $currentDirection && $currentDirection.length) {
                ...
                if (direction) {
                    ...
                    $currentDirection.text(tr("centered"));
                }
            }
        ...
        /**
         * Updates the current direction display based on stick position.
         * @param {number} leftX - X axis value (-1 to 1)
         * @param {number} leftY - Y axis value (-1 to 1)
         * @returns {string|null} Direction text or null if centered
         */
        GamepadDevice.prototype.updateCurrentDirection = function(leftX, leftY) {
            ...
            if (angle >= -22.5 && angle < 22.5) direction = tr("East →");
            else if (angle >= 22.5 && angle < 67.5) direction = tr("North-East ↗");
            else if (angle >= 67.5 && angle < 112.5) direction = tr("North ↑");
            else if (angle >= 112.5 && angle < 157.5) direction = tr("North-West ↖");
            else if (angle >= 157.5 || angle < -157.5) direction = tr("West ←");
            else if (angle >= -157.5 && angle < -112.5) direction = tr("South-West ↙");
            else if (angle >= -112.5 && angle < -67.5) direction = tr("South ↓");
            else if (angle >= -67.5 && angle < -22.5) direction = tr("South-East ↘");
            
            var intensity = Math.sqrt(leftX * leftX + leftY * leftY);
            var speedText = intensity > 0.8 ? tr("fast") : (intensity > 0.3 ? tr("medium") : tr("slow"));
        ...
        /**
         * Starts background scanning for gamepad devices.
        ...
        // =====================================================================
        // SECTION 10: UI UPDATE FUNCTIONS
        // =====================================================================

        function updateConnectionStatus(connected, device) {
            if ($connectionStatus && $connectionStatus.length) {
                $connectionStatus.html(connected ? 
                        '<span style="color:#4CAF50;">● ' + tr("Connected") + '</span>' : 
                        '<span style="color:#f44336;">● ' + tr("Disconnected") + '</span>');
            }
        ...
        function updateDeviceSelector() {
            ...
            if (devices.length === 0) {
                $deviceSelector.append('<option value="">' + tr("No devices connected") + '</option>');
        ...
        /**
         * Populates the button customization panel with dynamic actions from Stellarium.
         * This function replaces the static action list with dynamically loaded StelActions.
         */
        function populateButtonCustomization() {
            ...
            if (!device) {
                $buttonCustomizationContainer.html('<div class="no-device-message"><p>🔴 ' + tr("Connect a controller to customize buttons") + '</p></div>');
                return;
            }
            ...
            // Wait for actions to be loaded before building the dropdown
            waitForActions(function() {
                var buttonGroups = {
                ...
                for (var groupName in buttonGroups) {
                    ...
                    for (var j = 0; j < buttonIndices.length; j++) {
                        ...
                        select.append("<option value='none'>" + tr("No action") + "</option>");
                        ...
                        select.on("change", (function(btnId, btnName) {
                            return function() {
                                ...
                                if (dev) {
                                    ...
                                    showNotification(tr("Button %1 mapped to %2", btnName, getActionDisplayName(newAction)));
                                }
                            };
                        })(buttonId, buttonName));
        ...
        function startCalibration() {
            var device = gamepadManager ? gamepadManager.getActiveDevice() : null;
            if (!device) {
                showNotification(tr("Please connect a controller first"));
                return;
            }
            ...
            if ($calibrateBtn) $calibrateBtn.text(tr("Calibrating...")).addClass("calibrating");
            showNotification(tr("Move both joysticks in circles, then press any button to finish"));
        ...
        function finishCalibration() {
            ...
            if (!device) {
                ...
                if ($calibrateBtn) $calibrateBtn.text(tr("Calibrate")).removeClass("calibrating");
                return;
            }
            ...
            if ($calibrateBtn) $calibrateBtn.text(tr("Calibrate")).removeClass("calibrating");
            showNotification(tr("Calibration complete! ") + calibrationData.samples.length + tr(" samples collected."));
        ...
        function updateCalibrationDisplayForDevice(device, isCalibrating) {
            ...
            if (isCalibrating) {
                container.html("<div class='calibration-status'>" + 
                                tr("Calibrating... Collected %1 samples", calibrationData.sampleCount) + 
                                "<br><span style='font-size:10px;'>" + 
                                tr("Move joysticks in circles, press any button to finish") + 
                                "</span></div>");
                ...
                container.html(
                    "<div class='calibration-values'>" +
                        "<div class='calibration-value-item'><span class='calibration-value-label'>" + tr("Left X offset:") + "</span> <span class='calibration-value-number'>" + data.leftX.toFixed(4) + "</span></div>" +
                        "<div class='calibration-value-item'><span class='calibration-value-label'>" + tr("Left Y offset:") + "</span> <span class='calibration-value-number'>" + data.leftY.toFixed(4) + "</span></div>" +
                        "<div class='calibration-value-item'><span class='calibration-value-label'>" + tr("Right X offset:") + "</span> <span class='calibration-value-number'>" + data.rightX.toFixed(4) + "</span></div>" +
                        "<div class='calibration-value-item'><span class='calibration-value-label'>" + tr("Right Y offset:") + "</span> <span class='calibration-value-number'>" + data.rightY.toFixed(4) + "</span></div>" +
                    "</div>"
                );
            } else {
                container.html("<div class='calibration-status'>" + tr("Not calibrated") + "</div>");
            }
        }
        ...
        function resetCalibration() {
            ...
            if (device) {
                ...
                device.saveCalibration();
                updateCalibrationDisplayForDevice(device);
                showNotification(tr("Calibration reset for ") + device.getDisplayInfo().name);
            }
        }

        // =====================================================================
        // SECTION 12: SETTINGS MANAGEMENT
        // =====================================================================
        ...
        function resetButtonMappings() {
            ...
            if (device) {
                ...
                populateButtonCustomization();
                showNotification(tr("Button mappings reset to defaults for ") + device.getDisplayInfo().name);
            } else {
                showNotification(tr("No controller connected"));
            }
        }
        ...
        function exportProfile() {
            ...
            showNotification(tr("Profile exported: ") + profileName);
        }
            ...
            reader.onload = function(e) {
                try {
                    ...
                    if (device.importProfile(profile)) {
                        populateButtonCustomization();
                        updateCalibrationDisplayForDevice(device);
                        showNotification(tr("Profile imported successfully for ") + device.getDisplayInfo().name);
                    } else {
                        showNotification(tr("Failed to import profile"));
                    }
                } catch(err) {
                    console.error("Import error:", err);
                    showNotification(tr("Invalid profile file"));
                }
            };
        ...
        function setupUI() {
            ...
            if ($vibrationFeedback.length) {
                $vibrationFeedback.prop("checked", controllerSettings.vibrationFeedback).on("change", function() {
                    ...
                    showNotification(controllerSettings.vibrationFeedback ? 
                            tr("Vibration feedback enabled") : tr("Vibration feedback disabled"));
                });
            ...
            if ($zoomSpeedSlider.length) {
                $zoomSpeedSlider.slider({
                    ...
                    slide: function(e, ui) {
                        ...
                        $("#gp-zoom-speed-value").text(ui.value.toFixed(3));
                        saveSettings();
                    }
                });
            }
            ...
            $("#gp-save-mappings").on("click", function() {
                ...
                if (device) {
                    device.saveButtonMappings();
                    showNotification(tr("Button mappings saved for ") + device.getDisplayInfo().name);
                } else {
                    showNotification(tr("No controller connected"));
                }
            });
            ...
            $("#gp-connect").on("click", function() {
                ...
                $btn.text(tr("Scanning...")).prop("disabled", true);
                ...
                if (gamepadManager) {
                    ...
                    if (foundNewDevices) {
                        ...
                        updateDeviceSelector();
                        populateButtonCustomization();
                        showNotification(tr("Found ") + gamepadManager.devices.size + tr(" controller(s)"));
                    } else {
                        ...
                        if (gamepadManager.devices.size > 0) {
                            ...
                            showNotification(tr("No controllers found. Please connect a controller and try again."));
                        }
                    }
                }
            ...
            $("#gp-zoom-speed-value").text(controllerSettings.zoomSpeed.toFixed(3));
        ...
        function updateLiveInputDisplay() {
            ...
            if (!device || activeDeviceIndex === null) {
                $('#gp-buttons-container').html('<div class="loading-placeholder">' + tr("No controller connected") + '</div>');
                $('#gp-axes-container').html('<div class="loading-placeholder">' + tr("No controller connected") + '</div>');
                $('#gp-device-info-live').html('<div class="loading-placeholder">' + tr("No controller connected") + '</div>');
                return;
            }
            
            var gp = navigator.getGamepads()[device.index];
            if (!gp) {
                $('#gp-buttons-container').html('<div class="loading-placeholder">' + tr("Controller data not available") + '</div>');
                $('#gp-axes-container').html('<div class="loading-placeholder">' + tr("Controller data not available") + '</div>');
                $('#gp-device-info-live').html('<div class="loading-placeholder">' + tr("Controller data not available") + '</div>');
                return;
            }
        ...
        function updateButtonsDisplay(gamepad) {
            ...
            if (!gamepad || !gamepad.buttons || gamepad.buttons.length === 0) {
                $container.html('<div class="loading-placeholder">' + tr("No button data available") + '</div>');
                return;
            }
        ...
        /**
         * Renders the axes panel with raw values and visual meters
         */
        function updateAxesDisplay(gamepad) {
            ...
            if (!gamepad || !gamepad.axes || gamepad.axes.length === 0) {
                $container.html('<div class="loading-placeholder">' + tr("No axis data available") + '</div>');
                return;
            }
        ...
        function updateDeviceInfoLive(gamepad, device) {
            ...
            var html = '';
            html += '<div class="device-info-live-row"><span class="label">' + tr("Device Name") + ':</span><span class="value">' + escapeHtml(info.name) + '</span></div>';
            html += '<div class="device-info-live-row"><span class="label">' + tr("Vendor/Product ID") + ':</span><span class="value">' + (device.vendorId ? device.vendorId + ':' + device.productId : 'Unknown') + '</span></div>';
            html += '<div class="device-info-live-row"><span class="label">' + tr("Button Count") + ':</span><span class="value">' + gamepad.buttons.length + '</span></div>';
            html += '<div class="device-info-live-row"><span class="label">' + tr("Axis Count") + ':</span><span class="value">' + (gamepad.axes ? gamepad.axes.length : 0) + '</span></div>';
            html += '<div class="device-info-live-row"><span class="label">' + tr("Operating System") + ':</span><span class="value">' + osInfo + '</span></div>';
            html += '<div class="device-info-live-row"><span class="label">' + tr("Browser") + ':</span><span class="value">' + browserInfo + '</span></div>';
            
            // Add browser-specific notes
            if (browserInfo.includes('Firefox') && info.name.includes('DualSense')) {
                html += '<div class="device-info-live-row"><span class="label">' + tr("Note") + ':</span><span class="value">Firefox may report PS5 DualSense axes differently. Verify using the Axes panel.</span></div>';
            }
            if (browserInfo.includes('Chrome') && info.name.includes('DualSense')) {
                html += '<div class="device-info-live-row"><span class="label">' + tr("Note") + ':</span><span class="value">Chrome provides full PS5 DualSense support including vibration.</span></div>';
            }
         ...
         * Loads native StelActions from Stellarium for dynamic button customization.
         * 
         * @returns {void}
         */
        function init() {
            console.log("[Gamepad Controller] Initializing with W3C Gamepad API v2.7.0");
            loadSettings();
            setupUI();
            
            // Load native StelActions from Stellarium before initializing the manager
            // This ensures actions are available when populating the dropdown
            loadNativeActions(function() {
                console.log("[Gamepad Controller] Native StelActions loaded successfully");
            });
        ...
        return {
            ...
            /**
             * Gets the list of available native StelActions
             * @returns {Object} Map of action IDs to action data
             */
            getNativeActions: function() {
                return $.extend(true, {}, nativeActionData);
            },
            
            /**
             * Gets the list of educational custom actions
             * @returns {Object} Map of action IDs to display names
             */
            getEducationalActions: function() {
                return $.extend(true, {}, EDUCATIONAL_CUSTOM_ACTIONS);
            }
        };
    }
);