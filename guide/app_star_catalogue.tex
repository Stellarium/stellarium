...
reproduced from public data (Gaia DR3 and SIMBAD) with the converter tools
in \file{util/gaiacat/}. Once the input data has been downloaded, the whole
pipeline runs offline; no network access is needed during generation.

The level 0--3 workflow is a C++ re-implementation of the
\texttt{Star\_Catalog\_lv0\_6.ipynb} notebook from Henry Leung's
\emph{stellarium\_star\_catalogs} repository (University of Toronto,
GPLv3\footnote{\url{https://github.com/henrysky/stellarium_star_catalogs}}).
The two Python helper scripts referenced below were derived from the same
repository and remain under the terms of its GPLv3 license.
...
\begin{enumerate}
  ...
  \item Verify the output against a reference catalogue with \program{cmpcat}.
\end{enumerate}
...
photometry, spectral types and radial velocities. Two scripts (derived from
\emph{stellarium\_star\_catalogs}, see the license note above) produce them:
...
A/B components, and cross-matches every entry to a Gaia DR3
\texttt{source\_id}. It writes two files into \file{simbad\_query\_results/}:
...
\texttt{source\_id}. Drop the following file as
\file{sitecustomize.py} into \file{venv\textbackslash Lib\textbackslash site-packages\textbackslash}
of the Python environment to replace the affected parser with a pure-Python
int64 version:
...
def _long_binparse_py(self, read):
    data = read(8)
    value = struct.unpack(">q", data)[0]
    if self._fast_has_null and value == self._fast_null_param:
        return value, True
    return value, False

converters.Long.binparse = _long_binparse_py
\end{configfile}
...
\begin{description}
  ...
  \item[\program{--erfa-python}] a Python interpreter with astropy
        ...
        reference pipeline; otherwise a built-in IAU SOFA equivalent is used.
\end{description}
