	...
	flagBoundaries(0)
{
	...
	stickyConstellationSelected = false;
	stickySelection.clear();
}
...
void ConstellationMgr::loadLinesAndArt(const string &fileName, const string &artfileName, const string &boundaryfileName, LoadingBar& lb)
{
	...
	while(!std::getline(inf, record).eof())
	{
		...
		if(cons->read(record, hipStarMgr))
		{
			...
			stickySelection.push_back(false);
		} else
		{ 
...
void ConstellationMgr::setFlagLines(bool b)
{
	...
	if (selected && isolateSelected)
	{
		...
		if (stickyConstellationSelected)
		{
			vector < Constellation * >::const_iterator iter;
			vector < bool> ::iterator iterStickySelection;
			for (iter = asterisms.begin(), iterStickySelection = stickySelection.begin(); 
				(iter != asterisms.end())&&(iterStickySelection != stickySelection.end()); 
				++iter, ++iterStickySelection)
			{
	...
	if (c != NULL)
	{
		...
		if (isolateSelected)
		{
				...
				++iter, ++iterStickySelection)
		    {
                if ((*iter) != selected )
	    	    {   	    
	    			if (!(*iterStickySelection)||!stickyConstellationSelected)
	    			{
