...
\item[Interface:] this group of options allows to change behaviour of the plugin in general.
 \begin{description}
   ...
   \item[Show resolution criteria] allow compute and show Rayleigh criterion, Dawes', Abbe's and Sparrow's limits for combination of telescope, lens and eyepiece. Plus this option will and show visual resolution for selected ``setup''. This data may be very helpful for double star observers.
   \item[Arrow button scale] allows to change the size of arrow-buttons.
 \end{description}
...
\item The \emph{Type} field indicates what kind of connection is this:
  ...
  \item[remote, unknown] means an INDIRECT connection over a network
    to a remote machine.
  \item[remote, RTS2] means an INDIRECT connection over a network
        to a RTS2 server.
  \item[remote, INDI] means an INDIRECT connection over a network
        to an INDI server.
  \end{description}
\end{itemize}
...
You will need access to INDI server through TCP/IP connection.\newFeature{v0.16.0}
Please see INDI documentation for details. You will need to select correct INDI device to manage of them.
...
\indexterm{INDI}, \newFeature{v0.17.0}the Instrument Neutral Distributed Interface, is a distributed control system (DCS) protocol to enable control, data acquisition and exchange among hardware devices and software front ends, emphasizing astronomical instrumentation\footnote{\url{https://en.wikipedia.org/wiki/Instrument_Neutral_Distributed_Interface}}.
 
INDI client (v1.6.0) has been incorporated into Telescope Control plugin by Alessandro Siniscalchi to allow the Stellarium communicate with INDI hardware drivers via INDI server. INDI server is the hub that sits between drivers and clients. It reroutes traffic for control and data across distributed networks. Each device or client in the network is a node and may communicate with other nodes whenever desired. The server supports broadcasting, chaining, and marshaling of data.

Stellarium may works with INDI mounts only and you will need to select correct INDI device to manage of them on the stage of configuration of telescope. For this purpose the plug-in has tool to getting list of connected devices to the INDI server (when server is working!). Please define hostname and port\footnote{Internet Assigned Numbers Authority (IANA) has assigned INDI to Transmission Control Protocol (TCP) and User Datagram Protocol (UDP), or TCP/UDP port 7624.} in ``INDI Settings'' block, press ``Refresh devices'' button and select required device in the list ``Devices''. 

List of supported type of telescopes by INDI\footnote{\url{http://indilib.org/devices/telescopes/all.html}}:
\begin{itemize}
\item Meade Autostar
...
This Plugin is activated with the button \guibutton[0.5]{0.3}{bt_observab_off}. 
