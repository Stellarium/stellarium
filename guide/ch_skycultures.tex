...
\begin{description}
\item[\jtag{id}] A unique figure identifier, consisting of \texttt{CON} (for \indexterm{constellations}), skyculture id, and a short label still unique within the skyculture.
...
\item[\jtag{common\_name}] Dictionary of names. %Required entries: \jtag{english}. Optional: \jtag{native}
	...
	and explain the context of stars, especially where only an untranslatable proper name is given.
...
To exclude entries from being displayed by a particular skyculture, add an entry like this to \file{config.ini}:
\begin{configfile}
[SCExcludeReferences]
