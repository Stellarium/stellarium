...
QList< QPair<float, float> > StarMgr::getListMagnitudeRadius(StelCore *core)
{
	...
	for (ZoneArrayMap::const_iterator it(zoneArrays.constBegin()); it!=zoneArrays.constEnd();++it)
	{
		...
		{
			double magnitude=mag_table[index];
			double maginteger=magnitude-(int) magnitude;
			if ((int) magnitude==0)
				magnitude=0;
			if (rcmag_table[2*index]>0. && rcmag_table[2*index]<13.&& (int) (maginteger*100000)==0)
			{
				QPair<float, float> pairMagnitudeRadius;
				pairMagnitudeRadius.first=(int) magnitude;
				pairMagnitudeRadius.second=rcmag_table[2*index];
				bool found=false;
