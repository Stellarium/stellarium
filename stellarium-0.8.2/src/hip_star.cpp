...
bool HipStar::apply_degamma = false;
bool HipStar::hyper_stars = false;
float HipStar::evenness_coeff = 0.0;
float HipStar::star_max_radius = 5.0;
...
void HipStar::draw(const Vec3d &XY)
{
  // Compute the equivalent star luminance for a 5 arc min circle and convert it
	...
	float rmag = eye->adapt_luminance(term1) * powf(proj->get_fov(),-0.85f) * 70.f;
	float cmag = 1.f;
  ...
  if (rmag<1.2f)
  {
      ...
      /*STARS FIX*/
      if (rmag/star_scale<0.1f) return;
	    //if (!hyper_stars)
	    //{
	      if (cmag*star_mag_scale < evenness_coeff) return;
	    //}
	    /*STARS FIX*/
      rmag=1.2f;
  }
  ...
  {
    float k2 = 2/(15-star_max_radius);
    float b2 = (15-3*star_max_radius)/(15-star_max_radius);

	  if (rmag>star_max_radius)
    {
      cmag = rmag*k2 + b2;
      rmag = 2.f/9.f * (4.f + sqrt((double)(16.f+4.5*(rmag-1))));
 	  }
	}
  ...
  cmag*=(1.-twinkle_amount*rand()/RAND_MAX);
	...
	/*STARS FIX*/
	Vec3f greyVector(1.0, 1.0, 1.0);
	greyVector.normalize();

	Vec3f resultColor = star_colors[type]*cmag;

	float interproduct = resultColor.dot(greyVector);

	//Apply degamma
	if (apply_degamma)
	{
		resultColor *= pow(interproduct, static_cast<float>(1.0/2.51)) / interproduct;
	}
	
	if (interproduct < 1.0)
	{
	    interproduct = resultColor.dot(greyVector);
	    resultColor *= 1.0 / interproduct;
	}	
