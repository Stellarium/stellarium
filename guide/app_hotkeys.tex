...
\emph{WARNING: The list of hotkeys in this Guide using notation for standard PC-compatible keyboards. 
			   Not all keyboards is a PC-compatible -- for example the Apple's computers using keyboards 
			   with own specifics keys -- \key{Option} and \key{Command}. The \key{Option} key is equivalent 
			   the \key{Alt} key on the PC-compatible keyboards. The Haiku operating system is the successor 
			   of BeOS and swapping behaviour for \key{\ctrl} and \key{Alt} keys -- e.g. PC's \key{\ctrl+A} 
			   will be \key{Alt+A} in Haiku.}
...
\key{\ctrl+\shift+N}	& Native planet names (from sky culture) \\
...
\toprule
\emph{Shortcut key} 	& \emph{Description}\\\midrule
\key{\ctrl+\shift+C}	 		& Copy selected object information to clipboard \\
\key{\ctrl+Q}	 		& Quit \\
