...
void HipStarMgr::draw(Vec3f equ_vision, ToneReproductor* eye, Projector* prj)
{
	...
	HipStar::apply_degamma = applyDegamma;

  HipStar::hyper_stars = hyperStars;
  if (hyperStars) 
  {
    HipStar::evenness_coeff = min(0.1f, HipStar::evenness_coeff + 0.002);
  }
  else
  {
    HipStar::evenness_coeff = max(0.0f, HipStar::evenness_coeff - 0.002);
  }
	
	HipStar::star_max_radius = starMaxRadius;
