...
\item The \emph{Type} field indicates what kind of connection this is:
  ...
  \item[direct, Stellarium] means a DIRECT connection to the telescope (see above).
...
\begin{description}
\item[Stellarium, directly (serial or network)] is the DIRECT case.
  LX200-compatible mounts can be connected either through a serial port or,
  \newFeature{26.3} over a TCP/IP network (see \emph{Device settings}).
...
\begin{description}
\item[Connection]\newFeature{26.3} selects \emph{Serial port} or, for
  LX200-compatible mounts, \emph{Network (TCP)} -- the LX200 protocol spoken
  directly over TCP/IP. With \emph{Network (TCP)} the \emph{Serial port} field
  is replaced by \emph{Host} and \emph{Port} (the telescope's network address
  and TCP port).
\item[Serial port] sets the serial port used by the telescope.
  ...
  \item On macOS, the list is empty as it names its ports in a
    peculiar way.
  ...
  all valid serial port names in macOS, open a terminal and type:
