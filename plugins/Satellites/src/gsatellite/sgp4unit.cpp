		  ...
		  double& zmos)
{
	...
	snodm  = std::sin(nodep);
	cnodm  = std::cos(nodep);
	sinomm = std::sin(argpp);
	cosomm = std::cos(argpp);
	sinim  = std::sin(inclp);
	cosim  = std::cos(inclp);
	emsq   = em * em;
	betasq = 1.0 - emsq;
	...
	xnodce = std::fmod(4.5236020 - 9.2422029e-4 * day, twopi);
	stem   = std::sin(xnodce);
	ctem   = std::cos(xnodce);
	...
	zmol = std::fmod(4.7199672 + 0.22997150  * day - gam, twopi);
	zmos = std::fmod(6.2565837 + 0.017201977 * day, twopi);
		   ...
		   double& xfact, double& xlamo, double& xli,    double& xni)
{
	...
	dndt   = 0.0;
	theta  = std::fmod(gsto + tc * rptim, twopi);
	em     = em + dedt * t;
	inclm  = inclm + didt * t;
	argpm  = argpm + domdt * t;
	nodem  = nodem + dnodt * t;
	...
	if (irez != 0)
	{
		...
		if (irez == 2)
		{
			...
			xlamo =  std::fmod(mo + nodeo + nodeo-theta - theta, twopi);
		...
		if (irez == 1)
		{
			...
			xlamo = std::fmod(mo + nodeo + argpo - theta, twopi);
		   ...
		   double& mm,    double& xni,   double& nodem,  double& dndt,  double& nm)
{
	...
	if (irez != 0)
	{
		...
		if ((atime == 0.0) || (t * atime <= 0.0) || (std::fabs(t) < std::fabs(atime)) )
		{
			atime  = 0.0;
			xni    = no;
			xli    = xlamo;
		...
		while (iretn == 381)
		{
			...
			if (irez != 2)
			{
				xndt  = del1 * std::sin(xli - fasx2) + del2 * std::sin(2.0 * (xli - fasx4)) +
					del3 * std::sin(3.0 * (xli - fasx6));
				xldot = xni + xfact;
				xnddt = del1 * std::cos(xli - fasx2) +
					2.0 * del2 * std::cos(2.0 * (xli - fasx4)) +
					3.0 * del3 * std::cos(3.0 * (xli - fasx6));
				xnddt = xnddt * xldot;
			}
			else
			{
				...
				x2omi = xomi + xomi;
				x2li  = xli + xli;
				xndt  = d2201 * std::sin(x2omi + xli - g22) + d2211 * std::sin(xli - g22) +
					d3210 * std::sin(xomi + xli - g32)  + d3222 * std::sin(-xomi + xli - g32)+
					d4410 * std::sin(x2omi + x2li - g44)+ d4422 * std::sin(x2li - g44) +
					d5220 * std::sin(xomi + xli - g52)  + d5232 * std::sin(-xomi + xli - g52)+
					d5421 * std::sin(xomi + x2li - g54) + d5433 * std::sin(-xomi + x2li - g54);
				xldot = xni + xfact;
				xnddt = d2201 * std::cos(x2omi + xli - g22) + d2211 * std::cos(xli - g22) +
					d3210 * std::cos(xomi + xli - g32)  + d3222 * std::cos(-xomi + xli - g32) +
					d5220 * std::cos(xomi + xli - g52)  + d5232 * std::cos(-xomi + xli - g52) +
					2.0 * (d4410 * std::cos(x2omi + x2li - g44) +
					d4422 * std::cos(x2li - g44) + d5421 * std::cos(xomi + x2li - g54) +
					d5433 * std::cos(-xomi + x2li - g54));
				xnddt = xnddt * xldot;
			}
		  ...
		  char opsmode)
{
	...
	if (opsmode == 'a')
	{
		...
		c1p2p = c1 + twopi;
		gsto  = std::fmod( thgr70 + c1*ds70 + c1p2p*tfrac + ts70*ts70*fk5r, twopi);
		if ( gsto < 0.0 )
			gsto = gsto + twopi;
	}
	else
	      ...
	      const double xnodeo,  elsetrec& satrec)
{
	...
	if ((omeosq >= 0.0 ) || ( satrec.no >= 0.0))
	{
		...
		satrec.cc4    = 2.0* satrec.no * coef1 * ao * omeosq *
				...
				(2.0 * etasq - eeta * (1.0 + etasq)) * std::cos(2.0 * satrec.argpo)));
		...
		satrec.delmo   = std::pow((1.0 + satrec.eta * std::cos(satrec.mo)), 3);
		satrec.sinmao  = std::sin(satrec.mo);
