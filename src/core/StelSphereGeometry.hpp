...
public:
	...
	virtual SphericalRegionType getType() const override {return SphericalRegion::Cap;}
	virtual OctahedronPolygon getOctahedronPolygon() const override;

	//! Get the area of the intersection of the halfspace on the sphere in steradian.
	virtual double getArea() const override {return 2.*M_PI*(1.-d);}

	//! Return true if the region is empty.
	virtual bool isEmpty() const override {return d>=1.;}

	//! Return a point located inside the SphericalCap.
	virtual Vec3d getPointInside() const override {return n;}

	//! Return itself.
	virtual SphericalCap getBoundingCap() const override {return *this;}

	// Contain and intersect	
	virtual bool contains(const Vec3d &v) const override {Q_ASSERT(d==0 || std::fabs(v.normSquared()-1.)<0.0000002);return (v*n>=d);}
	virtual bool contains(const Vec3f &v) const {Q_ASSERT(d==0 || std::fabs(v.normSquared()-1.f)<0.000002f);return (v.toVec3d()*n>=d);}
	virtual bool contains(const SphericalConvexPolygon& r) const override;
	virtual bool contains(const SphericalCap& h) const override
	{
		const double a = n*h.n-d*h.d;
		return d<=h.d && ( a>=1. || (a>=0. && a*a >= (1.-d*d)*(1.-h.d*h.d)));
	}
	virtual bool contains(const AllSkySphericalRegion&) const override {return d<=-1;}
	virtual bool intersects(const SphericalPolygon& r) const override;
	virtual bool intersects(const SphericalConvexPolygon& r) const override;
	...
	virtual bool intersects(const SphericalCap& h) const override
	{
		const double a = d*h.d - n*h.n;
		return d+h.d<=0. || a<=0. || (a<=1. && a*a <= (1.-d*d)*(1.-h.d*h.d));
	}
	virtual bool intersects(const AllSkySphericalRegion&) const override {return d<=1.;}
	...
	virtual QVariantList toQVariant() const override;

	virtual void serialize(QDataStream& out) const override {out << n << d;}
...
public:
	...
	virtual ~SphericalPoint() override {;}

	virtual SphericalRegionType getType() const override {return SphericalRegion::Point;}
	virtual OctahedronPolygon getOctahedronPolygon() const override;
	virtual double getArea() const override {return 0.;}
	virtual bool isEmpty() const override {return false;}
	virtual Vec3d getPointInside() const override {return n;}
	virtual SphericalCap getBoundingCap() const override {return SphericalCap(n, 1);}
	//! Serialize the region into a QVariant map matching the JSON format.
	//! The format is ["POINT", [ra, dec]], with ra dec in degree in ICRS frame.
	virtual QVariantList toQVariant() const override;
	virtual void serialize(QDataStream& out) const override {out << n;}

	// Contain and intersect
	virtual bool contains(const Vec3d& p) const override {return n==p;}
	virtual bool contains(const SphericalPolygon&) const override {return false;}
	virtual bool contains(const SphericalConvexPolygon&) const override {return false;}
	virtual bool contains(const SphericalCap&) const override {return false;}
	virtual bool contains(const SphericalPoint& r) const override {return n==r.n;}
	virtual bool contains(const AllSkySphericalRegion&) const override {return false;}
	virtual bool intersects(const SphericalPolygon&) const override;
	virtual bool intersects(const SphericalConvexPolygon&) const override;
	virtual bool intersects(const SphericalCap& r) const override {return r.contains(n);}
	virtual bool intersects(const SphericalPoint& r) const override {return n==r.n;}
	virtual bool intersects(const AllSkySphericalRegion&) const override {return true;}
...
{
public:
	virtual ~AllSkySphericalRegion() override {;}

	virtual SphericalRegionType getType() const override {return SphericalRegion::AllSky;}
	virtual OctahedronPolygon getOctahedronPolygon() const override {return OctahedronPolygon::getAllSkyOctahedronPolygon();}
	virtual double getArea() const override {return 4.*M_PI;}
	virtual bool isEmpty() const override {return false;}
	virtual Vec3d getPointInside() const override {return Vec3d(1,0,0);}
	virtual SphericalCap getBoundingCap() const override {return SphericalCap(Vec3d(1,0,0), -2);}
	//! Serialize the region into a QVariant map matching the JSON format.
	...
	// Contain and intersect
	virtual bool contains(const Vec3d&) const override {return true;}
	virtual bool contains(const SphericalPolygon&) const override {return true;}
	virtual bool contains(const SphericalConvexPolygon&) const override {return true;}
	virtual bool contains(const SphericalCap&) const override {return true;}
	virtual bool contains(const SphericalPoint&) const override {return true;}
	virtual bool contains(const AllSkySphericalRegion&) const override {return true;}
	virtual bool intersects(const SphericalPolygon&) const override {return true;}
	virtual bool intersects(const SphericalConvexPolygon&) const override {return true;}
	virtual bool intersects(const SphericalCap&) const override {return true;}
	virtual bool intersects(const SphericalPoint&) const override {return true;}
	virtual bool intersects(const AllSkySphericalRegion&) const override {return true;}
...
public:
	...
	virtual ~EmptySphericalRegion() override {;}

	virtual SphericalRegionType getType() const override {return SphericalRegion::Empty;}
	virtual OctahedronPolygon getOctahedronPolygon() const override {return OctahedronPolygon::getEmptyOctahedronPolygon();}
	virtual double getArea() const override {return 0.;}
	...
	// Contain and intersect
	virtual bool contains(const Vec3d&) const override {return false;}
	virtual bool contains(const SphericalPolygon&) const override {return false;}
	virtual bool contains(const SphericalConvexPolygon&) const override {return false;}
	virtual bool contains(const SphericalCap&) const override {return false;}
	virtual bool contains(const SphericalPoint&) const override {return false;}
	virtual bool contains(const AllSkySphericalRegion&) const override {return false;}
	virtual bool intersects(const SphericalPolygon&) const override {return false;}
	virtual bool intersects(const SphericalConvexPolygon&) const override {return false;}
	virtual bool intersects(const SphericalCap&) const override {return false;}
	virtual bool intersects(const SphericalPoint&) const override {return false;}
	virtual bool intersects(const AllSkySphericalRegion&) const override {return false;}
...
// These are too buggy in Qt6 to be useful.
	...
	virtual SphericalRegionType getType() const override {return SphericalRegion::Polygon;}
	virtual OctahedronPolygon getOctahedronPolygon() const override {return octahedronPolygon;}
	...
	virtual SphericalCap getBoundingCap() const override;

	virtual bool contains(const Vec3d& p) const override {return octahedronPolygon.contains(p);}
	virtual bool contains(const SphericalPolygon& r) const override {return octahedronPolygon.contains(r.octahedronPolygon);}
	virtual bool contains(const SphericalConvexPolygon& r) const override;
	virtual bool contains(const SphericalCap& r) const override {return octahedronPolygon.contains(r.getOctahedronPolygon());}
	virtual bool contains(const SphericalPoint& r) const override {return octahedronPolygon.contains(r.n);}
	virtual bool contains(const AllSkySphericalRegion& r) const override {return octahedronPolygon.contains(r.getOctahedronPolygon());}

// These are too buggy in Qt6 to be useful.
#if (QT_VERSION<QT_VERSION_CHECK(6,0,0))
	virtual bool intersects(const SphericalPolygon& r) const override {return octahedronPolygon.intersects(r.octahedronPolygon);}
	virtual bool intersects(const SphericalConvexPolygon& r) const override;
	virtual bool intersects(const SphericalCap& r) const override {return r.intersects(*this);}
	virtual bool intersects(const SphericalPoint& r) const override {return octahedronPolygon.contains(r.n);}
	virtual bool intersects(const AllSkySphericalRegion&) const override {return !isEmpty();}

	virtual SphericalRegionP getIntersection(const SphericalPoint& r) const override {return contains(r.n) ? SphericalRegionP(new SphericalPoint(r)) : EmptySphericalRegion::staticInstance;}
	virtual SphericalRegionP getIntersection(const AllSkySphericalRegion& ) const override {return SphericalRegionP(new SphericalPolygon(octahedronPolygon));}

	virtual SphericalRegionP getUnion(const SphericalPoint&) const override {return SphericalRegionP(new SphericalPolygon(octahedronPolygon));}
	virtual SphericalRegionP getUnion(const EmptySphericalRegion&) const override {return SphericalRegionP(new SphericalPolygon(octahedronPolygon));}
#endif

	virtual SphericalRegionP getSubtraction(const SphericalPoint&) const override {return SphericalRegionP(new SphericalPolygon(octahedronPolygon));}
	virtual SphericalRegionP getSubtraction(const EmptySphericalRegion&) const override {return SphericalRegionP(new SphericalPolygon(octahedronPolygon));}
...
public:
	...
	virtual SphericalRegionType getType() const override {return SphericalRegion::ConvexPolygon;}
	virtual OctahedronPolygon getOctahedronPolygon() const override {return OctahedronPolygon(contour);}
	virtual StelVertexArray getFillVertexArray(const Vec3d &observerVelocityForAberration) override;
	virtual StelVertexArray getFillVertexArray() const override;
	virtual StelVertexArray getOutlineVertexArray() const override;
	virtual StelVertexArray getOutlineVertexArray(Vec3d observerVelocityForAberration) override;
	//virtual StelVertexArray getOutlineVertexArray(Vec3d observerVelocityForAberration) const override;
	virtual double getArea() const override;
	virtual bool isEmpty() const override {return contour.isEmpty();}
	virtual Vec3d getPointInside() const override;
	virtual SphericalCap getBoundingCap() const override {return cachedBoundingCap;}
	...
	virtual QVariantList toQVariant() const override;
	virtual void serialize(QDataStream& out) const override {out << contour;}

	// Contain and intersect
	virtual bool contains(const Vec3d& p) const override;
	virtual bool contains(const SphericalPolygon& r) const override;
	virtual bool contains(const SphericalConvexPolygon& r) const override;
	virtual bool contains(const SphericalCap& r) const override;
	virtual bool contains(const SphericalPoint& r) const override {return contains(r.n);}
	virtual bool contains(const AllSkySphericalRegion&) const override {return false;}
	virtual bool intersects(const SphericalCap& r) const override {if (!cachedBoundingCap.intersects(r)) return false; return r.intersects(*this);}
	virtual bool intersects(const SphericalPolygon& r) const override;
	virtual bool intersects(const SphericalConvexPolygon& r) const override;
	virtual bool intersects(const SphericalPoint& r) const override {return contains(r.n);}
	virtual bool intersects(const AllSkySphericalRegion&) const override {return true;}
...
public:
	...
	virtual StelVertexArray getFillVertexArray() const override {Q_ASSERT(0); return StelVertexArray();}
	virtual StelVertexArray getFillVertexArray(const Vec3d &observerVelocityForAberration) override {Q_ASSERT(0); Q_UNUSED(observerVelocityForAberration) return StelVertexArray();}
	//! Serialize the region into a QVariant map matching the JSON format.
	//! The format is:
	...
	virtual QVariantList toQVariant() const override;
	virtual void serialize(QDataStream& out) const override {Q_UNUSED(out); Q_ASSERT(0);}
...
public:
	...
	virtual void serialize(QDataStream& out) const override {out << contour << textureCoords;}
