...
file \file{catalog.pack} to storing the binary catalog. After converting the data into binary format you should gzipped them by the command \command{gzip -nc catalog.pack > catalog.dat}.
...
\item[\textbf{ACO}]  A Catalog of Rich Clusters of Galaxies~\cite{1989ApJS...70....1A}\newFeature{V0.16.1}
\end{description}
...
A coordinates for each point of outline is represented in equatorial coordinate system for epoch J2000.0. The possible values of third ``column'' (\emph{Command}) is:
\begin{description}
\item[start] This command mark a start point of the outline. This command should contain designation of deep-sky object also.
\item[vertex] This command mark a middle point of the outline.
\item[end] This command mark an end point of the outline.
\end{description}
...
The format of the file \file{outlines.dat} is compatible with similar file of the \program{SkyChart/Cartes du Ciel} planetarium.
...
\begin{figure}[h]
\centering\includegraphics[width=\textwidth]{nebula-display}
