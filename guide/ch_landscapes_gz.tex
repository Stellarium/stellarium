...
clouds\footnote{You should have created control points with the
  \program{Celeste} option!} which have moved between photos. For handheld
...
\program{ImageMagick} (\program{IM})\footnote{\url{http://www.imagemagick.org/}} can be described as ``Swiss Army Knife of image
...
to include \program{IM} in your own \file{.BAT} scripts. We will use it to do our final
cut and resize operations. I cannot give an exhaustive tutorial about
more than a few of \program{IM}'s functions, but the commands given here should be
...
In the Hugin \menu{Photos} tab, select the \menu{Positions} view on
...
while ``attaching'' the lower photos. In \program{Hugin}'s \menu{Photos} tab,
