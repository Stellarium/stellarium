...
\newcommand{\guibutton[2]}[0.45]{\includegraphics[scale=#1]{#2}}
\begin{table}[h]
\centering
\begin{tabular}{c c l}\toprule
\emph{Button} & \emph{Shortcut key} & \emph{Description}\\\midrule
\guibutton[0.75]{bt_timerate_decrease} & \key{J} & Decrease the rate at which time passes \\
\guibutton[0.75]{bt_timerate_normal}   & \key{K} & Make time pass as normal \\
\guibutton[0.75]{bt_timerate_increase} & \key{L} & Increase the rate at which time passes \\
\guibutton[0.75]{bt_time_normal}       & \key{8} & Return to the current time \& date \\
\bottomrule
\end{tabular}
\caption{Time Travel}
\end{table}
...
speed button \guibutton{bt_timerate_increase}. 
...
\guibutton{bt_timerate_normal}.  The stars and/or the
Sun should stop scooting across the sky. Now press the decrease time
speed button \guibutton{bt_timerate_decrease} once. Look
...
\begin{figure}[htb]
\centering\includegraphics[width=0.9\textwidth]{002}
...
Constellations           & \guibutton[0.75]{bt_constellation}     & \key{C} & Draw constellations as ``stick figures'' \\
Constellation Names      & \guibutton[0.75]{bt_constellation_name}& \key{V} & Draw name of the constellations \\
Constellation Art        & \guibutton[0.75]{bt_constellation_art} & \key{R} & Superimpose artistic representations of the constellations \\
Equatorial Grid          & \guibutton[0.75]{bt_eq_grid}           & \key{E} & Draw grid lines for the RA/Dec coordinate system \\
Azimuth Grid             & \guibutton[0.75]{bt_az_grid}           & \key{Z} & Draw grid lines for the Alt/Azi coordinate system \\
Toggle Ground            & \guibutton[0.75]{bt_ground}            & \key{G} & Toggle drawing of the ground. Turn this off to see objects that are below the horizon. \\
Toggle Cardinal Points   & \guibutton[0.75]{bt_cardinal}          & \key{Q} & Toggle marking of the North, South, East and West points on the horizon. \\
Toggle Atmosphere        & \guibutton[0.75]{bt_atmosphere}        & \key{A} & Toggle atmospheric effects. Most notably makes the stars visible in the daytime.  \\
Deep-Sky Objects         & \guibutton[0.75]{bt_nebulae}           & \key{D} & Toggle marking the positions of Deep-Sky Objects. \\
Planet Hints             & \guibutton[0.75]{bt_planets}           & \key{P} & Toggle indicators to show the position of planets. \\
Coordinate System        & \guibutton[0.75]{bt_coord_type}        & \key{Enter} & Toggle between Alt/Azi \& RA/Dec coordinate systems. \\
Goto                     & \guibutton[0.75]{bt_goto}              & \key{Space} & Center the view on the selected object \\
Night Mode               & \guibutton[0.75]{bt_night_mode}        & \key{Ctrl+N} & Toggle ``night mode'', which changes the coloring of same display elements to be easier on the dark-adapted eye. \\
Nebula background images & \guibutton[0.75]{bt_DSS}        & [none] & Toggle ``nebula background images'', which turns the Textures on or off. \\
Full Screen Mode         & \guibutton[0.75]{bt_fullscreen} & \key{F11} & Toggle full screen mode. \\
Flip view (horizontal)   & \guibutton[0.75]{bt_fliph}      & \key{Ctrl+Shift+H} & Flip the image in the horizontal plane. Note this button is not enable by default. See section {[}sec:imageflipping{]} \\
Flip view (vertical)     & \guibutton[0.75]{bt_flipv}      & \key{Ctrl+Shift+V} & Flip the image in the vertical plane. Note this button is not enable by default. See section {[}sec:imageflipping{]} \\
Quit Stellarium          & \guibutton[0.75]{bt_quit}       & \key{Ctrl-Q} & Close Stellarium.\\
                         &                                 & \key{CMD-Q}  & on Mac OS X\\
Help Window              & \guibutton[0.5]{btd_help}       & \key{F1} & Show the help window, with key bindings and other useful information \\
Configuration Window     & \guibutton[0.5]{btd_config}     & \key{F2} & Show the configuration window \\ 
Search Window            & \guibutton[0.5]{btd_find}       & \key{F3} or \key{Ctrl+F} & Show the object search window \\
View Window              & \guibutton[0.5]{btd_view}       & \key{F4} & Show the view window \\
Time Window              & \guibutton[0.5]{btd_time}       & \key{F5} & Show the time window \\
Location Window          & \guibutton[0.5]{btd_location}   & \key{F6} & Show the observer location window (map) \\
\bottomrule
...
\key{Control-S}. Screenshots are taken in \file{.png} format, and
