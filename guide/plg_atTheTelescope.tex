...
\begin{figure}[bp]
\begin{center}
\VerbatimInput[fontsize=\scriptsize]{pictures/Stellarium_telescope_protocol.txt}
\end{center}
...
To use the plugin, first enable it by selecting the "Load at startup" checkbox under the "Plugins" tab of the main Configuration window.
Click the "configure" button to open the Mosaic Camera configuration window.
This will open the Mosaic Camera configuration window.  Within the "Cameras" tab, you can select from the available cameras, setting their RA, Dec, rotation, and visibility as desired.
...
\subsubsubsection{\textbf{LSSTCam}}
...
\subsubsubsection{\textbf{HyperSuprimeCam (HSC)}}
...
\subsubsubsection{\textbf{Dark Energy Camera (DECam)}}
...
\subsubsubsection{\textbf{MegaPrime}}
...
When the Mosaic Camera plugin is first enabled, it checks for the existence of a directory \file{<USERDATA>/modules/MosaicCamera/}.
...
If you ever need to reset the plugin to its default state, simply delete the \file{<USERDATA>/modules/MosaicCamera/} directory.
