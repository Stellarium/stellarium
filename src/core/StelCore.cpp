...
{
	const QMetaEnum& en = metaObject()->enumerator(metaObject()->indexOfEnumerator("ProjectionType"));
	QString s(getProjection(StelCore::FrameJ2000, false, (ProjectionType)en.keyToValue(key.toAscii()))->getNameI18());
	return s;
}
...
QString StelCore::projectionNameI18nToTypeKey(const QString& nameI18n) const
{
	...
	for (int i=0;i<en.keyCount();++i)
	{
		if (getProjection(StelCore::FrameJ2000, false, (ProjectionType)i)->getNameI18()==nameI18n)
