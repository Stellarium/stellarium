...
//Original == snprintf(vs, 999, "%sconfig.ini", app->getConfigDir().c_str());
#ifdef __MSVC__
	_snprintf(vs, 999, "%sconfig.ini", app->getConfigDir().c_str());
#else
...
/***** MSVC Stellarium Build *****/	
	...
	language_lb->addItemList(Translator::getAvailableLanguagesNamesNative(core->getLocaleDir()));
	language_lb->setOnChangeCallback(callback<void>(this, &StelUI::setAppLanguage));
	language_lb->setCurrent(app->getFullAppLanguage());
	...
	languageSky_lb->addItemList(Translator::getAvailableLanguagesNamesNative(core->getLocaleDir()));
	languageSky_lb->setOnChangeCallback(callback<void>(this, &StelUI::setSkyLanguage));
	languageSky_lb->setCurrent(core->getFullSkyLanguage());
