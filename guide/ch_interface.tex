...
The Main tab in the configuration window (Fig.~\ref{fig:gui:configuration:main}) provides control for
...
\noindent The Information tab (Fig.~\ref{fig:gui:configuration:info}) allows you to 
set the type and amount of information displayed (left column) or output by 
speech (right column; see \ref{sec:gui:configuration:speech}\newFeature{25.4}) about a 
selected object. 
The Radio buttons on the top of the dialog allow quick selection of some useful 
...
\label{sec:gui:configuration:extras:infoformat}
...
\item[Allow negative hour angles] Count hour angles $-12h\ldots12h$ instead of $0h\ldots24h$. \newFeature{25.4}
\item[Polar distance, not declination] Used by some telescopes. Using label $p$ or $PD$ instead of $\delta$. \newFeature{25.4}
\end{description}


\subsubsection{Customization of button visibility on bottom toolbar}
\label{sec:gui:configuration:extras:buttons}
...
\subsubsection{Toolbar position} \newFeature{26.3}
The \emph{Toolbar location} drop-down in the Planetarium options section of the Tools tab lets you
dock both toolbars to any of the four screen corners:
\begin{description}
\item[Bottom-left] Default position: tool button bar at the bottom, dialog button bar on the left.
\item[Bottom-right] Tool button bar at the bottom, dialog button bar on the right.
\item[Top-left] Tool button bar at the top, dialog button bar on the left. When the tool button bar is at the top,
  the status row (location, FOV, date/time) moves below the buttons so it remains visible when
  the bar is partially hidden.
\item[Top-right] Tool button bar at the top, dialog button bar on the right.
\end{description}
The selection is saved as \key{toolbar\_corner} (integer 0--3) in the \texttt{[gui]} section.
...
\subsection{The Speech Tab}
\label{sec:gui:configuration:speech}

\begin{figure}[htbp]
\centering\includegraphics[width=0.75\textwidth]{config_dialog_speech_tab.png}
\caption{Configuration Window: Speech Tab}
\label{fig:gui:configuration:speech}
\end{figure}

\noindent The Speech tab (Fig.~\ref{fig:gui:configuration:speech})\newFeature{25.4} allows configuring speech output.%
\footnote{This fully depends on capabilities of the operating system and is not available 
for builds/platforms with Qt version less than 6.4.} You can select some speech engine, 
then (hopefully) a list of voices for your user language becomes available from which you can choose. 
You can influence pitch and speed (Rate) and balance volume against your system volume. 
Then test the content of the text box with the \button{$\filledmedtriangleright$ Say}, or cut this off via \button{$\filledmedsquare$ Stop}.

Speech output is available for data of the currently selected object. 
In the Information tab (see \ref{sec:gui:configuration:info}), the right checkbox colums let you select 
those information bits which should be output by voice. Most users will keep coordinate readings off, 
but some physical or morphological data may be interesting to listen to.  

The default hotkey for speech output is \key{Shift+R}. Pressing it another time will cut off narration.

Also the long texts in the Landscape (see \ref{sec:gui:view:landscape}) and 
Skyculture (\ref{sec:gui:view:skyculture}) tabs of the View Settings dialog can be narrated 
with the \button{$\filledmedtriangleright$ Read text}/\button{$\filledmedsquare$ Stop} buttons 
near the respective text boxes.
...
\item[Scale] will increase the apparent size of the selected class of objects:
  ...
  \begin{description}
\item[Dynamic (FOV-based) Moon size] When this option is enabled, the Moon's \newFeature{26.1}
  display size is no longer fixed, but interpolates smoothly between $1\times$
  (natural size) and the scale factor set above, depending on the current field
  of view (FOV).
  \end{description}
...
\begin{figure}[p]
\centering\includegraphics[width=\textwidth]{view_dialog_skyculture_tab.png}
\caption{View Settings Window: Sky Culture Tab, Details and Appearance}
\label{fig:gui:view:skyculture}
\end{figure}

\begin{figure}[p]
\centering\includegraphics[width=\textwidth]{view_dialog_skyculture_map.png}
\caption{View Settings Window: Sky Culture Tab, Exploration and Selection}
\label{fig:gui:view:skyculture:map}
\end{figure}


The Sky Culture tab of the View window provides two sub-tabs\newFeature{26.1}:
\begin{description}
\item[Details and Appearance] (Fig.~\ref{fig:gui:view:skyculture}) provides textual information about the currently selected skyculture, 
and user interface elements to fine-tune the display of available data.
\item[Exploration and Selection] (Fig.~\ref{fig:gui:view:skyculture:map}) provides a list of available skycultures, sorted into blocks of geographic regions, 
together with a world map on which the available cultures are displayed\footnote{As of version 26.1 this is a new feature, and only Norse (Edda) so far has the polygons available.}. 
Clicking on a polygon will select the respective skyculture.
\end{description}

On the tab ``Details and Appearance'', you find numerous
controls to configure how the current culture's constellations and bright star names will be
...
%You can select from a choice of native, transliteration (as pronounciation aid), 
%scientific transliteration where such system exists, 
%phonetic annotation using the International Phonetic Alphabet (IPA), translation and augmentation by modern names.
%These items are stored immediately and per-skyculture, so that you can easily configure the program matching to your language skills.
%Not all data items may be available, and fallbacks will be used where applicable.
...
And, after the actual \emph{translation}, for reference, you may want to add the commonly used \emph{modern} name to an object like star or planet. 
...
\item[Abbreviated names] To provide a less cluttered screen in case constellation names are long, you can show abbreviations. 
The Modern skycultures use the 3-letter codes given by the IAU, for the other skycultures we depend on our skyculture authors 
to define unique short labels that help memorizing. Some skycultures still just show legacy numbers.
...
\item[Constellation art] Activate display of constellation
...
\item[Constellation areas] optional, advanced research feature. See page~\pageref{SC:constellation_area} for details. 
\item[Zodiac] If this skyculture has a concept of Zodiac (usually a region around the ecliptic split into 12 equal-sized parts), 
  you can enable it here. 
\item[Lunar Stations] If this skyculture has a concept of Lunar stations or mansions, you can enable them here. 
  For example, Indian skycultures observe 27 \emph{nakshatras} along the ecliptic, and Chinese skycultures have 28 Lunar 
  mansions of unequal width in equatorial coordinates defined by particular stars. 
\end{description}
...
Open the Search window (\guibutton[0.35]{1.5}{btd_find.png}, \key{F3}, or \key{Ctrl+F}).
...
\begin{figure}[htbp]
	\centering\includegraphics[width=0.75\textwidth]{astrocalc_dialog_positions_mp_tab.png}
...
\begin{figure}[p]
	\centering\includegraphics[width=0.75\textwidth,trim=0 15 0 15,clip]{astrocalc_dialog_ephemeris_tab.png}
	\caption{Astronomical Calculations (AstroCalc): Plot trace of planet}
	\label{fig:gui:AstroCalc:Ephemeris}
\end{figure}

\begin{figure}[p]
	\centering\includegraphics[width=0.75\textwidth,trim=0 15 0 15,clip]{astrocalc_dialog_ephemeris_extra.png}
	\caption{Astronomical Calculations (AstroCalc): Extra options for ephemeris}
	\label{fig:gui:AstroCalc:Ephemeris:Extra}
\end{figure}

\begin{figure}[p]
	\centering\includegraphics[width=0.75\textwidth,trim=0 15 0 15,clip]{astrocalc_dialog_ephemeris_analemma.png}
...
\begin{figure}[p]
	\centering\includegraphics[width=0.75\textwidth]{astrocalc_dialog_ephemeris_analemma_mars.png}
	\caption{Astronomical Calculations (AstroCalc): Analemma on Mars}
	\label{fig:gui:AstroCalc:Ephemeris:AnalemmaMars}
\end{figure}

\begin{figure}[p]
	\centering\includegraphics[width=0.75\textwidth]{astrocalc_dialog_ephemeris_asteroids.png}
	\caption{Astronomical Calculations (AstroCalc): Two asteroids nearby to one place}
	\label{fig:gui:AstroCalc:Ephemeris:Asteroids}
\end{figure}

\begin{figure}[p]
	\centering\includegraphics[width=0.95\textwidth]{astrocalc_dialog_ephemeris_sun_at_altitude.png}
	\caption{Astronomical Calculations (AstroCalc): Venus when Sun is at the end of civil twilight (-6°)}
	\label{fig:gui:AstroCalc:Ephemeris:SunAtAltitude}
\end{figure}

\begin{figure}[p]
	\centering\includegraphics[width=0.95\textwidth]{astrocalc_dialog_ephemeris_planet_opposition.png}
	\caption{Astronomical Calculations (AstroCalc): Points of Saturn's oppositions during a few decades}
	\label{fig:gui:AstroCalc:Ephemeris:PlanetOpposition}
\end{figure}
...
an object's position when the Sun is at a specific altitude relative to the horizon. Once selected from the list of time steps, 
the \guibutton[0.6]{0.1}{uibtTime.png}~\emph{Custom interval} window is enabled, and a preset solar altitude can be selected 
(only values $ \le 0^\circ$ are valid), and the ephemeris is computed accordingly. If the Sun never reaches the altitude selected,
during the specified interval, e.g. $ -12^\circ$ during Scandinavian summer solstice, no marker is drawn. 
(Fig.~\ref{fig:gui:AstroCalc:Ephemeris:SunAtAltitude}).
Using it as intended requires the ephemeris to be drawn in horizontal coordinates!

The other time step that requires some care is the \emph{Opposition of planet}. This is useful for displaying where
and when a planet will be at its opposition for a given year in relation to background stars. For it to make sense, 
the planet in question needs to be selected both in the ephemeris list and in the \emph{Custom interval} window 
Fig.~\ref{fig:gui:AstroCalc:Ephemeris:PlanetOpposition}.

It is only applicable for the outer planets, i.e. Mars through Neptune. The computation is quite intense, so showing 
e.g. Neptune oppositions throughout its 165 year orbit might take a while if you're on a weak system.
...
\begin{figure}[p]
\centering\includegraphics[width=0.75\textwidth]{astrocalc_dialog_graphs_tab_altvstime.jpg}
\caption{Astronomical Calculations (AstroCalc): Graphs / Altitude vs.\ Time}
\label{fig:gui:AstroCalc:Graphs:AltVsTime}
\end{figure}
\begin{figure}[p]
\centering\includegraphics[width=0.75\textwidth]{astrocalc_dialog_graphs_tab_azivstime.jpg}
\caption{Astronomical Calculations (AstroCalc): Graphs / Azimuth vs.\ Time}
\label{fig:gui:AstroCalc:Graphs:AziVsTime}
\end{figure}

\begin{figure}[p]
\centering\includegraphics[width=0.75\textwidth]{astrocalc_dialog_graphs_tab_me.jpg}
...
\begin{figure}[p]
\centering\includegraphics[width=0.75\textwidth]{astrocalc_dialog_graphs_tab_graphs.jpg}
\caption{Astronomical Calculations (AstroCalc): Graphs}
\label{fig:gui:AstroCalc:Graphs:Graphs}
\end{figure}
\begin{figure}[p]
\centering\includegraphics[width=0.75\textwidth]{astrocalc_dialog_graphs_tab_ad.jpg}
...
\end{figure}
\begin{figure}[p]
\centering\includegraphics[width=0.75\textwidth]{astrocalc_dialog_pc_graphs_tab.jpg}
\caption{Astronomical Calculations (AstroCalc): Planetary Calculator (PC), Graphs Tab}
\label{fig:gui:AstroCalc:PC:Graphs}
\end{figure}

\begin{figure}[p]
\centering\includegraphics[width=0.75\textwidth]{astrocalc_dialog_eclipses_ase.jpg}
...
\begin{figure}[htbp]
\centering\includegraphics[width=0.75\textwidth]{astrocalc_dialog_eclipses_lse.jpg}
...
\begin{figure}[tbp]
\centering\includegraphics[width=0.75\textwidth]{astrocalc_dialog_eclipses_le.jpg}
...
\begin{figure}[tbp]
\centering\includegraphics[width=0.75\textwidth]{astrocalc_dialog_transits.jpg}
...
The Almanac tool\newFeature{25.1} has one subtab: ``Specific Time''. 

\subsubsection{The ``Specific Time'' Subtab}
\label{sec:gui:AstroCalc:Almanac:SpecificTime}

\begin{figure}[tbp]
\centering\includegraphics[width=0.75\textwidth]{astrocalc_dialog_almanac_specifictime.jpg}
