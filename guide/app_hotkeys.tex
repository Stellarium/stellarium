...
\emph{WARNING: The list of hotkeys in this Guide uses notation for standard
PC-compatible keyboards. Not all keyboards are PC-compatible -- for example
Apple's computers use keyboards with their own specific keys -- \key{Option}
and \key{Command}. The \key{Option} key is equivalent to the \key{Alt} key on
the PC-compatible keyboards. The Haiku operating system is the successor of
BeOS, and it swaps behaviour for \key{\ctrl} and \key{Alt} keys -- e.g. PC's
\key{\ctrl+A} will be \key{Alt+A} in Haiku.}
