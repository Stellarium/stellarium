...
double SolarSystem::draw(Projector * prj, /*const*/ Navigator * nav, const ToneReproductor* eye, bool flag_point)
{
  ...
  // kornyakov: keep distance2selected actual
  vector<Planet*>::iterator iter;
  iter = system_planets.begin();
  while (iter != system_planets.end())
  {
    (*iter)->compute_distance(obs_helio_pos);
    if ((*iter)->getEnglishName() == selected.getEnglishName())
    { 
      nav->distance2selected = (*iter)->get_distance();
    }
    ++iter;
  }
  ...
  if (Planet::getflagShow())//kornyakov: bug fix or not??
  {
    iter = system_planets.begin();
    while (iter != system_planets.end())
    {
  ...
  // This effect currently only looks right from earth viewpoint
