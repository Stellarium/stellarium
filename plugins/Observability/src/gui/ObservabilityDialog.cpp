...
void ObservabilityDialog::setAboutHtml(void)
{
	...
	html += "<tr><td>" + q_("Largest Sun separation:") + "</td><td>Happens when the angular separation between the Sun and the celestial object are maximum. This is equivalent to say that the Equatorial longitudes of the Sun and the object differ by 180 degrees. When an object is at its maximum possible angular separation from the Sun (no matter if it is a planet or a star), it culminates roughly at midnight, and on the darkest possible area of the Sky at that declination. Hence, that is the 'best' night to observe a particular object.<br></td></tr>";
