...
void CLIProcessor::parseCLIArgsPreConfig(const QStringList& argList)
{
	...
	if (argsGetOption(argList, "-h", "--help"))
	{
		...
		std::cout << "Usage:\n"
			  ...
			  << "--opengldetails (or -d) : dump information about OpenGL support to logfile\n"
...
void CLIProcessor::parseCLIArgsPostConfig(const QStringList& argList, QSettings* confSettings)
{
	...
	try
	{
		bool dumpOpenGLDetails = argsGetOption(argList, "-d", "--opengldetails");
		qApp->setProperty("dump_OpenGL_diagnostics", dumpOpenGLDetails);
