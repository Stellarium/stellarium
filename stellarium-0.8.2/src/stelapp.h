...
//Screen resolution in case we use offscreen rendering
//In that case the screen is just to show user what we are processing
#define W_SCREEN 800 
#define H_SCREEN 800
...
public:
	...
	StelApp(const string& CDIR, const string& LDIR, const string& DATA_ROOT, 
		bool _CreateAviAndExit = false, bool _CreateAviOnFlyAndExit = false, 
    std::string ScriptToRunOnStartUp = "startup.sts", 
		int _resultVideoWidth = -1, int _resultVideoHeight = -1, std::string _videoOutputPath = "", 
  ...
  ~StelApp();
	...
	wstring getFullAppLanguage() { return Translator::globalTranslator.iso639_1LanguageCodeToNativeName(Translator::globalTranslator.getTrueLocaleName()); }
...
private:
	...
	// Script to create avi related
	string SelectedScriptForAvi;
	string SelectedScriptForAviDirectory;

	// Script to edit in the notepad
	string SelectedScriptForEditing;
	string SelectedScriptForEditingDirectory;
	...
	//True if the Stellarium must exit after saving the startup script
	bool CreateAviAndExit;
  bool CreateAviOnFlyAndExit;

	//The script which will be executed first
	std::string ScriptToRunOnStartUp;

	//Output video resolution
	int resultVideoWidth;
	int resultVideoHeight;

	//Output video full path
	std::string videoOutputPath;
	
	//Codec of output video
	std::string videoCodec;

	//Variables used for creating pbuffer
	HPBUFFERARB hbuffer;
	HDC hpbufdc;
	HGLRC pbufglctx;
	HDC ScreenDC;
	...
	//OpenCV class which is able to write video stream
	CvVideoWriter* VideoWriter;

	//Video output filepath
	string VideoOutput;

	//Video codec 4-characters string
	string VideoCodec;

  //kornyakov: fields for onfly image splitting
  HANDLE hReadEvent;
  HANDLE hWriteEvent;
  HANDLE hQuitEvent;
  HANDLE  hMapping;
  unsigned char * onFlyPixels; 
};
