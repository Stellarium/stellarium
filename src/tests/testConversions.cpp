...
void TestConversions::testHMSToRad()
{
	...
	while (data.count() >= 4)
	{
		...
		QVERIFY2(::abs(StelUtils::hmsToRad(h, m, s)-rad)<=ERROR_LIMIT, qPrintable(QString("%1h%2m%3s=%4").arg(h).arg(m).arg(s).arg(rad)));
	}
}
...
void TestConversions::testDMSToRad()
{
	...
	while (data.count() >= 4)
	{
		...
		rad = data.takeFirst().toDouble();
		QVERIFY2(::abs(StelUtils::dmsToRad(deg, min, sec)-rad)<=ERROR_LIMIT, qPrintable(QString("%1d%2m%3s=%4").arg(deg).arg(min).arg(sec).arg(rad)));
	}
}
...
void TestConversions::testRadToHMS()
{
	...
	while (data.count()>=4)
	{
		...
		t2 = so+mo*60+ho*3600;
		QVERIFY2(::abs(t1-t2)<=0.1, qPrintable(QString("%1rad=%2h%3m%4s").arg(rad).arg(ho).arg(mo).arg(so)));
	}
}
...
void TestConversions::testRadToDMS()
{
	...
	while (data.count()>=4)
	{
		...
		if (!sign)
		{
			...
			s = "+";
		QVERIFY2(::abs(angle1-angle2)<=ERROR_LIMIT, qPrintable(QString("%1rad=%2%3d%4m%5s").arg(rad).arg(s).arg(dego).arg(mino).arg(seco)));
	}
}
