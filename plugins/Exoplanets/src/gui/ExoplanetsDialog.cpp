...
void ExoplanetsDialog::setInfoHtml(void)
{
	...
	html += QString("<p><b>%1</b> &mdash; %2</p>").arg(q_("Mean Surface Temperature")).arg(q_("Temperature in (°C) based on a similar terrestrial atmosphere to planet mass ratio and a greenhouse effect due to 1% CO<sub>2</sub> (assuming an albedo of 0.3 in all cases)."));
