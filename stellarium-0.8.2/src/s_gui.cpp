...
void ListBox::setCurrent(const wstring& ws)
{
	...
	int i = 0; //firstItemIndex;
	//vector<LabeledButton*>::iterator iter = itemBt.begin();

	vector<wstring>::iterator iter = items.begin();
...
Picture::Picture(const s_texture * _imageTex,
	...
	imageTex(_imageTex), imgcolor(s_color(1,1,1,0))
{
	setPos(xpos, ypos);
	setSize(xsize, ysize);
	setTexture(imageTex);
...
#define CITY_SIZE 5
#define CITY_SELECT s_color(1,0,0,0)
#define CITY_HOVER s_color(1,1,0,0)
#define CITY_WITH_NAME s_color(1,.4,0,0)
#define CITY_WITHOUT_NAME s_color(.8,.6,0,0)
