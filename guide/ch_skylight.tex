...
based on the CIE clear-sky model by Kittler and Darula
\cite{Kittler-Darula:199x}, and models chromaticity distribution with
...
One setting allows to compute
$T$ from $k$ with the above relation.  However, please note that the
parameters are far from intuitive, therefore we added two sets of
reset functions. One sets the values to those in the original paper,
the other re-establishes Stellarium's sky colors from version 0.21.3
and earlier, which however seem only suitable for $T=5$.
...
Tonemapping is the process which attempts to compress the brightness
values found in nature into the range of brightness values which can
be achieved on a display device so that it looks reasonably natural
...
Parameters to tweak the tonemapping can be reached in the view
settings dialog after pressing \button{Tonemapping\ldots}.
...
\item[Use extra Gamma term] The original implementation of tonemapping
