...
The two checkboxes under \emph{Toolbar position} let you move the toolbars
(both the horizontal button bar and the vertical window buttons) to any of the four corners of the screen:
\begin{description}
\item[Toolbar at top] When enabled, the horizontal button bar moves to the top edge of the screen instead of the bottom. The status text row (location, FOV, date/time) is placed at the bottom of the bar so it remains visible when the bar is partially hidden.
\item[Toolbar at right] When enabled, the vertical window-button bar moves to the right edge of the screen instead of the left. The horizontal bar follows accordingly.
\end{description}
Both settings are saved as \key{toolbar\_at\_top} and \key{toolbar\_at\_right} in the \texttt{[gui]} section.
