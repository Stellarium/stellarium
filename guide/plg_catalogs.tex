...
enable\_at\_startup     & bool & Enable displaying meteor showers at startup plugin \\%\midrule
...
\begin{figure}[htbp]
  \centering\includegraphics[width=0.8\textwidth]{navstars_plugin_settings.jpg}
...
\begin{figure}[htbp]
  \centering\includegraphics[width=0.8\textwidth]{navstars_plugin_today.jpg}
...
\item[I.D. (The International Designator)] In the \emph{Satellite
  ...
  International Designator also known as \indexterm[COSPAR ID (satellite ID)]{COSPAR ID},
...
\item[Standard magnitude] The \indexterm[standard magnitude
  ...
  a mean value derived from visual observations (see \ref{sec:plugins:Satellites:VisualMagnitude}).
\item[RCS] The \indexterm[RCS, Radar Cross Section]{Radar Cross
  ...
  square meters (see \ref{sec:plugins:Satellites:VisualMagnitude}).
...
\begin{figure}[tb]
	\centering\includegraphics[width=0.6\textwidth]{satellites_plugin_comms.jpg}
...
You can edit communication data for individual satellites by pressing \guibutton{0.6}{bt_comms_sat.png}
...
format, in a file named \file{satellites.json}. A default copy is embedded in the
...
\item[rcs] optional parameter, containing the satellite's  \indexterm[RCS, Radar Cross Section]{Radar Cross
	Section}. The units of the RCS are square meters.
\end{description}
...
To calculate an approximated visual magnitude of satellites we use the 
...
\begin{figure}[tbp]
	\centering\includegraphics[width=0.8\textwidth]{satellites_plugin_sources.jpg}
...
\section{Missing Stars Plugin}
\label{sec:plugins:MissingStars}


Over the years some users have reported that some stars, usually telescopic targets only, are missing from Stellarium's star catalogs. 
This can be hardly avoided given selection criteria of catalogs (see Appendix~\ref{ch:Catalogues}). There are doubtlessly many more stars missing. 
If you need to compare your own photographs or observations and check for stars that may be missing from Stellarium's catalog, 
it is best to switch on the DSS layer (see section~\ref{sec:TOAST}). 

This plugin \newFeature{23.3} is enabled by default and has no UI button. 
If enabled (see section~\ref{sec:Plugins:EnablingPlugins}), it just draws the stars from its add-on catalog.

\iffalse
% GZ: I prepared this section for when this may become active one day.
\subsection{Missing Stars Catalog}
\label{sec:plugins:MissingStars:catalog}

The catalog of missing stars is stored on disk in JSON\footnote{\url{https://www.json.org/}}
format. A default copy is embedded in the plug-in at compile time. 
A working copy can be used in the user data directory, in a file named \file{modules/MissingStars/MissingStars.json}

\begin{configfileScr}
{
	"shortName": "Missing stars data",
	"catalog":
	{
		"TYC 1309-1609-1":
		{
			"RA": "05h35m04.9551206928s",
			"DEC": "+22d09m26.060109732s",
			"pmRA": 1.595,
			"pmDEC": -1.389,
                        "bMag": 11.5,
                        "vMag": 11.3,
