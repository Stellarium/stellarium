...
double FontSizeCardinalPoints = 30.;
double FontSizeSolarSystem = 14.;
double FontSizeGeneral = 12.;
double FontSizeConstellations = 16.;
        ...
        deltaAlt(0.), deltaAz(0.), move_speed(0.00025), firstTime(1)
{
    ...
    navigation = new Navigator(observatory, projection);
...
{
    FontSizeCardinalPoints = conf.get_int("gui:flag_font_size_cardinal_points");
    FontSizeSolarSystem = conf.get_int("gui:flag_font_size_solar_system");
    FontSizeGeneral = conf.get_int("gui:flag_font_size_general");
    FontSizeConstellations = conf.get_int("gui:flag_font_size_constellations");
    ...
    setFlagApplyDegamma(conf.get_boolean("video:flag_apply_degamma"));
    ...
    setStarMaxRadius(conf.get_double ("stars:star_max_radius"));
    ...
    setFlagHyperStars(conf.get_boolean("stars:flag_hyper_stars"));
    ...
    navigation->set_local_vision(InitViewPos);
	  ...
	  setCenterFovRatio(conf.get_double("navigation", "init_center_fov_ratio", 0.0));
    ...
    setNebulaFadeDurationCoefficient(conf.get_double("viewing", "nebula_fade_coefficient", 1.1f));
...
double StelCore::draw(int delta_time)
{
    ...
    // Draw the nebula
    //if (NebulaMgr::getFlagShow()) //kornyakov: bug fix or not?? 
...
bool StelCore::selectObject(const string &type, const string &id)
{
    ...
    */
    ...
    else if (type=="planet")
    {
        ...
        //kornyakov: store some information for flying
        navigation->radiusOfSelected = ssystem->getSelectedRadius();
        navigation->nameOfSelected = ssystem->getSelected().getEnglishName();
    }
    else if (type=="nebula")
    {
...
void StelCore::autoZoomIn(float move_duration, bool allow_manual_zoom)
{
    ...
    if (!navigation->get_flag_tracking())
    {
        navigation->set_flag_tracking(true);
...
void StelCore::autoZoomOut(float move_duration, bool full)
{
    ...
    navigation->set_flag_tracking(false);
...
void StelCore::setProjectionType(const string& sptype)
{
    ...
    delete projection;
    projection = ptemp;
    navigation->set_projector(projection);
    glFrontFace(projection->needGlFrontFaceCW()?GL_CW:GL_CCW);
}
...
void StelCore::setFlagTracking(bool b)
{
    ...
    else
    {
        navigation->move_to(selected_object.get_earth_equ_pos(navigation),
                            getAutomoveDuration());
        navigation->set_flag_tracking(1);
    }
...
void StelCore::setFlagApplyDegamma(bool b)
{
    hip_stars->setFlagApplyDegamma(b);
}
bool StelCore::getFlagApplyDegamma(void) const
{
    return hip_stars->getFlagApplyDegamma();
}

void StelCore::setFlagHyperStars(bool b)
{
    hip_stars->setFlagHyperStars(b);
}
bool StelCore::getFlagHyperStars(void) const
{
    return hip_stars->getFlagHyperStars();
}
...
void StelCore::setStarMaxRadius(float f)
{
    hip_stars->setStarMaxRadius(f);
}
float StelCore::getStarMaxRadius(void) const
{
    return hip_stars->getStarMaxRadius();
}
