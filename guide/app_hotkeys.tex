...
\subsection{Special local keys}
\label{ch:Hotkeys:Special}
All these hotkeys available to run when specific window or tab is opened.
\begin{center}
\begin{tabular}{rl} 
\toprule
\emph{Shortcut key}	& \emph{Description}\\\midrule
  		        & \textbf{Script console} \\
\key{\ctrl+O}		& Load script from file \\
\key{\ctrl+S}		& Save script to file \\
\key{\ctrl+Return}	& Run script \\\midrule
  		        & \textbf{Astronomical calculations} \\
