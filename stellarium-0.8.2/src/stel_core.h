...
// default font sizes
extern double FontSizeCardinalPoints;
extern double FontSizeSolarSystem;
extern double FontSizeGeneral;
extern double FontSizeConstellations;
...
public:
	...
	std::wstring getFullSkyLanguage()
	{
		return skyTranslator.iso639_1LanguageCodeToNativeName(skyTranslator.getTrueLocaleName());
	}
	...
	//! Get object tracking
	bool getFlagTracking(void) {return navigation->get_flag_tracking();}
  ...
  void gotoSelectedObject(void) {
    ...
    void unSelect(void) {
      ...
      // todo: create method of navigator and delete field with defaultAngle
      navigation->distance2selected = .0f;
      navigation->currentTranslation = .0f;
      navigation->radiusOfSelected = .0f;
      navigation->desiredAngleOfViewSelected = navigation->defaultAngleOfView;
      navigation->currentAngleOfViewSelected = navigation->defaultAngleOfView;
      navigation->initialAngleOfViewSelected = navigation->defaultAngleOfView;
    }
	...
	//! True if once selected constellations won't disapear
	//! If isolateSelected == false this flag is ignored	
	void setFlagStickyContellationSelection(bool b) {asterisms->setFlagStickyContellationSelection(b);};
	bool getFlagStickyContellationSelection(void) {return asterisms->getFlagStickyContellationSelection();};   
	...
	void setFlagApplyDegamma(bool b);
	bool getFlagApplyDegamma(void) const;
	
	void setFlagHyperStars(bool b);
	bool getFlagHyperStars(void) const;
  ...
  //! Set stars maximum radius
	void setStarMaxRadius(float f);
	//! Get stars maximum radius
	float getStarMaxRadius(void) const;
  ...
  void setFlagBlackPlanets(bool b) {ssystem->setFlagBlackPlanets(b);}
  bool getFlagBlackPlanets(void) const {return ssystem->getFlagBlackPlanets();}
	...
	void setFlagStopDayMotion(bool b) {ssystem->setFlagStopDayMotion(b);};
	bool getFlagStopDayMotion(void) {return ssystem->getFlagStopDayMotion();};

	//Option to have the centered object at specific angle, not exactly in the center
	void setCenterFovRatio(float cfr) {navigation->set_center_fov_ratio(cfr);};
	float getCenterFovRatio(void) {return navigation->get_center_fov_ratio();}
  ...
  //kornyakov: the following methods are for fade duration
  //! Set Nebulae Hints circle scale
  void setNebulaFadeDurationCoefficient(float f) {nebulas->setNebulaFadeDurationCoefficient(f);}
  ...
  // kornyakov: flying to the selected object
  void setDesiredAngleOfViewSelectedObject(float desiredAngle) 
  {
    navigation->initialAngleOfViewSelected = navigation->currentAngleOfViewSelected;
    navigation->desiredAngleOfViewSelected = desiredAngle;    
  }
  void setFlyingSteps(int numberOfFlyingSteps) {navigation->flyingSteps = numberOfFlyingSteps;}
