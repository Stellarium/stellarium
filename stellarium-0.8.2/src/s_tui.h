...
namespace s_tui
{
    ...
    {
    public:
		MultiSet_item(const wstring& _label = wstring()) : CallbackComponent(), label(_label)
		{
			current = items.end();
		}
...
//ORIGINAL == if(current==items.end()) current = items.begin();
#ifdef __MSVC__
		  if(items.size() == 1)
		  {
			current = items.begin();
		  ...
		  {
			  typename multiset<T>::iterator endIter;
			  endIter = items.end();
			  if(current == endIter) current = items.begin();
		  }
#else
...
/***** MSVC Stellarium Build *****/
		...
		bool setValue(const T& i)
		{
			...
			{
				current = items.find(i); return true;
			}
		}

		wstring getLabel(void) const
		{
			return label;
		}
		...
		void setLabel(const wstring& _label)
		{
			label = _label;
		}
