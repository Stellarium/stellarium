...
int StelCommandInterface::execute_command(string commandline, unsigned long int &wait, bool trusted) {
  ...
  if(command == "flag") {
    ...
    else if(args["planets_fading_coefficient"]!="") stcore->setPlanetsFadingCoefficient(str_to_double(args["planets_fading_coefficient"]));
    ...
    else if(args["star_max_radius"]!="") stcore->setStarMaxRadius(str_to_double(args["star_max_radius"]));
    ...
    else if((args["nebula_fade_coefficient"]!=""))
    {
      float cf = str_to_double(args["nebula_fade_coefficient"]);
      stcore->setNebulaFadeDurationCoefficient(cf);
    }
    ...
    else if (args["center_fov_ratio"]!="")
    {
      stcore->setCenterFovRatio(str_to_double(args["center_fov_ratio"]));
    }
    // kornyakov: flying to the selected object
    // script command for displacement along z-axis (line of vision)
    else if (args["desired_angle_of_view_selected_object"]!="")
    {
      stcore->setDesiredAngleOfViewSelectedObject(str_to_double(args["desired_angle_of_view_selected_object"]));
    }
    else if (args["number_of_flying_steps"]!="")
    {
      stcore->setFlyingSteps(str_to_double(args["number_of_flying_steps"]));
    }
    else status = 0;
    ...
    else 
    {
      if(args["action"]=="load" && args["filename"]!="") 
      {
        ...
        if(args["coordinate_system"] == "horizontal") 
          img_pos = Image::POS_HORIZONTAL;
        else if(args["coordinate_system"] == "equatorial") 
          img_pos = Image::POS_EQUATORIAL;
        else if(args["coordinate_system"] == "j2000") 
          img_pos = Image::POS_J2000;
        ...
        if(status==0) 
          debug_message = _("Unable to open file: ") + StelUtility::stringToWstring(image_filename);
      }
    ...
    execute_command("set auto_move_duration 0");
    ...
    //start rolling the Earth
    execute_command("flag stop_day_motion off");
    execute_command("set center_fov_ratio 0.0");
    execute_command("set auto_move_duration 10");
...
int StelCommandInterface::set_flag(string name, string value, bool &newval, bool trusted) {
  ...
  if(value == "toggle") {
    ...
    else if(name=="sticky_constellation_selection") {
      newval = !stcore->getFlagStickyContellationSelection();
      stcore->setFlagStickyContellationSelection(newval);
    }
    ...
    else if(name=="apply_degamma") {
      newval = !stcore->getFlagApplyDegamma();
      stcore->setFlagApplyDegamma(newval);
    }
    else if(name=="hyper_stars") {
      newval = !stcore->getFlagHyperStars();
      stcore->setFlagHyperStars(newval);
    }
    ...
    */
    ...
    else if(name=="black_planets") {
      newval = !stcore->getFlagBlackPlanets();
      stcore->setFlagBlackPlanets(newval);
      if (stcore->getFlagBlackPlanets()) stcore->setFlagPlanetsHints(false);
    }
    ...
    else if(name=="stop_day_motion")
    {
      newval = !stcore->getFlagStopDayMotion();
      stcore->setFlagStopDayMotion(newval);
    }
    ...
    else if(name=="sticky_constellation_selection") stcore->setFlagStickyContellationSelection(newval);
    else if(name=="star_twinkle") stcore->setFlagStarTwinkle(newval);
    else if(name=="point_star") stcore->setFlagPointStar(newval);
    else if(name=="apply_degamma") stcore->setFlagApplyDegamma(newval);
    else if(name=="hyper_stars") stcore->setFlagHyperStars(newval);
    ...
    */
    ...
    else if(name=="stop_day_motion") stcore->setFlagStopDayMotion(newval);
    else return(status);
