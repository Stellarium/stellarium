...
\noindent\newFeature{v0.16.1}Sometimes there is a need to extract some part of constellation in the own figure or merge parts from different constellations in one big figure --- e.g. ``Big Dipper'' for Ursa Major or ``Summer Triangle'' for constellations Cygnus, Lyra and Aquila. Those features names as ``asterisms'' as rule. The asterismss are listed in
...
The stick figures of asterisms are similar to constellation figures and coded in \file{asterismship.fab}. Lines
look like:
...
\item[pairs] is the number of line pairs which follow.
\item[pairN\_starA] Hipparcos numbers for the stars which form the
  asterism stick figure (similar to constellations). 
\end{description}
