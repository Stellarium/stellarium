...
The Satellite plugin could predict Iridium flares for the current
location up to 2 weeks in the future from the current moment. The
calculations of visual magnitude based on standard magnitude,
orientation of satellite and places an observer and Sun. Of course all
predictions could be saved in the CSV file.
%(Figure~\ref{fig:plugins:Satellites:IridiumFlares}).

The generation of satellites with highly reflective antennae which
caused predictable flares have been de-orbited by early 2020 and
replaced by models with different geometry which do not cause these
bright spectacles. A few defunct satellites are still in orbit, but
their orientation is no longer predictable, so that any remaining
results in the ``Iridium flares'' dialog will likely be not usable,
and the function will soon be removed.
