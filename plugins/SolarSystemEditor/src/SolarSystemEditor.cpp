...
SsoElements SolarSystemEditor::readMpcOneLineCometElements(const QString &oneLineElements) const
{
	...
	*/
	...
	if (eccentricity < 1.0)
	{
		...
		result.insert("orbit_good", qMin(1000, static_cast<int>(std::floor(0.5*period)))); // validity for elliptical osculating elements, days. Goes from aphel to next aphel or max 1000 days.
	}
	else
...
SsoElements SolarSystemEditor::readMpcOneLineMinorPlanetElements(const QString &oneLineElements) const
{
	...
	if (!column.isEmpty())
	{
		if (minorPlanetNumber)
		{
			...
			static const QRegularExpression asteroidDate("\\d+");
