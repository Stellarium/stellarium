...
void ScriptMgr::record_script(string script_filename) {
	...
	if(script_filename != "") {
		...
		//Save the script to the data dir
		sdir = DataDir + string("scripts/");
#else
...
/***** MSVC Stellarium Build *****/
	...
	string tmpDirSch = directory + "/*.*";
	//MessageBox(NULL, tmpDirSch.c_str(), "Debug", MB_OK);
	hFind = FindFirstFile(tmpDirSch.c_str(), &FindFileData);
	...
	else
	{
		...
		do
		{
			...
			int q  = FindFileData.dwFileAttributes & FILE_ATTRIBUTE_READONLY;

			if(tmp.length()>4 && tmp.find(".sts", tmp.length()-4) != string::npos && ((FindFileData.dwFileAttributes & FILE_ATTRIBUTE_READONLY) == 0)) 
			{
				result += tmp + "\n";
			}
		} while(FindNextResult = FindNextFile(hFind, &FindFileData));
		FindClose(hFind);
	}
#else
...
bool ScriptMgr::play_startup_script(string script_name, bool fullPath) {

	if (fullPath && 
		(script_name.find_last_of('\\') != -1 || script_name.find_last_of('/') != -1))
	{
		string scriptPath;
		int lastSlashIndex = script_name.find_last_of('\\');
		if (lastSlashIndex == -1)
		{
	...
	if(RemoveableScriptDirectory !="" &&
	   play_script(RemoveableScriptDirectory + string("scripts/") + script_name, 
	   RemoveableScriptDirectory + "scripts/")) {
		return 1;
	} else {
		// try in stellarium tree
		return play_script(DataDir + string("scripts/") + script_name, DataDir + "scripts/");
	} 
