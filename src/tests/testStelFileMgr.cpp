...
void TestStelFileMgr::testConfigMigration() {
	auto tmpDir = QDir(QDir::tempPath());
	auto dataDirPath = tmpDir.absoluteFilePath("data");
	auto configDirPath = tmpDir.absoluteFilePath("config");
	auto legacyDirPath = tmpDir.absoluteFilePath("legacy");

	tmpDir.mkpath("data");
	tmpDir.mkpath("config");
	tmpDir.mkpath("legacy");

	// Add dummy files
	auto dataDir = QDir(dataDirPath);
	...
	options.dataDirPath = dataDirPath;
	options.configDirPath = configDirPath;
	options._test_legacyUserPath = legacyDirPath;
};