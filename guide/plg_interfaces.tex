...
\section{Time Navigator Plugin}
\label{sec:plugins:TimeNavigator}
 
\begin{figure}[th]\centering
\includegraphics[height=74mm]{TimeNavigatorplugin_tab1.jpg}
%\hfill
\includegraphics[height=74mm]{TimeNavigatorplugin_tab2.jpg}
%\hfill
\includegraphics[height=74mm]{TimeNavigatorplugin_tab3.jpg}

\caption{Interface of Time Navigator plugin: Time Steps tab (left), Specific Time tab (center), and Planetary Events (right)}
\label{fig:TimeNavigator}
\end{figure}
 
\noindent The Time Navigator plugin brings a convenient GUI to many different
astronomical time steps and allows addition or subtraction of one (1)
unit of any of these periods interactively.
If enabled (see section~\ref{sec:Plugins:EnablingPlugins}), click on
the plugin button \guibutton{0.6}{bt_TimeNavigator_Off.png} on the
bottom toolbar to open the plugin window.
 
The first tab \menu{Time Steps} shows step buttons for different
definitions of days, months and years, and even longer cycles such as
the Saros. The second tab \menu{Specific Time} shows buttons to jump
to the previous, current or next event of the selected object, e.g.\
its meridian transit, as well as buttons for specific twilight moments.


%\newpage

The third tab \menu{Planetary Events} shows step buttons for some interesting
planetary events, such as oppositions for outer planets and greatest elongations
for inner planets. For the Moon, the four phases (new, first quarter, full, last quarter)
and times for apogee and perigee are shown, as well as times for Earth aphelion 
and perihelion. Clicking any of these buttons sets the time to that of the event.

%\begin{figure}[th]\centering
%\includegraphics[width=.6\textwidth]{TimeNavigatorplugin_tab3.jpg}
%\caption{The Planetary Events tab}
%\label{fig:TimeNavigator:PEvents}
%\end{figure}
 
\newpage

\section{Object Visibility Plugin}
\label{sec:plugins:ObjectVisibility}

\noindent The Object Visibility plugin provides map-based tools which help to
answer the question where selected objects can be seen from, and how
the solar twilight zones are distributed on Earth or on other supported
Solar System bodies. If enabled (see
section~\ref{sec:Plugins:EnablingPlugins}), click on the plugin button
\guibutton[0.09]{0.6}{bt_object_visibility_off.png} on the bottom
toolbar to open the plugin window.

\begin{figure}[th]\centering
\includegraphics[width=.82\textwidth]{object_visibility_visibility.png}
\caption{The Visibility tab of the Object Visibility plugin.}
\label{fig:ObjectVisibility:Visibility}
\end{figure}

\noindent The first tab, \menu{Visibility}, works with the currently selected
star or deep-sky object. Press \button{Calculate} to draw the visibility
limits for the selected object on a world map. The solid curve shows
where the object just reaches the horizon, while the dashed curve marks
where it reaches the user-defined
``good visibility'' altitude. Additional symbols indicate where the
object can pass through the zenith and where it becomes circumpolar in
the northern or southern hemisphere.

The map marker follows the observer's current location. Activate
\menu{Set location by map click} to move the observer by clicking on
the map. The \menu{Sync maps} option keeps pan and zoom synchronised
between all map tabs. The place labels can be enabled with a minimum
population threshold, and can optionally be restricted to labels near
the plotted lines.

\begin{figure}[th]\centering
\includegraphics[width=.82\textwidth]{object_visibility_twilight_limits.png}
\caption{The Twilight limits tab of the Object Visibility plugin.}
\label{fig:ObjectVisibility:TwilightLimits}
\end{figure}

The second tab, \menu{Twilight limits}, is Earth-only. It shows the
latitude limits related to the solstices for the current epoch,
including the equator, tropics, polar circles, and the limits at which
the Sun's lowest or highest altitude is $-6\degree$, $-12\degree$ or
$-18\degree$. These correspond to the conventional limits for civil,
nautical and astronomical twilight. The calculation follows the
obliquity valid for the current simulation date.

\begin{figure}[th]\centering
\includegraphics[width=.82\textwidth]{object_visibility_twilight_map_earth.png}
\caption{The Twilight Map tab on Earth, showing sunrise and sunset,
civil, nautical and astronomical twilight, night, and the subsolar and
sublunar points.}
\label{fig:ObjectVisibility:TwilightMapEarth}
\end{figure}

The third tab, \menu{Twilight Map}, follows the planetarium time in
real time. On Earth it shades the map according to the apparent solar
altitude. The boundary between day and night uses the upper limb of the
Sun with an allowance for atmospheric refraction, while the additional
contours mark civil, nautical and astronomical twilight. The Sun and
Moon symbols show the locations where these bodies are in the zenith.

\begin{figure}[th]\centering
\includegraphics[width=.82\textwidth]{object_visibility_twilight_map_moon.png}
\caption{The Twilight Map tab used from the Moon. On bodies other than
Earth, the plugin shows the geometric solar terminator and shades the
night side.}
\label{fig:ObjectVisibility:TwilightMapMoon}
\end{figure}

When the observer is located on another supported Solar System body,
the \menu{Twilight Map} tab switches to a simpler terminator view. In
this mode the plugin uses the Sun's centre and the geometric horizon,
and shades the night side of the current body. This is useful for
planning observations or surface views from the Moon, planets, Pluto,
or the Galilean satellites supported by Stellarium's location system.
...
main bottom toolbar. The time panel provides two tabs for entering the date
and time---either as a calendar date and clock time, or as Julian Day (JD)
and Modified Julian Day (MJD) values---along with a display of the current
$\Delta$T value. Below the time fields, a row of buttons allows decreasing
the time speed, pausing or resuming real-time motion, setting the time to
now, and increasing the time speed. To the right, a list of time-jump
buttons lets the user add or subtract one unit of various astronomical
intervals (solar hour, solar day, calendar year, sidereal year, synodic
month, draconic year, and many others). An additional control allows moving
the view like when dragging the mouse or using the arrow keys in Stellarium,
and a slider enables the changing of the field of view.

The View section provides:
\begin{itemize}
\item A joystick control for panning the view in any direction.
\item A mouse-wheel zoom speed selector (1x, 3x, 6x, or 12x) that controls
  the zoom rate when scrolling over the joystick area.
\item Quick-view buttons to look towards the horizon at specific azimuths
  (0\degree, 30\degree, 60\degree, \ldots, 330\degree).
\item A vertical view offset slider with preset buttons (-50\% to +20\%),
  useful for adjusting the horizon position in presentations. The slider
  value can also be adjusted with the mouse wheel while hovering over it.
\item A field-of-view (FOV) slider with preset buttons ranging from
  0.1\degree\ to 230\degree. The slider value can also be adjusted with the
  mouse wheel while hovering over it.
\end{itemize}
...
\label{sec:plugins:RemoteControl:gamepad}

The Gamepad Controller tab\newFeature{3.0.0} provides comprehensive support for
gamepad/joystick devices using the W3C Gamepad API.\footnote{The W3C Gamepad
API has been widely available since March 2017 and is supported by all modern
browsers including Chrome 21+, Firefox 29+, Edge 12+, and Safari 10.1+.}
This feature enables hands-free control of Stellarium, making it particularly
useful for planetarium presentations, educational demonstrations, and
accessibility applications.

\subsubsection{Connection and Device Management}

To use a gamepad, simply connect it via USB or Bluetooth and click the
\button{Scan} button in the Gamepad Controller tab. The interface will detect
connected devices, but for security reasons, the browser only exposes the
gamepad after the user performs an intentional input action---pressing a
button or moving a joystick axis. Once this gesture is detected, the device
information (name, model, vendor, and unique identifier) will be displayed.

Multiple controllers can be connected simultaneously. Use the \emph{Active
Device} dropdown to switch between connected devices. Each device maintains
its own independent profile, including button mappings, calibration data, and
settings, all stored locally in the browser's \texttt{localStorage}.

\subsubsection{Button Customization}

The button customization system is dynamically populated from Stellarium's
native action system via the \texttt{/api/stelaction/list} endpoint. This
means that:

\begin{itemize}
\item All available StelActions (including those from enabled plugins) appear
  in the dropdown menus.
\item When a plugin is activated, its actions automatically appear in the
  customization lists, categorized under the plugin's name.
\item The action list is consistent with the \emph{Actions and Scripts} tab
  --- any action available there can be mapped to a gamepad button.
\item In addition to native StelActions, a set of educational custom actions
  (\emph{View Directions}, \emph{Zoom Levels}, \emph{Search Objects}, etc.)
  is also available for quick access.
\end{itemize}

To customize a button, simply select it from the dropdown menu next to the
desired button. Changes are saved per-device and persist across browser
sessions.

\subsubsection{Joystick Controls}

The two analog sticks provide intuitive control:

\begin{description}
\item[Left Stick] Controls view panning. The sensitivity, deadzone, and axis
  inversion (X/Y) can be adjusted via sliders and checkboxes.
\item[Right Stick] Controls continuous zoom. The zoom speed can be adjusted,
  and the Y-axis can be inverted to match user preference (pull up to zoom in
  or out).
\end{description}

\subsubsection{Calibration}

The calibration feature compensates for hardware drift on joystick axes.
To calibrate:

\begin{enumerate}
\item Click the \button{Calibrate} button.
\item Move both joysticks in circular motions.
\item Press any button on the controller to finish.
\end{enumerate}

The calibration offsets are saved per-device and can be reset at any time.

\subsubsection{Vibration Feedback}

If the connected gamepad supports vibration (works in Chrome and Edge
browsers), vibration feedback can be enabled. The intensity can be adjusted
with a slider, and a \button{Test Vibration} button allows manual
verification.

\subsubsection{Live Input Monitor}

The Live Input Monitor displays real-time raw values from the browser's
Gamepad API, showing:

\begin{itemize}
\item Button states with numerical values (0.0 to 1.0) and visual indicators.
\item Axis positions with visual meters (-1.0 to 1.0).
\item Device information including button and axis counts.
\end{itemize}

This panel is invaluable for debugging button mappings and understanding how
different browsers report gamepad inputs.

\subsubsection{Profiles (Export/Import)}

Each device profile (including button mappings and calibration data) can be
exported as a JSON file for backup or sharing. Profiles can also be imported,
allowing users to quickly switch between different controller configurations
or share setups across multiple Stellarium installations.

\subsubsection{Technical Notes}

The Gamepad Controller module is built on the W3C Gamepad API and integrates
seamlessly with Stellarium's Remote Control plugin architecture:

\begin{itemize}
\item All button actions are executed via the same API endpoints used by other
  interface components (\texttt{/api/stelaction/do}, \texttt{/api/main/fov},
  \texttt{/api/main/move}, etc.).
\item The module respects Stellarium's plugin system --- actions from enabled
  plugins (ArchaeoLines, Scenery3d, ExoPlanets, etc.) are automatically
  available for button mapping.
\item The module uses \texttt{requestAnimationFrame} for smooth 60fps polling
  and adapts to server connection status by pausing/resuming polling
  automatically.
\end{itemize}
...
The Remote Control plugin opens many more options from just triggering commands via cronjob to developing entire alternative GUIs. 
For a start, you may want to explore\newFeature{26.3} the API in a simple SwaggerUI interface at \url{http://localhost:8090/docs/index.html}. 
...
\item[ASE] The \indexterm{All Skies Encyclopaedia}%
  \footnote{\url{https://ase.exopla.net/index.php/All_Skies_Encyclopaedia}} 
  of the IAU Working Group on Star Names (WGSN) which collates and collects 
  the data on cultural names of stars and constellations in order to preserve 
  their heritage and distribute the knowledge about them beyond their culture of origin.
\end{description}
...
ase\_url             &string & (don't override)\\ % https://ase.exopla.net/index.php/\%1 \\
...
\begin{enumerate}
    ...
    \begin{itemize}
        \item Click \button{Goto Center} to locate the texture's center.
        \item Use \button{Test this texture} to preview the texture. If it overlaps with default textures, you should check \button{Disable default textures} to avoid visual conflicts. 
        ...
        \item For images solved via Astrometry.net, if manual coordinate adjustments deviate too much, \button{Reset} will appear. Click it to reset the coordinates back to the original astrometric solution.
    ...
    \item \textbf{Locate a Texture}: Double-clicking a texture item in the list will center the view on that texture.
