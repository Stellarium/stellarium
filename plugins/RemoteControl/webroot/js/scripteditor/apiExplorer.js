 ...
 * - Scenery3D Service (plug-in) (/api/scenery3d/listscene, /api/scenery3d/loadscene, /api/scenery3d/scenedescription/) 
 *
 * @module apiExplorer
 * @requires jquery
...
define(["jquery", "api/remotecontrol"], 
    ...
    var currentDisplayMode = 'formatted'; // 'formatted' or 'raw'		
		...
		var currentExecutionError = null;  // Set by collectExplorerParams when validation fails
    ...
    function setApiExplorerConfig(config) {
		...
		/**
		 * Parameter type conventions (used by renderParamField + collectExplorerParams):
		 * 
		 * INPUT-LIKE TYPES:
		 *   'string'    → <input type="text">                — Free-form text
		 *   'integer'   → <input type="number" step="1">     — Whole numbers only (e.g., IDs, pixel sizes)
		 *   'double'    → <input type="number" step="any">   — Decimal numbers (e.g., coordinates, FOV)
		 *   'number'    → DEPRECATED alias for 'double'      — Kept for backward compatibility
		 *   'vector3'   → <input type="text">                — JSON array format [x, y, z]
		 *   'textarea'  → <textarea>                         — Multi-line code (e.g., scripts/direct)
		 *   'datetime'  → <input type="date"> + <input type="time"> — Converted to Julian Day for time param
		 *   'file'      → <input type="text"> with context-aware placeholder
		 * 
		 * SELECT-LIKE TYPES:
		 *   'select'    → <select> with predefined options   — Dropdown selection
		 *   'checkbox'  → <input type="checkbox">            — Boolean with optional default
		 *   'flag'      → <input type="checkbox">            — Presence-only flag (no value in URL)
		 * 
		 * OPERATION-LEVEL FIELDS (read from the operation definition object):
		 *   responseType : 'auto'   — Default. Auto-detects type from Content-Type
		 *                             header, then falls back to body heuristics.
		 *                : 'json'   — Force JSON rendering (syntax-highlighted).
		 *                : 'html'   — Force HTML rendering (inline in a wrapper div).
		 *                : 'text'   — Force plain-text rendering (escaped <pre>).
		 *                : 'image'  — Renders as <img> preview (raw = URL).
		 *                : 'svg'    — Inlines SVG markup, or falls back to <img>.
		 *                : 'xml'    — Renders XML in an escaped <pre>.
		 *                : 'csv'    — Converts CSV to an HTML table.
		 *                : 'binary' — Shows size + optional download link.
		 * 
		 *   suggest      : string   — Renders a "Browse" button that fetches suggestions from
		 *                             a Stellarium endpoint and fills the input on selection.
		 *                             Supported values:
		 ...
		 *                               'scenes'      → /api/scenery3d/listscene
		 *                               'locations'   → /api/location/list
		 * 
		 *   overridePath : string   — Complete path override (e.g., POST main/view shares URL with GET)
		 *   trailingSlash: boolean  — Append "/" to the default service.path/operationKey
		 * 
		 * PATH ROUTING FIELDS (buildOperationPath):
		 *   Default          → service.path + "/" + operationKey
		 *   overridePath     → Use value as-is (complete replacement)
		 *   trailingSlash    → service.path + "/" + operationKey + "/"
		 */
		var services = {
						main: {
								name: 'Main Service',
								...
								operations: {
										status: {
												...
												parameters: {
														actionId: { required: false, type: 'integer', description: 'Last known action change ID' },
														propId: { required: false, type: 'integer', description: 'Last known property change ID' }
												},
												responseType: 'json'
										},
										time: {
...
"loaded": true
										...
										window: {
												...
												parameters: {
														w: { required: true, type: 'integer', description: 'Width in pixels' },
														h: { required: true, type: 'integer', description: 'Height in pixels' }
												}
										}
												}
										},
												...
												<div style="margin-top: 10px;">
														...
														<ul style="margin-top: 5px; padding-left: 20px;">
																<li>
																		...
																		<span style="color: #888;">Use the "Browse Types" button to select from available types.</span>
																</li>
						...
						location: {
								...
								path: 'location',
									...
									setlocationfields: {
												...
												parameters: {
														id: { required: false, type: 'string', description: 'Location ID from database', suggest: 'locations' },
														...
														name: { required: false, type: 'string', description: 'Custom location name' },
														region: { required: false, type: 'string', description: 'Custom region name' },
...
{"name": "Moon", "name_i18n": "Lune"}
									...
									regionlist: {
												...
												<div style="margin-top: 10px;">
														<strong>Parameter (optional):</strong>
														<ul style="margin-top: 5px; padding-left: 20px;">
																<li>
																		<code>planet</code> (optional, string): Planet name to filter regions.<br>
																		<span style="color: #888;">If provided, only regions belonging to that planet are returned.</span><br>
																		<span style="color: #888;">If omitted, all regions from all planets are returned (backward compatible).</span>
																</li>
														</ul>
												</div>
												...
												<div style="margin-top: 10px;">
														<strong>Examples:</strong>
														<ul style="margin-top: 5px; padding-left: 20px;">
																<li><code>GET /api/location/regionlist</code> → Returns all regions from all planets.</li>
																<li><code>GET /api/location/regionlist?planet=Earth</code> → Returns only regions on Earth.</li>
																<li><code>GET /api/location/regionlist?planet=Mars</code> → Returns only regions on Mars.</li>
														</ul>
												</div>
    ...
    {"name": "Eastern Europe", "name_i18n": "Eastern Europe"}
											...
											parameters: {
													planet: { 
															required: false, 
															type: 'string', 
															description: 'Planet name to filter regions (e.g., Earth, Mars). If omitted, returns all regions from all planets.' 
													}
											}
									}
								}
										...
										info: {
												...
												parameters: {
														...
														html: { 
																required: false, 
																type: 'flag', 
																description: 'Include this flag to return HTML description instead of JSON' 
														}
												}
										},
										run: {
...
{"id": "actionShow_ArchaeoLines", "isCheckable": true, "isChecked": false, "text": "ArchaeoLines"}
								...
								do: {
										...
										<div style="margin-top: 10px; background: rgba(102, 217, 239, 0.1); padding: 10px; border-radius: 4px;">
												...
												<span style="color: #B4B7B0;">
														Use the <strong>"Browse Actions"</strong> button below to fetch this list directly and select an action ID.
														Categories are loaded dynamically from Stellarium and reflect the current 
...
"LandscapeMgr.allLandscapeNames": {
		...
		"isWritable": false
												...
												<div style="margin-top: 10px; background: rgba(102, 217, 239, 0.1); padding: 10px; border-radius: 4px;">
														<strong style="color: #66D9EF;">Pro Tip:</strong>
														<span style="color: #B4B7B0;">
																Use the <strong>"Browse Properties"</strong> button below to fetch this list directly and select a property ID.
																The list includes current values, data types, and writability information.
						...
						simbad: {
							...
							operations: {
									lookup: {
											method: 'GET',
											desc: 
											`<p>Query the online SIMBAD astronomical database for celestial objects.</p>
...
<pre style="background: #888; padding: 10px; border-radius: 4px; font-size: 11px; margin: 5px 0;">// Example: Focus on the first SIMBAD result using its position vector
...
"ProjectionHammer": "Hammer-Aitoff"
						...
						scenery3d: {
								name: 'Scenery3D Service',
								path: 'scenery3d',
								operations: {
										listscene: {
												method: 'GET',
												desc: `<p>Retrieve the list of all available 3D scenery scenes.</p>
															<p>Returns a JSON object where each key is a scene ID and each value is the scene name (localized).</p>
															<div style="margin-top:10px;background:rgba(102,217,239,0.1);padding:10px;border-radius:4px;">
																	<strong style="color:#66D9EF;">Example Response:</strong>
<code style="background:#888;padding:10px;border-radius:4px;font-size:12px;margin-top:5px;">
{
		"Sterngarten": "Vienna Sterngarten",
		"Testscene": "Testscene"
}</code>
															</div>`,
												parameters: {}
										...
										loadscene: {
												method: 'POST',
												desc: `<p>Load a specific 3D scenery scene by its ID.</p>
															<div style="margin-top:10px;background:rgba(249,38,114,0.1);padding:10px;border-radius:4px;">
																	<strong style="color:#F92672;">Note:</strong>
																	<span style="color:#B4B7B0;">Scene loading is asynchronous. Check the status via properties like <code>Scenery3d.loadingSceneID</code>.</span>
															</div>
															<div style="margin-top:10px;background:rgba(253,216,134,0.1);padding:10px;border-radius:4px;">
																	<strong style="color:#E6DB74;">Tip:</strong>
																	<span style="color:#B4B7B0;">Use the <strong>"Browse Scenes"</strong> button below to select a scene ID.</span>
															</div>`,
												parameters: {
														id: {
																required: true,
																type: 'string',
																description: 'Scene identifier (e.g., "Sterngarten", "Testscene")',
																suggest: 'scenes'
														}
												}
										},
										...
										scenedescription: {
												method: 'GET',
												desc: `<p>Access files and resources from a specific scene directory.</p>
															<p>This endpoint follows the pattern: <code>/api/scenery3d/scenedescription/{id}/</code></p>
															<ul style="margin-top:5px;padding-left:20px;">
																	<li><strong>Without path:</strong> Returns the HTML description of the scene.</li>
																	<li><strong>With path:</strong> Returns a specific file (e.g., <code>image.png</code>, <code>description.en.utf8</code>).</li>
															</ul>
															<div style="margin-top:10px;background:rgba(102,217,239,0.1);padding:10px;border-radius:4px;">
																	<strong style="color:#66D9EF;">Example:</strong>
																	<br>																	
																	<code>/api/scenery3d/scenedescription/Sterngarten/</code> → HTML description
																	<br>
																	<code>/api/scenery3d/scenedescription/Sterngarten/image.png</code> → Image file
																	<br>																	
																	<code>/api/scenery3d/scenedescription/Sterngarten/description.en.utf8</code> → Description file																	
															</div>`,
												trailingSlash: true,
												parameters: {
														sceneId: {
																required: false,
																type: 'string',
																description: 'Scene ID (e.g., "Sterngarten"). If empty, may return description of current scene.',
																suggest: 'scenes'
														},
														file: {
																required: false,
																type: 'string',
																description: 'File path within the scene folder (e.g., "description.en.utf8", "Sterngarten_20100320.png")'
														}
												}
										}
								}
     ...
     * 
     * Handles three distinct cases that appear in the Stellarium Remote Control API:
     *   1. **overridePath** → Use it as a complete replacement (e.g., POST main/view 
     *      shares the same URL as GET main/view).
     *   2. **trailingSlash: true** → Append a trailing "/" to the default path 
     *      (required by endpoints like view/skyculturedescription/ which return 
     *      a "folder-like" resource description).
     *   3. **Default** → service.path + "/" + operationKey (no trailing slash).
     * 
     * This centralizes path construction logic so that executeRequest(),
     * generateCurlCommand(), and showOperationPanel() all use the same rules.
     * 
     * @param {string} serviceKey - The service key (e.g., 'view', 'main')
     * @param {string} operationKey - The operation key (e.g., 'skyculturedescription')
     * @param {Object} op - The operation definition object from the services registry
     * @returns {string} The full API path (without the leading '/api/' prefix)
     * 
     * @example
     * buildOperationPath('view', 'skyculturedescription', { trailingSlash: true });
     * // → 'view/skyculturedescription/'
     * 
     * buildOperationPath('main', 'setview', { overridePath: 'main/view' });
     * // → 'main/view'
     * 
     * buildOperationPath('objects', 'find', {});
     * // → 'objects/find'
     */
    function buildOperationPath(serviceKey, operationKey, op) {
        var service = services[serviceKey];
        if (!service || !op) {
            console.warn('[ApiExplorer] buildOperationPath: invalid service or operation', serviceKey, operationKey);
            return '';
        }
        
        // Case 1: Complete path override
        if (op.overridePath) {
            return op.overridePath;
        }
        
        // Case 2 & 3: Build from service path + operation key
        var path = service.path + '/' + operationKey;
        
        // Add trailing slash if the endpoint requires it
        if (op.trailingSlash) {
            path += '/';
        }
		...
		function browseSkyCultureFiles() {
				...
				$.ajax({
						url: '/api/stelproperty/list',// using this endpoint to check for current skyculture ID
						dataType: 'json',
						timeout: 5000
				}).done(function(propData) {
						var cultureId = null;
						if (propData && propData['StelSkyCultureMgr.currentSkyCultureID']) { //ckeck current sky culture ID
		...
		function showFileBrowser(commonFiles, illustrations, cultureId) {
				...
				if (illustrations.length > 0) {
						...
						illustrations.forEach(function(item) {
								...
								html += '<div class="thumb-error" style="display:none;position:absolute;top:0;left:0;width:100%;height:100%;align-items:center;justify-content:center;color:#666;font-size:10px;background:rgba(0,0,0,0.2);">' + tr("No image") + '</div>';
								html += '</div>';
    ...
    /**
     * Display a "type mismatch" error block in the response area.
     * 
     * Shown when the server's Content-Type does not match the declared
     * responseType of the operation. The raw body is still stored so the
     * "Show Raw" toggle remains functional.
     * 
     * @param {jQuery} $responseArea - The response area container.
     * @param {jQuery} $statusEl - The status span.
     * @param {string} rawBody - The raw response body.
     * @param {string} declaredType - The operation's declared responseType.
     * @param {string} actualContentType - The Content-Type header from the server.
     * @param {string} reason - Human-readable explanation.
     */
    function showTypeMismatch($responseArea, $statusEl, rawBody, declaredType, actualContentType, reason) {
        var html =
                '<div class="response-error">' +
                '<div class="error-icon">!</div>' +
                '<div class="error-message"><strong>Response type mismatch:</strong> ' +
                escapeHtml(reason) + '</div>' +
                '<div class="error-details" style="color:#8A8C8E;font-size:11px;margin-top:8px;">' +
                'Declared <code>responseType</code>: <code>' + escapeHtml(declaredType) + '</code>. ' +
                'Actual <code>Content-Type</code>: <code>' + escapeHtml(actualContentType || '(none)') + '</code>. ' +
                '<br>Either remove <code>responseType</code> from the operation to allow auto-detection, ' +
                'or set it to match the server\'s Content-Type.' +
                '</div>' +
                '<div style="margin-top:10px;">' +
                '<strong style="color:#B4B7B0;">Raw body (truncated):</strong>' +
                '<pre style="max-height:200px;overflow:auto;background:rgba(0,0,0,0.3);padding:8px;border-radius:4px;font-size:10px;">' +
                escapeHtml(rawBody && rawBody.length > 500 ? rawBody.substring(0, 500) + '…' : (rawBody || '(empty)')) +
                '</pre>' +
                '</div>' +
                '</div>';
        
        // Preserve the raw body for the toggle
        currentRawResponse = rawBody;
        currentFormattedHtml = html;
        currentDisplayMode = 'formatted';
        
        $statusEl.html('<span class="response-status error">TYPE MISMATCH</span>');
        $responseArea.html(
                '<div class="response-toolbar">' +
                '<div class="response-toolbar-left">' +
                '<span class="response-type-badge error">TYPE MISMATCH</span>' +
                '<span style="color:#8A8C8E;font-size:10px;">declared: ' + escapeHtml(declaredType) + '</span>' +
                ...
                '</div>' +
                '</div>' +
                '<div class="response-content-wrapper">' + html + '</div>'
        );
    ...
    /**
     * Determine the effective response type for a given response.
     * 
     * Resolution order (highest priority first):
     *   1. Explicit non-'auto' responseType → strict override.
     *   2. Content-Type response header     → authoritative source.
     *   3. Body heuristics                  → last-resort fallback
     *                                          (only when the header is missing
     *                                          or too generic, e.g., text/plain).
     * 
     * @param {string} responseText - The raw response body.
     * @param {string} responseType - Declared type ('auto' | specific).
     * @param {Object} [context] - Optional { url, headers, statusCode }.
     * @returns {string} Resolved type.
     */
    function resolveResponseType(responseText, responseType, context) {
        // 1. Explicit non-auto type wins
        if (responseType && responseType !== 'auto') {
            return responseType;
        }
        
        // 2. Content-Type header (authoritative)
        if (context && context.headers) {
            var ct = (context.headers['content-type'] || context.headers['Content-Type'] || '').toLowerCase();
            
            if (ct) {
                // Strip parameters (e.g., "application/json; charset=utf-8" → "application/json")
                var mime = ct.split(';')[0].trim();
                
                // Explicit mapping from MIME to internal type
                ...
                if (mime === 'application/octet-stream' ||
                    ...
                    mime.indexOf('video/') === 0)                         return 'binary';
                if (mime === 'text/plain')                                return 'text';
                // Unknown MIME → fall through to heuristics
            }
        }
        
        // 3. Body heuristics (fallback)
        if (!responseText || responseText.trim() === '') {
            return 'text';
        }
        ...
        if (trimmed.startsWith('<')) {
            if (trimmed.indexOf('<?xml') === 0) return 'xml';
            if (trimmed.indexOf('<svg') === 0)   return 'svg';
            return 'html';
        }
        return 'text';
    }
    ...
    /**
     * Render a JSON body with syntax highlighting.
     * Arrays are rendered with the JSON viewer for consistency.
     * Falls back to an error block if parsing fails.
     * 
    ...
    function renderJson(text) {
        try {
            ...
            return '<div class="response-error">' +
                   '<div class="error-message">Invalid JSON: ' + escapeHtml(e.message) + '</div>' +
    ...
    /**
     * Render an HTML body inside a wrapper that inherits direction.
     * 
     * @param {string} text - Raw HTML text.
     * @returns {string} HTML.
     */
    function renderHtml(text) {
        return '<div class="html-response">' + text + '</div>';
    }
    ...
    function renderText(text) {
        return '<pre class="plain-text-response">' + escapeHtml(text) + '</pre>';
    }

    /**
     * Render an image URL as an <img> preview with a fallback message.
     * 
     * @param {string} url - The image URL.
     * @returns {string} HTML.
     */
    function renderImage(url) {
        return '<div class="image-response-container" style="text-align:center;padding:10px;">' +
               '<img src="' + escapeAttr(url) + '" alt="Image" ' +
               'style="max-width:100%;max-height:70vh;border:1px solid #ccc;border-radius:4px;" ' +
               'onerror="this.style.display=\'none\';this.parentElement.querySelector(\'.img-fallback\').style.display=\'block\';">' +
               '<div class="img-fallback" style="display:none;color:#888;padding:20px;">' +
               '<strong>Image not available</strong><br>Could not load: ' + escapeHtml(url) + '</div>' +
               '<div style="margin-top:10px;color:#888;font-size:11px;">URL: <a href="' +
               escapeAttr(url) + '" target="_blank" style="color:#66D9EF;">' + escapeHtml(url) + '</a></div>' +
               '</div>';
    }

    /**
     * Render an SVG response. Tries to inline the SVG markup when the body
     * itself is SVG; otherwise treats it as an image URL.
     * 
     * @param {string} text - Raw SVG markup OR fallback text.
     * @param {Object} [context] - { url } when the body has no SVG markup.
     * @returns {string} HTML.
     */
    function renderSvg(text, context) {
        if (text && text.trim().startsWith('<svg')) {
            return '<div class="svg-response" style="text-align:center;padding:10px;">' +
                   text +
                   '</div>';
        }
        if (context && context.url) {
            return renderImage(context.url);
        }
        return renderText(text);
    }

    /**
     * Render an XML body inside a <pre> block with escaping.
     * 
     * @param {string} text - Raw XML text.
     * @returns {string} HTML.
     */
    function renderXml(text) {
        return '<pre class="xml-response" style="white-space:pre-wrap;word-break:break-word;">' +
               escapeHtml(text) +
               '</pre>';
    }

    /**
     * Render a CSV body as an HTML table.
     * Handles simple comma-separated data; quoted fields are not fully parsed.
     * 
    ...
    function renderCsv(text) {
        ...
        if (lines.length === 0) return renderText(text);
    ...
    /**
     * Render a binary response as a size notice with an optional download link.
     * 
     * @param {string} text - Raw binary-as-text (length is used).
     * @param {Object} [context] - { url } for the download link.
     * @returns {string} HTML.
     */
    function renderBinary(text, context) {
        var size = text ? text.length : 0;
        return '<div class="binary-response" style="padding:20px;text-align:center;color:#B4B7B0;">' +
               '<p><strong>Binary data</strong> (' + formatBytes(size) + ')</p>' +
               (context && context.url
                   ? '<p><a href="' + escapeAttr(context.url) + '" download style="color:#66D9EF;">Download</a></p>'
                   : '') +
               '</div>';
    }

    /**
     * Parse jQuery XHR response headers into a plain object.
     * Returns an empty object if headers are unavailable.
     * 
     * @param {Object} xhr - jQuery XHR object.
     * @returns {Object} Headers map (lower-cased keys).
     */
    function parseResponseHeaders(xhr) {
        ...
        try {
            ...
            // Headers unavailable — silent fallback to {}
        }
        return headers;
    }

    /**
     * Verify that a response body matches the declared responseType.
     * 
     * The primary check uses the Content-Type header (authoritative).
     * Body heuristics are used only when the header is missing or too generic.
     * 
     * Returns { valid: true } or { valid: false, reason, contentType }.
     * 
     * @param {string} responseText - Raw response body.
     * @param {string} declaredType - The operation's declared responseType.
     * @param {Object} [context] - { headers } from the XHR.
     * @returns {Object}
     */
    function validateResponseAgainstType(responseText, declaredType, context) {
        ...
        var ct = '';
        if (context && context.headers) {
            ct = (context.headers['content-type'] || '').toLowerCase();
        }
        var mime = ct.split(';')[0].trim();
        
        // Map MIME prefix → internal type
        var mimeToType = function(mime) {
            if (!mime) return null;
            if (mime === 'application/json' || mime.endsWith('+json')) return 'json';
            if (mime === 'text/html')                                 return 'html';
            if (mime === 'image/svg+xml')                             return 'svg';
        ...
        var actualType = mimeToType(mime);
        
        // If we have an authoritative Content-Type, compare directly
        if (actualType) {
            if (actualType === declaredType) {
                return { valid: true };
            }
            // Special allowance: declared 'json' but actual 'text/plain' —
            // still try JSON.parse as a fallback (some servers mislabel JSON).
            if (declaredType === 'json' && actualType === 'text') {
                try {
                    ...
                    return {
                        valid: false,
                        reason: 'responseType is "json", but Content-Type is "text/plain" and body does not parse as JSON.',
                        contentType: ct
                    };
                }
            }
            return {
                valid: false,
                reason: 'responseType is "' + declaredType + '", but Content-Type is "' + ct + '".',
                contentType: ct
            };
        }
        
        // No authoritative Content-Type → fall back to body heuristics
        if (!responseText || responseText.trim() === '') {
            return (declaredType === 'text' || declaredType === 'binary')
                ? { valid: true }
                : { valid: false, reason: 'Empty body, but responseType was "' + declaredType + '".' };
        }
        ...
        switch (declaredType) {
            ...
            case 'html':
                return trimmed.startsWith('<')
                    ? { valid: true }
                    : { valid: false, reason: 'Body does not start with "<".' };
            case 'xml':
                return (trimmed.startsWith('<?xml') || trimmed.startsWith('<'))
                    ? { valid: true }
                    : { valid: false, reason: 'Body does not look like XML.' };
            case 'svg':
                return trimmed.startsWith('<svg')
                    ? { valid: true }
                    : { valid: false, reason: 'Body does not start with "<svg".' };
            case 'csv':
                return (trimmed.indexOf(',') !== -1 || trimmed.indexOf('\n') !== -1)
                    ? { valid: true }
                    : { valid: false, reason: 'Body contains neither commas nor newlines.' };
            case 'image':
            case 'binary':
            case 'text':
            default:
     ...
     * 
     * Renders the operation title, method badge, path (built via buildOperationPath),
     * description (HTML), parameter fields, action buttons, and the response area.
     * Also binds all dynamic suggestion buttons (Browse Types, Actions, Properties,
     * Scripts, Scenes, Locations) and the "Browse Files" button when applicable.
     * 
     * @param {string} serviceKey - The service key (e.g., 'view', 'main')
     * @param {string} operationKey - The operation key (e.g., 'skyculturedescription')
     */
		function showOperationPanel(serviceKey, operationKey) {
				...
				// Build full path using centralized helper
				// (respects overridePath, trailingSlash, or default service.path/operationKey)
				var fullPath = buildOperationPath(serviceKey, operationKey, op);
				...
				// ============================================================
				// BIND ALL SUGGESTION BUTTONS DYNAMICALLY
				// ============================================================
				
				// 1. BIND "Browse Types" Button (for objecttypes)
				...
				// 3. BIND "Browse Properties" Button
				...
				// ============================================================
				// NEW: DYNAMIC BINDING FOR ALL SUGGESTION BUTTONS
				// ============================================================
				// This handles: scripts, scenes, locations, and any future suggest types
				
				// Find all suggestion buttons that have data-suggest attribute
				$('.suggestion-btn[data-suggest]').each(function() {
						var $btn = $(this);
						var suggestType = $btn.data('suggest');
						
						// Find the target input within the same param-group
						var $targetInput = $btn.closest('.param-group').find('input[type="text"], input[type="number"], select');
						
						if (suggestType && $targetInput.length) {
								// Remove any existing click handlers to avoid duplicates
								$btn.off('click.suggestion');
								
								// Bind the click event
								$btn.on('click.suggestion', function(e) {
										e.preventDefault();
										createExplorerSuggestionDropdown($(this), $targetInput, suggestType);
								});
						}
				});
				
				// Bind Browse Files button (for sky culture)
				$('#btn-browse-files').off('click').on('click', function() {
						browseSkyCultureFiles();
				});
				
				// Bind Execute and Curl buttons
		 ...
		 * 
		 * Supports the following parameter types (in order of priority):
		 * 
		 *   Input-like types (rendered as <input>):
		 *     - 'flag'     → <input type="checkbox"> without a value (presence-only)
		 *     - 'datetime' → <input type="date"> + <input type="time"> + hidden field
		 *     - 'file'     → <input type="text"> with context-aware placeholder
		 *     - 'integer'  → <input type="number" step="1"> (whole numbers only)
		 *     - 'double'   → <input type="number" step="any"> (decimal numbers)
		 *     - 'number'   → DEPRECATED alias for 'double' (kept for backward compat)
		 *     - 'vector3'  → <input type="text"> (documented as JSON [x,y,z])
		 *     - 'textarea' → <textarea> for multi-line code (used by scripts/direct)
		 *     - 'text'     → <input type="text"> (default)
		 * 
		 *   Select-like types:
		 *     - 'select'   → <select> with predefined options
		 *     - 'checkbox' → <input type="checkbox"> with optional default value
		 * 
		 * Additional operation-level fields (read from currentOpData):
		 *   - responseType: 'image'      → Hint that the endpoint returns binary image data
		 *   - suggest:      'objecttypes'|'actions'|'properties'|'scripts'|'scenes'|'locations'
		 *                                 → Renders a "Browse" button that fetches suggestions
		 *                                   from the corresponding Stellarium endpoint.
		 * 
		 * @param {string} key - Parameter key/name
		 * @param {Object} param - Parameter definition containing type, description, options, suggest
		...
		function renderParamField(key, param, required) {
				...
				// ============================================================
				// FLAG TYPE - Just a presence flag without value
				// ============================================================
				if (param.type === 'flag') {
						html += '<div style="display:flex;align-items:center;gap:8px;margin-top:4px;">';
						html += '<input type="checkbox" id="explorer-param-' + key + '" ';
						html += 'style="width:auto;margin:0;">';
						html += '<span style="color:#8A8C8E;font-size:11px;">' + escapeHtml(param.description || '') + '</span>';
						html += '</div>';
						html += '<span style="font-size:10px;color:#FD971F;display:block;margin-top:3px;">';
						html += 'This is a flag parameter - it will be added to the URL without a value when checked';
						html += '</span>';
						html += '</div>';
						return html;

				// ============================================================
				// DATETIME TYPE - Two inputs (date + time) + hidden field
				// ============================================================
				...
				// ============================================================
				// FILE TYPE - Context-aware placeholder per operation
				// Used by: view/skyculturedescription, view/landscapedescription,
				//          scenery3d/scenedescription
				// ============================================================
				} else if (key === 'file') {
						// Build a context-aware placeholder based on the current operation
						var filePlaceholder = 'Path to a file within the resource folder (e.g., index.json, description.md, image.png)';
						
						if (currentService === 'view' && currentOperation === 'skyculturedescription') {
								filePlaceholder = 'e.g., index.json, description.md, illustrations/andromeda.png';
						} else if (currentService === 'view' && currentOperation === 'landscapedescription') {
								filePlaceholder = 'e.g., landscape.png, textures/ground.jpg, description.html';
						} else if (currentService === 'scenery3d' && currentOperation === 'scenedescription') {
								filePlaceholder = 'e.g., description.en.utf8, Sterngarten_20100320.png';
						}
						
						html += '<input type="text" id="explorer-param-' + key + '" ';
						html += 'placeholder="' + escapeAttr(filePlaceholder) + '" style="width:100%;">';
						if (param.description) {
								html += '<span class="param-hint">' + escapeHtml(param.description) + '</span>';
						}
						html += '<div style="margin-top:5px;font-size:9px;color:#8A8C8E;">';
						html += 'Tip: Use the "Browse Files" or "Browse" button (if shown below) to explore available files.';
						html += '</div>';
						html += '</div>';
						return html;
				
				// ============================================================
				// SELECT TYPE - Dropdown with predefined options
				// ============================================================
				} else if (param.type === 'select') {
						html += '<select id="explorer-param-' + key + '" style="width:100%;">';
						// Add empty option if not required
						if (!required) {
								html += '<option value="">-- Select ' + escapeHtml(key) + ' --</option>';
						}
				...
				// ============================================================
				// CHECKBOX TYPE - Boolean with optional default
				// ============================================================
				...
				// ============================================================
				// TEXTAREA TYPE - Multi-line code input (scripts/direct)
				// ============================================================
				} else if (param.type === 'textarea') {
						html += '<textarea id="explorer-param-' + key + '" ';
						html += 'rows="6" ';
						html += 'placeholder="' + escapeAttr(param.description || 'Enter code here...') + '" ';
						html += 'style="width:100%;font-family:monospace;font-size:11px;padding:6px;background:rgba(0,0,0,0.3);color:#DCDBDA;border:1px solid #3A3C3E;border-radius:4px;resize:vertical;">';
						html += '</textarea>';
						if (param.description) {
								html += '<span class="param-hint">' + escapeHtml(param.description) + '</span>';
						}
				...
				// ============================================================
				// NUMERIC TYPES (integer, double, number) + TEXT + VECTOR3
				// ============================================================
				} else {
						var inputType = 'text';
						var stepAttr = '';
						var placeholderText = param.description || '';
						var hintText = '';
						
						// Integer: whole numbers only
						if (param.type === 'integer') {
								inputType = 'number';
								stepAttr = ' step="1" min="-10"';// setp='1' integers only
								placeholderText = 'e.g., 32 (no decimals)';
								hintText = 'Whole numbers only (no decimals).';
						
						// Double: decimal numbers (also handles deprecated 'number' alias)
						} else if (param.type === 'double' || param.type === 'number') {
								inputType = 'number';
								stepAttr = ' step="any"';// step='any' integers and decimals
								placeholderText = 'e.g., 3.14 Decimal numbers accepted';
								hintText = 'Decimal numbers accepted.';
						
						// Vector3: JSON array [x,y,z] - treated as text input
						} else if (param.type === 'vector3') {
								inputType = 'text';
								placeholderText = param.description || 'e.g., [0.933, 0.357, 0.035]';
								hintText = 'JSON array format: [x, y, z]';
						
						// Default: plain text
						} else {
								inputType = 'text';
								placeholderText = param.description || '';
						}
						
						html += '<input type="' + inputType + '" id="explorer-param-' + key + '" ';
						html += 'placeholder="' + escapeAttr(placeholderText) + '"';
						html += stepAttr + ' style="width:100%;">';
						...
						if (hintText) {
								html += '<span style="font-size:9px;color:#8A8C8E;display:block;margin-top:2px;">' + hintText + '</span>';
						}
				}
				
				// ============================================================
				// CONTEXTUAL BROWSE BUTTONS (suggest)
				// ============================================================
				...
				// Browse Scripts button for script ID parameter (scripts/run, scripts/info)
				else if (param.suggest === 'scripts') {
						html += '<div style="margin-top:5px;">';
						html += '<button type="button" class="suggestion-btn jquerybutton" data-suggest="scripts" title="Browse available scripts from Stellarium">';
						html += '<span class="suggestion-btn-icon">ssc</span> Browse Scripts';
						html += '</button>';
						html += '</div>';
				}

				// Browse Scenes button for scene ID parameter (scenery3d/loadscene)
				else if (param.suggest === 'scenes') {
						html += '<div style="margin-top:5px;">';
						html += '<button type="button" class="suggestion-btn jquerybutton" data-suggest="scenes" title="Browse available 3D scenery scenes">';
						html += '<span class="suggestion-btn-icon">sc</span> Browse Scenes';
						html += '</button>';
						html += '</div>';
				}

				// Browse Locations button for location ID parameter (location/setlocationfields)
				else if (param.suggest === 'locations') {
						html += '<div style="margin-top:5px;">';
						html += '<button type="button" class="suggestion-btn jquerybutton" data-suggest="locations" title="Browse available locations from Stellarium database">';
						html += '<span class="suggestion-btn-icon">l</span> Browse Locations';
						html += '</button>';
						html += '</div>';
				}
		...
		function fetchExplorerSuggestions(type, callback) {
				...
				$.ajax({
						...
						// ============================================================
						// TYPE: scenes - 3D Scenery Scenes List
						// ============================================================
						if (type === 'scenes') {
								if (typeof data === 'object' && data !== null) {
										var sceneIds = Object.keys(data);
										// Sort by display name (value)
										sceneIds.sort(function(a, b) {
												return (data[a] || a).localeCompare(data[b] || b);
										});
										
										sceneIds.forEach(function(id) {
												items.push({
														id: id,                         // Scene ID: "Sterngarten"
														text: id + ' → ' + (data[id] || id),  // "Sterngarten → Vienna Sterngarten"
														name: data[id] || id            // Display name: "Vienna Sterngarten"
												});
										});
								}
						}
												...
												if (Array.isArray(actions)) {
														...
														actions.forEach(function(action) {
																...
																if (action.isCheckable === true) {
																		var state = action.isChecked ? '✓ on' : '✗ off';
																		displayText += ' [' + state + ']';
																}
																items.push({
																		id: action.id,
    ...
    // =====================================================================
		 ...
		 * 
		 * Performs type-aware processing:
		 *   - 'datetime'  → Converts date + time to Julian Day (for the time parameter)
		 *   - 'file'      → Kept as string for path building
		 *   - 'flag'      → Only added if checked (presence-only, no value)
		 *   - 'integer'   → Validated strictly as a whole number (allows negative).
		 *                   On failure: records currentExecutionError and skips the parameter.
		 *   - 'double'    → Parsed as float (also handles deprecated 'number' alias)
		 *   - 'number'    → Treated as 'double' for backward compatibility
		 *   - 'checkbox'  → Boolean value
		 *   - 'select'    → String value from the dropdown
		 *   - 'vector3'   → String (JSON array) passed through
		 *   - 'textarea'  → String (multi-line) passed through
		 *   - default     → String value
		 * 
		 * Automatically adds timerate=1 for time operations if time is set but timerate is not.
		 * 
		 * Sets the module-level flag `currentExecutionError` when an integer parameter
		 * receives a value that is not a whole number (e.g., "2.3" or "abc").
		 * This flag is checked by executeRequest() to abort the request before it is sent.
		 * 
		 * @returns {Object} Collected and type-coerced parameters
		 */
		function collectExplorerParams() {
				...
				Object.keys(currentOpData.parameters).forEach(function(key) {
						...
						// ============================================================
						// FLAG TYPE - Only add if checked (presence-only, no value)
						// ============================================================
						if (paramType === 'flag') {
								if ($input.is(':checked')) {
										params[key] = true;
						...
						// ============================================================
						// DATETIME TYPE - Convert to Julian Day
						// ============================================================
						if (key === 'time' && paramType === 'datetime') {
						...
						// ============================================================
						// FILE TYPE - Keep as string for path building
						// ============================================================
						} else if (key === 'file') {
								value = $input.val();
								if (value && value.trim() !== '') {
						...
						// ============================================================
						// SELECT TYPE - Read selected value
						// ============================================================
						...
						// ============================================================
						// TEXTAREA TYPE - Multi-line string
						// ============================================================
						} else if ($input.is('textarea')) {
								value = $input.val();
						
						// ============================================================
						// STANDARD INPUT - Apply type-aware validation
						// ============================================================
						} else {
								value = $input.val();
								
								// Only process if we have a non-empty value
								if (value !== undefined && value !== null && value !== '') {
										
										// INTEGER: strict whole-number validation (allows negative)
										if (paramType === 'integer') {
												var rawValue = String(value).trim();
												
												// Reject decimals, letters, spaces, and anything that
												// is not a whole number. Allows an optional leading '-'.
												if (!/^-?\d+$/.test(rawValue)) {
														var errMsg = 'The "' + key + '" parameter must be a whole number (no decimals). Received: "' + rawValue + '"';
														console.warn('[ApiExplorer] Invalid integer for "' + key + '":', rawValue);
														
														// Record the error so executeRequest can abort
														currentExecutionError = errMsg;
														
														// Skip adding this parameter
														return;
												}
												
												// Valid integer → parse it (base 10)
												value = parseInt(rawValue, 10);
												console.log('[ApiExplorer] Integer "' + key + '" =', value);
										
										// DOUBLE (or deprecated NUMBER): parse as float
										} else if (paramType === 'double' || paramType === 'number') {
												var floatVal = parseFloat(value);
												if (!isNaN(floatVal)) {
														value = floatVal;
														console.log('[ApiExplorer] Double "' + key + '" =', floatVal);
												} else {
														console.warn('[ApiExplorer] Invalid double for "' + key + '":', value, '- passing as-is');
												}
										}
										// Other types (string, vector3, textarea, etc.): pass through as-is
								}
						}
						
						// ============================================================
						// Only add if value is not undefined, null, or empty string
						// ============================================================
						if (value !== undefined && value !== null && value !== '') {
								params[key] = value;
						}
		 ...
		 * - Image responses (planetimage with responseType: 'image')
		 * - Sky culture / landscape file requests (with trailing slash + file path)
		 * - Scenery3D scene description requests (with sceneId + file path)
		 * - Normal JSON/HTML/text responses
		 * 
		 * Path construction uses buildOperationPath() for all standard cases,
		 * with explicit overrides only for file-based endpoints that need
		 * a dynamic path segment appended after the trailing slash.
		 * 
		 * @returns {void}
		 */
		function executeRequest() {
				...
				// ============================================================
				currentExecutionError = null;
				
				// ============================================================
				// COLLECT PARAMS (may set currentExecutionError)
				// ============================================================
				var params = collectExplorerParams();
				
				// ============================================================
				// ABORT IF ANY PARAMETER FAILED VALIDATION
				// ============================================================
				if (currentExecutionError) {
						console.warn('[ApiExplorer] Execution aborted due to validation error:', currentExecutionError);
						
						// Show a clear error message in the response area
						$statusEl.html('<span class="response-status error">VALIDATION ERROR</span>');
						$responseArea.html(
								'<div class="response-error">' +
								'<div class="error-icon">!</div>' +
								'<div class="error-message"><strong>Validation Error:</strong> ' + escapeHtml(currentExecutionError) + '</div>' +
								'<div class="error-details" style="color:#8A8C8E;font-size:11px;margin-top:8px;">' +
								'Please correct the value and try again. Whole numbers only (e.g., 0, 5, -1, 42).' +
								'</div>' +
								'</div>'
						);
						
						// Reset the flag so the next attempt starts clean
						currentExecutionError = null;
				...
				// Check if html flag is present (for scripts/info endpoint)
				var hasHtmlFlag = params.html === true;
				if (hasHtmlFlag) {
						// Remove html from params - we'll handle it as a flag in the URL
						delete params.html;
				}
				
				// Declared response type for this operation (used by formatResponseByType)
				var declaredResponseType = op.responseType || 'auto';
				...
				// ============================================================
				// BUILD FULL PATH - Special handling for file-based endpoints
				// ============================================================
				var fullPath;
				...
				// Special handling: Scenery3D scenedescription endpoint
				// Path pattern: /api/scenery3d/scenedescription/{sceneId}/{file}
				if (currentService === 'scenery3d' && currentOperation === 'scenedescription') {
						// Extract sceneId and file from params
						var sceneId = serverParams.sceneId || '';
						var filePath = serverParams.file || '';
						
						// Remove them from serverParams (they will be in the path)
						delete serverParams.sceneId;
						delete serverParams.file;
						
						// Build the path: /api/scenery3d/scenedescription/{sceneId}/{file}
						fullPath = 'scenery3d/scenedescription/';
						if (sceneId) {
								fullPath += encodeURIComponent(sceneId) + '/';
						}
						if (filePath) {
								fullPath += encodeURIComponent(filePath);
						}
						
						// If no sceneId is provided, we still need the trailing slash
						if (!sceneId && !filePath) {
								fullPath += '/';
						}
						
						console.log('[ApiExplorer] Scenery3D description path:', fullPath);
				}

				// Special handling: Sky culture / landscape description endpoints
				// Path pattern: /api/{service}/{operation}/{file}
				else if (currentOperation === 'skyculturedescription' || currentOperation === 'landscapedescription') {
						...
						if (serverParams.file && serverParams.file.trim() !== '') {
								...
								isFileRequest = true;
								// Build with file appended after trailing slash
								...
								// No file → use the standard builder (respects trailingSlash: true)
								fullPath = buildOperationPath(currentService, currentOperation, op);
						}
				} 
				
				// Default: use centralized path builder
				else {
						fullPath = buildOperationPath(currentService, currentOperation, op);
				}
				...
				if (isImageResponse) {
								...
								if (hasHtmlFlag) queryParams.push('html');
								if (queryParams.length > 0) imageUrl += '?' + queryParams.join('&');
								
								$.ajax({
												url: imageUrl,
												method: op.method === 'GET' ? 'GET' : 'POST',
												data: op.method === 'GET' ? null : (function() {
																var d = {};
																Object.keys(serverParams).forEach(function(k) { d[k] = serverParams[k]; });
																if (hasHtmlFlag) d.html = '';
																return d;
												})(),
												dataType: 'text',
												success: function(probeData, ts, probeXhr) {
																// Authoritative source: Content-Type
																var ct = '';
																try {
																				ct = (probeXhr.getResponseHeader('Content-Type') || '').toLowerCase();
																} catch (e) { /* ignore */ }
																...
																if (!isImage) {
																				// Mismatch: server did not return an image
																				showTypeMismatch(
																								$responseArea,
																								$statusEl,
																								probeData,
																								'image',
																								ct,
																								'responseType is "image", but the server returned Content-Type "' + (ct || '(none)') + '".'
																				);
																				return;
																}
																
																// Real image — render it
																$statusEl.html('<span class="response-status success">HTTP 200 OK (Image)</span>');
																currentRawResponse = 'Image: ' + imageUrl;
																currentDisplayMode = 'formatted';
																
																var formattedContent = renderImage(imageUrl);
																currentFormattedHtml = formattedContent;
												...
												error: function(xhr) {
																...
																currentRawResponse = xhr.responseText || 'Image request failed';
																var errorContent = '<div class="response-error"><div class="error-icon">!</div>' +
																									 '<div class="error-message"><strong>Error:</strong> Image request failed.</div></div>';
																...
																$responseArea.html(
																				...
																				'</div>' +
																				'</div>' +
																				'<div class="response-content-wrapper">' + errorContent + '</div>'
																);
																$('#toggle-display-btn').off('click').on('click', function() { toggleResponseDisplay(); });
																$('#copy-response-btn').off('click').on('click', function() { copyResponse(); });
												}
				...
				var ajaxOptions = {
						...
						success: function(data, textStatus, xhr) {
										...
										// RAW storage — always the unmodified body
										currentRawResponse = data;
										currentDisplayMode = 'formatted';
										
										// Build a minimal context (url + headers + status) for the renderer
										var context = {
														url: url,
										...
										// ============================================================
										// VALIDATE: does the body match the declared responseType?
										// ============================================================
										var validation = validateResponseAgainstType(data, declaredResponseType, context);
										...
										// ============================================================
										var formattedContent = formatResponseByType(data, declaredResponseType, context);
										currentFormattedHtml = formattedContent;
				...
				// ============================================================
				// BUILD REQUEST WITH PROPER METHOD AND PARAMETERS
				// ============================================================
				if (op.method === 'GET') {
						ajaxOptions.method = 'GET';
						if (Object.keys(serverParams).length > 0 || hasHtmlFlag) {
								var paramsForUrl = [];
								
								// Add regular parameters
								Object.keys(serverParams).forEach(function(key) {
										paramsForUrl.push(encodeURIComponent(key) + '=' + encodeURIComponent(String(serverParams[key])));
								});
								
								// Add html as flag WITHOUT value (e.g., &html)
								if (hasHtmlFlag) {
										paramsForUrl.push('html');
								}
								
								if (paramsForUrl.length > 0) {
										var connector = url.indexOf('?') >= 0 ? '&' : '?';
										var queryString = paramsForUrl.join('&');
										ajaxOptions.url = url + connector + queryString;
										// Clear data to avoid double encoding
										ajaxOptions.data = null;
								}
						}
				} else {
						ajaxOptions.method = 'POST';
						// For POST, add html as a parameter with empty value if flag is present
						var postData = {};
						Object.keys(serverParams).forEach(function(key) {
								postData[key] = serverParams[key];
						});
						if (hasHtmlFlag) {
								postData.html = '';
						}
    ...
    function formatBytes(bytes) {
		 ...
		 * 
		 * Uses dynamic base URL resolution (getCurlBaseUrl) to work with any
		 * Stellarium configuration. Handles several special cases:
		 * - Converts timerate from user value (1 = normal speed) to server JD_SECOND value.
		 * - Includes the time parameter with proper Julian Day conversion.
		 * - Handles the html flag parameter without a value for scripts/info.
		 * - Handles Scenery3D scenedescription with dynamic path segments (sceneId + file).
		 * 
		 * Path construction uses buildOperationPath() for all standard cases.
		 * 
		 * @returns {void}
		...
		function generateCurlCommand() {
				...
				// ============================================================
				// BUILD FULL PATH - Special handling for scenedescription
				// ============================================================
				var fullPath;
				var isScenery3dDescription = (currentService === 'scenery3d' && currentOperation === 'scenedescription');
				
				if (isScenery3dDescription) {
						// Extract sceneId and file from params
						...
						// Remove from params (they are in the path)
						delete convertedParams.sceneId;
						delete convertedParams.file;
						
						console.log('[ApiExplorer] Scenery3D description path:', fullPath);
				} else {
						// Use centralized path builder (respects overridePath and trailingSlash)
						fullPath = buildOperationPath(currentService, currentOperation, op);
				}
				...
				if (op.method === 'GET') {
						if (Object.keys(convertedParams).length > 0 || hasHtmlFlag) {
								cmd += ' -G';
								
								// Add regular parameters
								Object.keys(convertedParams).forEach(function(key) {
										cmd += ' -d "' + key + '=' + encodeURIComponent(String(convertedParams[key])) + '"';
								});
								
								// Add html as flag WITHOUT value (e.g., -d "html")
								if (hasHtmlFlag) {
										cmd += ' -d "html"';
								}
						}
