...
const char *hip_main_name = "cdsweb.u-strasbg.fr/ftp/cats/I/239m/hip_main_r.dat"; // modified for J2000.0
const char *h_dm_com_name = "cdsweb.u-strasbg.fr/ftp/cats/I/239m/h_dm_com_r.dat"; // modified for J2000.0
...
int ReadHipMain(const char *fname,Accumulator &accu) {
  ...
  while (451==fread(buff,1,451,f)) {
    ...
    ASSERT(buff[434]=='|' && buff[447]=='|' && buff[450]=='\n');
    ...
    buff[434]='\0'; buff[447]='\0'; buff[450]='\0';
    ...
    char SpType[13];
    for (int i=0;i<12;i++) SpType[i] = buff[435+i];
    SpType[12] = '\0';
    for (int i=11;i>=0;i--) {
      if (SpType[i] != ' ') break;
      SpType[i] = '\0';
