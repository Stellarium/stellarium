...
StelApp::StelApp(const string& CDIR, const string& LDIR, const string& DATA_ROOT, 
				 bool _CreateAviAndExit, bool _CreateAviOnFlyAndExit, std::string _ScriptToRunOnStartUp,
				 int _resultVideoWidth, int _resultVideoHeight, std::string _videoOutputPath, 
        ...
        initialized(0), GMT_shift(0)
{
	...
	SelectedScriptForAvi = SelectedScriptForAviDirectory = "";
	SelectedScriptForEditing = SelectedScriptForEditingDirectory = "";
    ...
    time_multiplier = 1;
    distorter = 0;
	CreateAviAndExit = _CreateAviAndExit;
  CreateAviOnFlyAndExit = _CreateAviOnFlyAndExit;
	ScriptToRunOnStartUp = _ScriptToRunOnStartUp;
	hbuffer = NULL;
	hpbufdc = NULL;
	pbufglctx = NULL;
	VideoWriter = NULL;
	...
	resultVideoWidth = _resultVideoWidth;
	resultVideoHeight = _resultVideoHeight;
...
{
    if (this->CreateAviAndExit)
    {
      InitParser conf;
      std::string configFile = this->getConfigFile();
      conf.load(configFile+".save");
    ...
    if (this->CreateAviOnFlyAndExit)
    {//todo: insert some error processing
      if (hQuitEvent) SetEvent(hQuitEvent);

      if (onFlyPixels)
        UnmapViewOfFile(onFlyPixels);
...
{
    //Rewrite the config file so that the video file being generated looks better
    InitParser conf;
    std::string configFile = getConfigFile();
    std::string configFileSave = configFile + ".save";
    if (this->CreateAviAndExit || this->CreateAviOnFlyAndExit)
    {  
      if (GetFileAttributes(configFileSave.c_str()) != INVALID_FILE_ATTRIBUTES)
      {
        conf.load(configFileSave);
      }
      else
      {
        conf.load(configFile);
        conf.save(configFile + ".save");
      }

      conf.set_boolean("gui:flag_show_fps", false);
      conf.set_boolean("gui:flag_menu", false);
      conf.set_boolean("gui:flag_help", false);
      conf.set_boolean("gui:flag_infos", false);
      conf.set_boolean("gui:flag_show_topbar", false);
      conf.set_boolean("gui:flag_show_time", false);
      conf.set_boolean("gui:flag_show_date", false);
      conf.set_boolean("gui:flag_show_appname", false);
      conf.set_boolean("gui:flag_show_fov", false);
      conf.set_boolean("gui:flag_show_selected_object_info", false);
      conf.set_boolean("gui:flag_show_flip_buttons", false);
      conf.set_boolean("gui:flag_show_script_bar", false);
      conf.set_boolean("gui:flag_script_allow_ui", false);
      conf.set_boolean("viewing:flag_gravity_labels", true);
      conf.set_str("projection:type", "fisheye");
      conf.set_str("projection:viewport", "maximized");
      if (resultVideoWidth != -1)
      {
		  conf.set_int("video_output:output_w", resultVideoWidth);
	    }
	    if (resultVideoHeight != -1)
	    {
		  conf.set_int("video_output:output_h", resultVideoHeight);
	    }
	    if (!videoOutputPath.empty())
	    {
		  conf.set_str("video_output:output_video", videoOutputPath);
	    };
	    if (videoCodec != "")
	    {
		  conf.set_str("video_output:codec", videoCodec);
	    };
    ...
    else
    {
      if (GetFileAttributes(configFileSave.c_str()) != INVALID_FILE_ATTRIBUTES)
      {
        conf.load(configFileSave);
        DeleteFile(configFile.c_str());
        DeleteFile(configFileSave.c_str());
    ...
    // Load language codes
    Translator::initIso639_1LanguageCodes(core->getDataDir()+"iso639-1.utf8");
    ...
    if(!initialized)
    {
      if (!CreateAviAndExit && !CreateAviOnFlyAndExit)
		  ...
		  {
			  screenW = conf.get_int("video_output:output_w");
			  screenH = conf.get_int("video_output:output_h");
			  if (screenW <= 0)
			  {
				  screenW = 1024;
			  }
			  ...
			  VideoOutput = conf.get_str("video_output:output_video");
			  VideoCodec = conf.get_str("video_output:codec");
			  initSDL(screenW, screenH, conf.get_int("video:bbp_mode"), conf.get_boolean("video:fullscreen"), core->getDataDir() + "/icon.bmp");
		  }
    }
    ...
    if (tmpstr=="maximized")
        core->setMaximizedViewport(screenW, screenH);
    else
    ...
    setVisionModeNormal();
    if (conf.get_boolean("viewing:flag_chart"))
        setVisionModeChart();
    if (conf.get_boolean("viewing:flag_night"))
        setVisionModeNight();
    ...
    if(scripts)
	  {
		  scripts->play_startup_script(ScriptToRunOnStartUp, (CreateAviAndExit||CreateAviOnFlyAndExit));
	  }
...
void StelApp::update(int delta_time)
{
    ...
    if(!scripts->is_paused())
        core->getImageMgr()->update(delta_time);
...
#endif
{
  if ((CreateAviOnFlyAndExit||CreateAviAndExit)&&!(key == SDLK_q && (mod & KMOD_CTRL)))
	{
		return 0;
	}
    ...
    if (ui->FlagShowTuiMenu)
    {
        ...
        if (state==S_GUI_PRESSED && unicode=='m')
        {
			...
			{
#ifdef WIN32
				//Minimize this instance of Stellarium 
				...
				//Run another copy of Stellarium to save the script to avi
				// 1. Get directory where the current exe is placed
				char ModuleName[1025];
				GetModuleFileName( NULL, ModuleName, 1024);

				char CurrentDirectory[1025];
				GetCurrentDirectory(1024, CurrentDirectory);

				char CommandLine[2048];
				strcpy(CommandLine, ModuleName);
				strcat(CommandLine, " --avi ");
				strcat(CommandLine, SelectedScriptForAvi.c_str());
				...
				//Restore this instance of Stellarium
				::SendMessage(hWnd, WM_SYSCOMMAND, SC_RESTORE, 0);
#endif
			}

			if (SelectedScriptForEditing != "")
			{
#ifdef WIN32
				// Run notepad with the script selected
				char ScriptFullPath [2048] = "notepad ";
				strcat(ScriptFullPath,SelectedScriptForEditingDirectory.c_str());
				strcat(ScriptFullPath,SelectedScriptForEditing.c_str());				

				STARTUPINFO si = { sizeof(si)};
				PROCESS_INFORMATION pi;

				//::ShellExecute(NULL, NULL, NULL, ScriptFullPath.c_str(), 
				//	SelectedScriptForEditingDirectory.c_str(), SW_SHOW);
				CreateProcess( NULL, ScriptFullPath, NULL, NULL, FALSE, 0, NULL, NULL, &si, &pi);
#endif
			}

            // clear out now
            SelectedScriptDirectory = SelectedScript = "";
			SelectedScriptForAviDirectory = SelectedScriptForAvi = "";
			SelectedScriptForEditing = SelectedScriptForEditingDirectory = "";
            return 1;
        }
...
void StelApp::saveCurrentConfig(const string& confFile)
{
	...
	conf.set_boolean("viewing:flag_stop_day_motion", core->getFlagStopDayMotion());
    ...
    conf.set_double ("stars:star_max_radius", core->getStarMaxRadius());
    ...
    conf.set_str ("navigation:startup_time_mode", StartupTimeMode);
	  conf.set_double ("navigation:init_center_fov_ratio", core->getCenterFovRatio());
    ...
    conf.set_boolean("astro:flag_black_planets", core->getFlagBlackPlanets());
...
wstring StelApp::get_printable_date_UTC(double JD) const
{
    ...
    switch(date_format)
    {
        ...
        StelUtility::my_strftime(date, 254, "%x", &time_utc);
        break;
        case S_DATE_MMDDYYYY :
        StelUtility::my_strftime(date, 254, "%m/%d/%Y", &time_utc);
        break;
        case S_DATE_DDMMYYYY :
        StelUtility::my_strftime(date, 254, "%d/%m/%Y", &time_utc);
        break;
        case S_DATE_YYYYMMDD :
        StelUtility::my_strftime(date, 254, "%Y-%m-%d", &time_utc);
        break;
    }
...
wstring StelApp::get_printable_time_UTC(double JD) const
{
    ...
    switch(time_format)
    {
        ...
        StelUtility::my_strftime(heure, 254, "%X", &time_utc);
        break;
        case S_TIME_24H :
        StelUtility::my_strftime(heure, 254, "%H:%M:%S", &time_utc);
        break;
        case S_TIME_12H :
        StelUtility::my_strftime(heure, 254, "%I:%M:%S %p", &time_utc);
        break;
    }
...
wstring StelApp::get_printable_date_local(double JD) const
{
    ...
    {
        case S_DATE_SYSTEM_DEFAULT :
        StelUtility::my_strftime(date, 254, "%x", &time_local);
        break;
        case S_DATE_MMDDYYYY :
        StelUtility::my_strftime(date, 254, "%m/%d/%Y", &time_local);
        break;
        case S_DATE_DDMMYYYY :
        StelUtility::my_strftime(date, 254, "%d/%m/%Y", &time_local);
        break;
        case S_DATE_YYYYMMDD :
        StelUtility::my_strftime(date, 254, "%Y-%m-%d", &time_local);
        break;
    }
...
wstring StelApp::get_printable_time_local(double JD) const
{
    ...
    {
        case S_TIME_SYSTEM_DEFAULT :
        StelUtility::my_strftime(heure, 254, "%X", &time_local);
        break;
        case S_TIME_24H :
        StelUtility::my_strftime(heure, 254, "%H:%M:%S", &time_local);
        break;
        case S_TIME_12H :
        StelUtility::my_strftime(heure, 254, "%I:%M:%S %p", &time_local);
        break;
    }
    return StelUtility::stringToWstring(heure);
}
...
{
    if (df == "system_default")
        return S_DATE_SYSTEM_DEFAULT;
    if (df == "mmddyyyy")
        return S_DATE_MMDDYYYY;
    if (df == "ddmmyyyy")
        return S_DATE_DDMMYYYY;
    if (df == "yyyymmdd")
        return S_DATE_YYYYMMDD;  // iso8601
...
{
    if (df == S_DATE_SYSTEM_DEFAULT)
        return "system_default";
    if (df == S_DATE_MMDDYYYY)
        return "mmddyyyy";
    if (df == S_DATE_DDMMYYYY)
        return "ddmmyyyy";
    if (df == S_DATE_YYYYMMDD)
        return "yyyymmdd";
...
float StelApp::get_GMT_shift(double JD, bool _local) const
{
    if (time_zone_mode == S_TZ_GMT_SHIFT)
        ...
        return StelUtility::get_GMT_shift_from_system(JD,_local);
}
