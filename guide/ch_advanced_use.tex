...
\noindent In the Location panel (see section~\ref{sec:gui:location})
...
On Linux and Mac-OS X, Stellarium does not connect directly to a GPS
USB device, serial device, bluetooth device, etc., but uses a
connection to the \program{gpsd} daemon running on a computer in your
network which provides GPS services concurrently for any interested
application. In most cases, this will be a \program{gpsd} running on
your localhost, receiving data from some GPS device plugged in via
USB.
...
\item On Ubuntu 16.04 and likely other systems, USB hotplug devices
  ...
  such. This converter is also used in other Serial-to-USB converter
...
\item You can also connect to another computer which runs
  ...
  or even your smartphone.  To configure this, you must manually edit
  \file{stellarium.ini}. Find the \texttt{[gui]} section and edit
\begin{configfile}
[gui]
...
bit (8N1), however some devices can be configured for faster transfer.
...
COM ports (e.g., your telescope?), you must find out which COM port is
used by the GPS device and manually edit \file{stellarium.ini}.  Find
