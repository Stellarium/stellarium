...
\subsection{macOS}
\label{sec:FilesAndDirectories:macOS}
...
\item[user directory] This is in the user's home directory, i.e., 
  ...
  or the sub-directory \file{/boot/home/config/settings/Stellarium/}.
\item[screenshot save directory] Screenshots are saved to the user's
  home directory.
\end{description}
...
from Stellarium's Github website\footnote{\url{https://github.com/Stellarium/stellarium-data/releases/tag/dso-3.23}}:

\noindent\begin{tabular}{lllr}
\toprule
\emph{Version; Edition} & \emph{Filename} & \emph{MD5 hash} & \emph{Size}\\\midrule
3.23; extended & \file{catalog-3.23.dat} & \texttt{56dfcdcedc3bfc18ce730984ab039308} & 28MB\\\bottomrule
\end{tabular}
...
\subsubsection{GPSD (Linux, macOS only)}
...
Calling the respective options on the command line overrides settings from environment variables (apart from \texttt{STEL\_OPTS}).
...
Some command-line options or settings can be set permanently by storing them into
...
\noindent These environment variables are important:

\begin{description}
\item[{STEL\_OPTS}] may contain a default commandline with options in the syntax of the table above. Also some Qt settings can only be tweaked with this.
For example, Windows users of international keyboards with \key{Alt Gr} key may want to use
...
Qt based programs can write log files where status messages of varying severity can be 
written\footnote{\scriptsize{\url{https://doc.qt.io/qt-6/qloggingcategory.html}, \url{https://doc.qt.io/qt-6/debug.html}}}. There are categories 
\texttt{debug} (usually only of developer interest), 
\texttt{info} (informational), 
\texttt{warning} (something unexpected), 
\texttt{critical} (usually connected to some real failure) and 
\texttt{fatal} (program fails (usually exits) here).
...
The shadow (defaults shown) can be made slightly darker, but not completely black.\newFeature{26.3}
For additional darkness in screenshots at night, you can set a background opacity for the info panel text.
If you set this number negative, the dark background will be permanent, even in daylight, 
using the color configured as ``Overwrite text color'' (s.~\ref{fig:gui:configuration:tools}).
\begin{configfile}
[gui]
flag_info_shadow=true
info_shadow_radius=6      | [0...64]
info_shadow_opacity=127   | [0...255]
info_panel_opacity=0      | [-100...100]
\end{configfile}
...
\file{normalHtml.css} from Github to your own 
...
This mode does not work in ANGLE mode and requires reasonably modern graphics hardware with the \texttt{WGL\_NV\_DX\_interop} 
...
%% Ubuntu (and maybe other Debian-based distributions) could install Piper in a virtual environment, but how would this be setup system-wide at startup?
%% Other distributions may differ. 


\section{Advanced Text To Speech output}
\label{sec:CommandLineOptions:Special:TTS-Piper}

While speech output on Windows is not bad, the default voices on Linux leave room for improvement. 
We can improve this by configuring better voices with \program{speech-dispatcher}. 
One viable solution uses \program{Piper}\footnote{\url{https://github.com/OHF-Voice/piper1-gpl}}. 
We stick to official documentation.\footnote{\url{https://github.com/OHF-Voice/piper1-gpl/blob/main/docs/CLI.md}}
...
Choose a voice (\url{https://rhasspy.github.io/piper-samples/}) and download. For example:

\begin{commands}
python3 -m piper.download_voices en_US-lessac-medium
\end{commands}

This will download to the current directory. Override with \texttt{--data-dir <DIR>}

Piper runs a web server which allows integration to \program{speechd}.
