...
\subsection{Intermediate Mode: The Lightweight Skylight Model}

This model \newFeature{26.2} combines a realistic view of the sky as in ShowMySky with light weight of the Preetham model, at the cost of being fixed to simulating a sea level sky (just as Preetham), lacking the realism of eclipse simulation (also as Preetham), and having no parameters to tweak, and not supporting OpenGL ES v2 (only v3 is supported, so it won't work with ANGLE included with Qt5).

The Lightweight model based on a set of meshes generated from a render of ShowMySky, so its look is just as realistic (at sea level and without any eclipse) as that of ShowMySky, except for the solar aureole, which is a bit simplified.
