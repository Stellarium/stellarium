...
The Extras tab (Fig.~\ref{fig:gui:configuration:extras}) allows you to customize information
displayed about a selected object, download more star catalogs and also allows to hide or show additional buttons in the lower button bar. If your screen is too narrow to show all buttons, you may have to choose your optimal setup.
\begin{itemize}
\item Customization of information displays.
\item Customization of visibility of buttons on bottom toolbar:
\begin{description}
...
\item[Show HiPS button] You can toggle display of Hierarchical Progressive Surveys with this button.
...
\end{description}
\item Download more star catalogs.
\end{itemize}
...
\newFeature{v0.18.2}In version 0.18.2 we added two additional columns for this tool --- \emph{elongation} and \emph{angular distance}. The first of those columns show angular distance from the Sun, second column --- angular distance from the Moon. Both column may be helpful for planning observation of phenomena.
...
and date and draw it as a graph (Fig.~\ref{fig:gui:AstroCalc:AltVsTime}).
    
\begin{figure}[htbp]
\centering\includegraphics[width=0.75\textwidth]{astrocalc_dialog_altvstime_tab.png}
...
\subsection{The ``Monthly Elevation'' (ME) Tab}
\label{sec:gui:AstroCalc:ME}

\begin{figure}[htbp]
\centering\includegraphics[width=0.75\textwidth]{astrocalc_dialog_me_tab.png}
\caption{Astronomical Calculations (AstroCalc): Monthly Elevation (ME)}
\label{fig:gui:AstroCalc:ME}
\end{figure}

\noindent\newFeature{v0.18.0} This tab can show ``Monthly Elevation'' graph for the current year at selected time. This tool was introduced for planning yearly observations (Fig.~\ref{fig:gui:AstroCalc:ME}).
...
\noindent\newFeature{v0.16.0} This tab can show two functions over time for the current year and draw graphs for them in one screen (Fig.~\ref{fig:gui:AstroCalc:Graphs}). 
You can select from 
\begin{itemize}
...
In version 0.18.2 we added tool for search items from list of \emph{Matching Objects}.\newFeature{v0.18.2}
...
\newFeature{v0.18.2}In version 0.18.2 we enhanced this tool via add new tab (\emph{Graphs}), where you may see the change in the linear and angular distances between selected celestial bodies in range 600 days (\emph{today} is in the center of graphs) as graphs (Fig.~\ref{fig:gui:AstroCalc:PC:Graphs}).
