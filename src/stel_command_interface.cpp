...
int StelCommandInterface::execute_command(string commandline, unsigned long int &wait, bool trusted) {
	...
	if(command == "echo") {
		...
		else if(args["home_planet"]!="") stcore->getObservatory()->setHomePlanet(args["home_planet"].c_str());
		else if(args["landscape_name"]!="") lmgr->setLandscapeByName(args["landscape_name"].c_str());
		...
		else
		{
			if(args["prefix"]=="")
			{
				...
				{
					StelMainWindow::getInstance().saveScreenShot(args["prefix"].c_str(), args["dir"].c_str());
				status = 1;
