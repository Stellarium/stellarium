...
For example, to remove all ``Debug'' messages, set it to \texttt{*.debug=false;}. To remove even the ``Info'' messages, add \texttt{*.info=false;}. (Close each rule, also the last, with semicolon!)
...
Some modules can be finetuned with respect to the amount of logging. 
Most users won't ever want to see the ugly details, so debug is deactivated by default for: 
\begin{description}
...
If you suspect issues with your computer, add those entries as shown above, e.g., add \texttt{stel.GPS.debug=true;}.
