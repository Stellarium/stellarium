...
\begin{description}
\item[\file{stars\_0\_0v0\_16.cat}]
...
\item[\file{name.fab}]
\item[\file{binary_orbitparam.dat}]
\end{description}
...
The file \file{binary_orbitparam.cat} (see section~\ref{sec:StarCatalogues:BinaryOrbitalParameters}) contains binary star system orbital parameters.
...
\midrule
\file{stars\_0\_0v0\_16.cat} & 0 & 48 bytes & 0 &        5,046 & Hipparcos + Gaia\\
...
dx0            & 20 & int                & 4 & First component of the star's proper motion vector at the catalog epoch scaled by $1\times10^3$\\%\midrule
dx1            & 24 & int                & 4 & Second component of the star's proper motion vector at the catalog epoch scaled by $1\times10^3$\\%\midrule
dx2            & 28 & int                & 4 & Third component of the star's proper motion vector at the catalog epoch scaled by $1\times10^3$\\%\midrule
b\_v           & 32 & short              & 2 & $B-V$ colour in milli-mag unit.\\%\midrule
mag            & 34 & short              & 2 & $V$-band magnitude in milli-mag unit.\\%\midrule
plx            & 36 & unsigned short     & 2 & parallax of the star in 0.02 mas unit.\\%\midrule
...
HIP/Gaia & int    & This is the HIP or Gaia DR3 identifier for the star. Used for reference to the main star catalogue.\\%\midrule
...
in the \file{stars/hip_gaia3} sub-directory of the Installation Directory and/or
User Directory (see section~\ref{sec:Directories}).
...
in the \file{stars/hip_gaia3} sub-directory of the Installation Directory and/or User Directory (see section~\ref{sec:Directories}).
...
The file \file{binary_orbitparam.dat} contains cross binary system orbital parameter data, which stored in plain text format.

\begin{table}[htb]
	\begin{tabularx}{\textwidth}{l|c|X}\toprule
	  \emph{Name}      & \emph{Type} & \emph{Description}                     \\%\midrule
	  binary_period    & double      & Orbital period [days]                  \\%\midrule
	  eccentricity     & float       & Eccentricity                           \\%\midrule
	  inclination      & float       & Orbit inclination [rad]                \\%\midrule
	  big_omega        & float       & Position angle of ascending node [rad] \\%\midrule
	  small_omega      & float       & Argument of periastron [rad]           \\%\midrule
	  periastron_epoch & double      & Julian Date at periastron passage      \\%\midrule
	  semi_major       & double      & Angular semi-major axis [arcsec]       \\%\midrule
	  bary_distance    & double      & distance [parsec]                      \\%\midrule
	  data_epoch       & double      & Julian Date of the data                \\%\midrule
	  bary_ra          & double      & Right Ascension of the barycenter [rad]\\%\midrule
	  bary_dec         & double      & Declination of the barycenter  [rad]   \\%\midrule
	  bary_rv          & double      & Barycenter radial velocity [km/s]      \\%\midrule
	  primary_mass     & float       & Primary star mass [solar mass]         \\%\midrule
	  secondary_mass   & float       & Secondary star mass [solar mass]       \\%\midrule
	  bary_pmra        & double      & Barycenter RA proper motion [mas/yr]   \\%\midrule
	  bary_pmdec       & double      & Barycenter DEC proper motion [mas/yr]  \\\bottomrule
	\end{tabularx}
