...
In case you have created your own skycultures in the old format, we have created a format converter  described in section~\ref{ch:SkyCultures:converter}.
For new skycultures, a dedicated plugin assists you (\ref{sec:plugins:SCM}).


If you add a new or extend an existing sky culture, please adhere to this description for an optimal result!
...
\item (optional) a definition\newFeature{26.1} of geographic distribution of the skyculture over time (see \ref{SC:territory.geojson})
...
The file is automatically broken into pieces, translated and converted to HTML for display or else processed for speech output. Also some important skyculture components are extracted from here.
...
subsection titles (HTML: \texttt{h2} or \texttt{h3}, respectively).  All header lines (except the first) should be enclosed by empty lines. 
...
the history or background of sky culture research that provided this work, tables of names/translations,
links to external resources, whatever seems suitable.  The length of the description texts is not limited, 
but of course if this is your PhD thesis work, there is no need to repeat all 500 pages, you can add a link to the references.

The skyculture description should include the following Level 2 sections. Any other section must be a subsection (i.e. Level 3 or deeper) of one of these sections. 
Each of these sections except \texttt{Constellations} will have its own entry in the translations files. 
The \texttt{Constellations} section will be split, so that each constellation's description will be translated separately.
...
\item[\texttt{Constellations}] -- a sequence of Level 5 sections, where the header is the \jtag{english} name of a constellation (as given in the \file{index.json} file), and the body gives some information on this constellation. This text is used\newFeature{25.4} for display and speech output when the constellation is selected.
...
Note that the \texttt{Constellations} section has the strict format described above. 
In particular, there's no place for an introduction for all the constellations, 
or closing remarks or a similar deviation from the format of this section. 
If this kind of text is desired, it can be placed in a subsection of the \texttt{Description} section.
...
PNG or JPG format please. Use the following syntax, where the last line is fixed 
and is required for proper formatting. 


\begin{configfile}[\scriptsize]
![](illustrations/my_image.png)
My image caption 
{: .img_and_caption }
\end{configfile}

Alternatively, you can format an image in HTML if you need specialized extras like right-alignment or size specifications:

\begin{configfile}[\scriptsize]
<p>
<img src="illustrations/my_image.png" width="150" align="right"/>
My image caption
{: .img_and_caption }
</p>
\end{configfile}

or (note the manual \texttt{<br/>})

\begin{configfile}[\scriptsize]
<p class="img_and_caption">
<img src="illustrations/my_image.png" width="150" align="right"/><br/>
My image caption
{: .img_and_caption }
</p>
\end{configfile}
...
\label{sec:skycultures:Testing}

Using correct syntax is required for making display and translation work. 
If you have checked out Stellarium's sources, you can run

\begin{commands}
util/skycultures/generate-pot.py -s my_culture
\end{commands}
This may emit some warnings about issues that should be fixed (if any remain). 
It will also generate a \file{.po} directory in the skycultures directory, 
but these files aren't supposed to be committed, you can remove it.
...
It allows some form of additional grouping. The names of regions are largely following the United Nations
...
\begin{description}
	...
	\item[South America] -- Argentina, Bolivia (Plurinational State of), Bouvet Island, Brazil, Chile, Colombia, Ecuador,
...
In case you are illustrating a fictional sky culture not actually used on Earth, you can use \textbf{Other} or even whatever else you like. They will be listed in the ``Other'' category in the View dialog.
...
\begin{description}
\item[\jtag{id}] A unique figure identifier, consisting of \texttt{CON} (for \indexterm[constellation]{constellations}), skyculture id, 
...
\item[\jtag{common\_name}] Dictionary of names. %Required entries: \jtag{english}. Optional: \jtag{native}
	...
	\begin{description}
	    ...
	    \item[\jtag{byname}] an optional explanatory addition to the name. \newFeature{25.3}  May be helpful to explain mythological figures. 
		This is translated to users' language, but is not included in search functions.
	    \item[\jtag{pronounce}] (optional, but recommended) a Latin-character based transliteration/pronunciation aid, e.g.\ Pinyin for Chinese. 
		This may also be translated to user language.
	    \item[\jtag{transliteration}] (optional) a scientific Latin-character based transliteration where applicable. 
	    ...
	    \item[\jtag{context}] (optional) Short english explanation that helps to disambiguate terminology. Not displayed in the program.
	    \item[\jtag{translators\_comments}] (optional) English explanations that help in translation on Transifex. Not displayed in the program.
	\end{description}
	...
	the \jtag{native} language, or a \jtag{byname} which may help better understanding a figure. Else translators will not be able to translate
...
\item[\jtag{label\_offset}] optional. An array of \texttt{[deltaRA, deltaDE]} (decimal degrees)
...
\begin{description}
\item[\jtag{id}] A unique asterism identifier, consisting of \texttt{AST} (for asterism), skyculture id, and a short Latin-character based label still unique within the skyculture.
...
\item[\jtag{common\_name}] Another dict with entries \jtag{native} (as used in the source), \jtag{english} (the translatable english name), 
                \jtag{pronounce}, \jtag{transliteration}, \jtag{IPA}, \jtag{byname}, \jtag{context} and \jtag{translators\_comments} as defined for constellations,
...
Ancient \newFeature{25.2}high cultures in and around Asia took special note of the zone where the planets move: 
...
The best-known system in modern times is that of the \indexterm[zodiac!tropical]{tropical zodiac} of $12\times30$ degrees along the ecliptic, 
counted from the equinox point (First point of Aries \Aries). The 12 sections, or \indexterm{zodiacal signs}, 
...
In India, the partitions were defined so that the star Spica rests at the 180° mark, 
so that the Indian or \indexterm[zodiac!sidereal]{sidereal zodiac} moves along the ecliptic with precession. 

\begin{jsonfile}[\scriptsize]
"zodiac": { 
  "name": { "pronounce": "%(*Rāśi*)", "english": "Indian Zodiac"},
  "context": "indian zodiac sign",
  "partitions": [ 12, 3, 2, 5],
  "extent": 9,
  "link": { "star": 65474, "offset": 180},
  "names": [ { "symbol": "\u2648", "native": "...", "english": "Ram"},
             { "symbol": "\u2649", "native": "...", "english": "Bull"},
               ...
             { "symbol": "\u2653", "native": "...", "english": "Fishes"}]},
\end{jsonfile}  
where 
\begin{description}
\item[\jtag{name}] cultural name as described in \ref{SC:common_names}.
\item[\jtag{context}] (optional, recommended) context data for all English names in zodiac (enhancement of localization support).
\item[\jtag{comment}] (optional) commentary for zodiac, it will be used as extra commentary for translators also.
\item[\jtag{partitions}] regular division of the centerline. The first element commonly is 12 (30-degree sign). 
...
\item[\jtag{extent}] The zone of interest is closed off by parallel lines this many degrees north and south of the centerline. 
A zodiac of 9 degrees encloses extreme positions of all major planets. 
\item[\jtag{link}] (optional) The Indian \emph{sidereal zodiac} is described to be defined with the longitude of a certain star. 
Usually and in this sample, Spica ($\alpha$ Vir, HIP~65474) is declared to lie at 180°.  If missing, the zodiac starts from \Aries. 
\item[\jtag{names}] Sequence of names. In addition to the elements listed in \ref{SC:common_names}, \jtag{symbol} provides a Unicode symbol 
...
\noindent More important for Indians however, and still relevant today, is the traditional system of \indexterm{lunar stations}, the \indexterm{nakshatras}. 
...
\begin{description}
\item[\jtag{name}] cultural name as described in \ref{SC:common_names}.
\item[\jtag{context}] (optional, recommended) context data for all English names in lunar mansions (enhancement of localization support).
\item[\jtag{comment}] (optional) commentary for lunar system, it will be used as extra commentary for translators also.
\item[\jtag{partitions}] regular division of the centerline. 27 stations which are further divided in 4 equal parts.
\item[\jtag{extent}] The zone of interest. With an obliquity of the Lunar orbit of about 5 degrees, a 5-degree zone seems appropriate.  
\item[\jtag{link}] (optional) The Indian system is again described to be defined with the longitude of Spica ($\alpha$ Vir, HIP~65474) to lie at 180°.  
...
\begin{jsonfile}[\scriptsize]
"lunar_system": {
  "name": { "native": "...", "pronounce": "%(*Xiù*)", "english": "Lunar Mansion"},
  "context": "chinese lunar mansion",
  "defining_stars": [65474, 69427, ...],
  "coordsys": "equatorial",
  "extent": 90,
  "names": [ { "symbol": "1", "native": "...", "pronounce": "%(*Jiǎo*)", "english": "Horn"},
             { "symbol": "2", "native": "...", "pronounce": "%(*Kàng*)", "english": "Neck"},
               ...
\end{jsonfile}  
where 
\begin{description}
...
\item[\jtag{context}] (optional, recommended) context data for all English names in lunar mansions (enhancement of localization support).
\item[\jtag{comment}] (optional) commentary for lunar system, it will be used as extra commentary for translators also.
\item[\jtag{defining\_stars}] List of HIP numbers of the 28 defining stars.
\item[\jtag{coordsys}] may be \jtag{equatorial} or \jtag{ecliptical} (default).
\item[\jtag{extent}] The zone of interest. The Chinese mansions extend from pole to pole.   
\item[\jtag{names}] Sequence of names. In addition to the elements listed in \ref{SC:common_names}, \jtag{symbol} provides a Unicode symbol 
or 1-2 letter string (for practical reasons, a number seems generally useful). 
The number of entries must be equal to (or larger than) the number of elements of \jtag{defining\_stars}. Excessive entries are ignored. 
\end{description}
...
\begin{description}
    ...
    \item[\jtag{transliteration}] (optional) a scientific Latin-character based transliteration where applicable. Not translatable to user language, but may use extended diacritics.
	The scheme may not be helpful as pronunciation aid. For example, Wylie for Tibetan.
    \item[\jtag{IPA}] (optional) International Phonetic Alphabet, a standardardized phonetic spelling.
    \item[\jtag{byname}] an optional explanatory addition to the name for constellations and stars only. \newFeature{25.3} May be helpful to explain mythological figures. 
	This is translated to users' language, but is not included in search functions.
    \item[\jtag{context}] (optional) Short english explanation that helps to disambiguate terminology. Not displayed in the program.
...
\subsubsection{No translation in Modern Skyculture?}
...
In contrast to what we recommend for non-modern skycultures, we still keep the original name as translation in the \jtag{english} string, 
...
At best, the \jtag{byname} can transport a translation of the meaning to user language. 
...
\item{\texttt{Translit}} to show a scientific transliteration that is not necessarily aimed at pronunciation.
...
\end{configfile}
%
Here, \texttt{modern} names the skyculture, and the comma-separated entries are references listed in 
the file \file{description.md} or asterism abbreviations you want to be ignored.


\section{Territory description}
\label{sec:skycultures:territory}
\label{SC:territory.geojson}

Most traditional skycultures are regional.\newFeature{26.1} The map interface in the View dialog should indicate where on Earth a skyculture is located. 
Stellarium makes use of a file in the GeoJSON format, \file{territory.geojson}, to encode where and when this culture is located. 
In case a cultural group can be traced over a longer time and the culture has moved or spread, the time slider below the map will show the changes in the geographic regions.  

GeoJSON is a flexible and simple JSON-based format to encode a collection of geographic features (points, lines and polygons) with geographic coordinates (pairs of longitude/latitude in decimal degrees, WGS84). 
Additional properties can be given in a \jtag{properties} dictionary.  Stellarium only uses polygonal features, and stores \jtag{beginDate} and \jtag{endDate} data (calendar years) for each polygon. 
If the end date should be ``now'', this is indicated by a special value \texttt{9146}.

% Internal documentation: Moritz Rätz previously used a self-coined JSON format with string properties. It used "INFINITY SYMBOL" as special value.   9-14-6 = I-N-F 

An example (Norse (Edda) SC):
  ...
  "features": [
    {
      ...
      "geometry": {
        "coordinates": [
           [
                [ -22.746442921526324, 64.13294959328397 ],
                [ -22.437762179297245, 64.03176502170129 ],
                ...
                [ -22.81812760274378, 64.08885223306777 ]
           ]
        ],
        "type": "Polygon"
      }
...
\item[\texttt{-\/-translated-md}] To check the look of the translated description texts you can use this option. 
      The output directory will contain files named like 
      \file{description.es\_419.DO\_NOT\_COMMIT.md}, in addition to \file{description.md}, 
      with the \texttt{DO\_NOT\_COMMIT} part reminding you that they are not a part of the sky culture.
\end{description}
